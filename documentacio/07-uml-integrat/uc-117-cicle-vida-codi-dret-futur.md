...
**Exemple normatiu funcional DEC-23, NO execució en BD:** valor JASOM 90 € → consum 90 € → baixa amb rectificativa crea un dret de baixa 90 € → nou curs utilitza 40 € i el dret de baixa conserva 50 € → en retornar JASOM, immobilitzar els 50 € i proposar recuperar NOMÉS 40 €, sense tornar a reclamar els 90 € històrics. Encara NO hi ha servei fiscal de canvi/baixa, concessions de saldo derivat `ACTIVE`, ús derivat ni endpoint de cancel·lació que executi aquest pla. Els [tretze tests purs de procedència](../../sif/tests/Unit/NovicePromotionLineagePolicyTest.php) i [set de partició/traspàs](../../sif/tests/Unit/NovicePromotionDestinationAdjustmentPolicyTest.php) estan escrits, no executats amb MySQL (fase ajornada).
### 1.5. UC-111 — aprovació interna de baixa i primer traspàs sense nova concessió

La [font d'aprovació de canvis i baixes](../../sif/src/Service/NovicePromotionAdjustmentApprovalSourceInterface.php) és un CONTRACTE intern pendent d'adaptador real amb secretaria: abans de mutar drets cal una decisió fiscal/comercial autenticada i FINAL, coherent amb imports, referències d'aplicació, rectificativa i data. No n'hi ha prou amb un `PENDING_FISCAL_REVIEW`, un actor lliure o una rectificativa que existeixi.

[NovicePromotionDerivedBalanceActivationService](../../sif/src/Service/NovicePromotionDerivedBalanceActivationService.php) pot transformar atòmicament la primera aplicació novell APPLIED en antecedent històric `REVERSED/CONVERTED_TO_DERIVED` i activar el nou dret de baixa amb import PROMOCIONAL aprovat i vigència pròpia +1 any, revalidant JASOM/rectificativa i deixant els diners reals al circuit monetari separat. [NovicePromotionFirstTransferConfirmationService](../../sif/src/Service/NovicePromotionFirstTransferConfirmationService.php) pot tancar la primera aplicació com a `REVERSED/TRANSFERRED_TO_COURSE` i registrar la transferència CONFIRMED amb factura final i cobrament residual conciliat al curs nou. **La transferència NO és un segon consum ni un saldo concedit de nou:** el registre de traspàs representa l'aplicació activa actual. La [migració 000016](../../sif/database/migrations/2026_09_25_000016_confirm_novice_promotion_course_transfer.sql) afegeix factura final, preu anterior/posterior, decisió i traça del primer traspàs.

**Encara NO HI HA gestor genèric UC-117 complet:** manca el connector d'aprovació AUTENTICADA, l'orquestració fiscal/cobraments i el checkout real, les transferències SUCCESSIVES, baixa del curs successor, bescanvi de drets derivats, cancel·lació per devolució JASOM i l'adaptador que converteixi `STATUS=REVERSED` + `REASON_CODE` + registres derivats/traspassos en una fotografia de procedència sense duplicació. Els serveis són interns i no desplegats. [Cinc tests purs de baixa](../../sif/tests/Unit/NovicePromotionApprovedCancellationPolicyTest.php) i [cinc de primer traspàs](../../sif/tests/Unit/NovicePromotionApprovedTransferPolicyTest.php) escrits, sense execució; MySQL ajornat.
## 2. UML de casos d'ús
