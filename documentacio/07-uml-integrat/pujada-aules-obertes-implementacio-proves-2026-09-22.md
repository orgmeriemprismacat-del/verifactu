...
**Verificació automàtica real sobre els fitxers d'aquesta branca:** [GitHub Actions «Intranet AO batch checks» — execució 35770377775](https://github.com/orgmeriemprismacat-del/verifactu/actions/runs/35770377775), **conclosa amb èxit** el 22/09/2026 sobre el commit `bcbea0d0b826049d7c0338f11ed363b6079a0b63`. El job configura **PHP 8.2 amb mbstring i mysqli**, executa `sh codi-drive/intranet-actual/tests/run_ao_checks.sh`, incloent `php -l` de la pàgina, `Intranet.php`, els endpoints nous/antics, el helper CSV i la prova; `node --check` sobre el JS complet; i **els nou casos unitaris CSV amb mbstring real**. Job `syntax-and-csv`: **SUCCESS**. El PR es manté en esborrany i aquest resultat és sintaxi + proves pures, no una prova de BD o servidor desplegat.

**Verificació local addicional anterior:** la CLI disponible a l'entorn d'anàlisi no tenia `mbstring`, per això el test retornava **SKIP (77)** en aquell entorn. Es van provar els nou casos amb una emulació temporal `iconv`, però **la prova amb mbstring real és ara la de GitHub Actions anterior**; no continuar presentant-la com a pendent.

**No executat en aquest tall:** tests contra la BD de la intranet, sessió/rol real, errors de disc/concurrència reals, prova a producció i els **17 escenaris d'acceptació integrats** de la fitxa. **No afirmar 17/17, desplegament, ni `FINAL IMPLEMENTAT EN PRODUCCIÓ`.**
