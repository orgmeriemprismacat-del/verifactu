...
partition "Servidor de sol·licituds" {
  ...
  :Secretaria redacta i envia MANUALMENT el correu d'accés després de l'alta real (DEC-108-05b/c), també amb butlletí No; no programar enviament automàtic al web;
...
:El correu operatiu d'accés activat el REDACTA I ENVIA MANUALMENT secretaria després de l'alta real (DEC-108-05b/c), no el genera ni envia automàticament aquest endpoint; automatització desitjada per al 2027 i fora d'abast;
...
if (Accés Moodle confirmat després d'alta manual?) then (Sí)
  ...
  :Mostrar l'estat verificat; secretaria redacta i envia MANUALMENT el correu d'accés després de l'alta real, també amb butlletí No (DEC-108-05b/c). Obrir la pàgina no ha de disparar cap correu automàtic;
else (No)
