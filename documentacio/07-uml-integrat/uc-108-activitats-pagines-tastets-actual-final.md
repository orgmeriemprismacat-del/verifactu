...
:El correu operatiu d'accés activat es prepara a partir d'una PLANTILLA JA EXISTENT i l'ENVIA MANUALMENT secretaria després de l'alta real (DEC-108-05b/c/d); aquest endpoint no el genera ni envia automàticament; l'automatització del campus prevista per al 2027 és fora d'abast;
...
if (Accés Moodle confirmat després d'alta manual?) then (Sí)
  ...
  :Mostrar l'estat verificat; secretaria prepara a partir d'una plantilla existent i envia MANUALMENT el correu d'accés després de l'alta real, també amb butlletí No (DEC-108-05b/c/d). Obrir la pàgina no ha de disparar cap correu automàtic;
else (No)
