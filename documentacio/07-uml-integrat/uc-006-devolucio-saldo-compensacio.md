...
## 6. El botó llegat «Anul·lar factura» no prova una devolució monetària

[Auditoria de la pantalla d'intranet i subflux AF-ANUL](02-auditoria-pantalla-alumnes-factura-2026-09-25.md). El [mètode llegat `anularFactura()` L14931–15193](../../codi-drive/intranet-actual/Intranet.php#L14931-L15193) rep `tornar` i data per GET, crea un registre negatiu a `factures` i, segons `TIPUS_INSC` i el nombre d'inscripcions vinculades, ajusta `PAGAMENT`, dates/fraccionament i observacions. **No s'hi ha acreditat** cap confirmació de retorn bancari, `REFUND` del ledger SIF ni autorització del titular al mateix mètode. Distingir tres fets que poden no coincidir: correcció de factura, saldo/assignació i devolució efectivament executada; validar ingrés real, titular, destinatari del retorn i repartiment de grup abans de crear el moviment. Els resums llegats s'han de conciliar amb el resultat real i no ser-ne la font que inventi diners retornats. **AF-T05/06** no executats; cap baixa/correu/Moodle forma part d'aquesta auditoria de pantalla.
