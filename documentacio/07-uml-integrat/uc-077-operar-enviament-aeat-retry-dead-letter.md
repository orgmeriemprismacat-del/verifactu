...
**SEQUÈNCIA MIXTA:** la comprovació de payload/registre i quarantena d'integritat marcada `[PHP main]` ja existeix; les altres accions `[pendent]` són disseny i no esdevenen implementades per aparèixer al diagrama. El registre fiscal i la cua es creen prèviament i es confirmen en una transacció local; la crida externa AEAT es fa després del commit.
...
| --- | --- | --- |
| UC77-INT-01 | Alterar el payload JSON fiscal de la cua després de congelar-lo, mantenint intacte el registre original | **PHP main** ha de rebutjar abans de `send()` via `assertImmutablePayload`, passar a DEAD_LETTER i obrir incidència FISCAL_PAYLOAD_CONFLICT; prova definida al repositori, no executada aquí. |
...
| UC77-DLQ-05 | Tres reclamacions amb fallades temporals acreditadament anteriors al transport | Màxim tres claims amb els valors per defecte; dos ajornaments i DEAD_LETTER al tercer; el cicle complet UC-81/outbox UC-58 continua pendent. |
