...
| Edició de dades | El wrapper existent passa diversos camps de l'edició a `Intranet::desarCanvisDadesEdicio()`. | **REVISAT AMB COS COMPLET després d'aquest primer lot:** el mètode executa UPDATE sobre `cursos` (LIKE prefix) i `curs` (clau exacta), emet SQL/valors a la resposta i no mostra transaction/rollback; [fitxa funcional amb font exacta](../06-fitxes-funcionals/uc-114.md#23-fitxa-funcional-de-les-accions-reals-dedició--contrast-del-codi-complet-22092026). El desplegament i les proves resten sense acreditar. |
| Canvi d'estat | El wrapper invoca `Intranet::desarCanvisEstatEnviarMsg_PreviIniciCursos()` amb estat anterior i nou. | **REVISAT AMB COS COMPLET després d'aquest primer lot:** anul·lació marca `INSC CURS=0` com `X`, mentre que la regla de negoci confirmada exigeix mantenir la inscripció inicial; pendent i actiu tenen altres efectes/avisos; [fitxa UC-127 i UML per estat](uc-127-canvi-estat-edicio-operacions-afectades.md). Desplegament i proves resten pendents. |
...
| --- | --- | --- | --- | --- |
| A114-01 · Desar dades d'edició | [`desarCanvisDadesEdicio.php` L16–32](../../codi-drive/intranet-actual/ajax/cursos/desarCanvisDadesEdicio.php#L16-L32) | Rep per POST `idCurs`, `nomCurs`, `dataI/dataF`, `hores`, `cursEsc`, codis GTAF/FISS, dates de resolució/qualificació/bloqueig, valors associats i observacions; els passa al mètode de la intranet. **Aquest wrapper NO rep `idPreu`.** | Cos **ja recuperat i contrastat**: dues escriptures `cursos`/`curs`, debug i absència de rollback visible; diferenciar nom/hores/dates contractuals dels camps administratius. Identificar la via **independent** que canvia `ID_PREU` (normalment preus per hores, reassignació excepcional des de BD), no presumir que aquest formulari ho fa. Fer previsualització de les operacions obertes, proposta versionada, autorització i publicació controlada quan hi ha efecte comercial/fiscal. | UC-114/112/115/121; **V114-01**. |
| A114-02 · Desar dades d'aula | [`desarCanvisDadesAulaEdicio.php` L16–24](../../codi-drive/intranet-actual/ajax/cursos/desarCanvisDadesAulaEdicio.php#L16-L24) | Rep `idCurs`, `idAula`, aula, dates de revisió/informe i observacions; invoca `desarCanvisDadesAulaEdicio()`. | Cos **ja recuperat i contrastat**: UPDATE `cursos`, després validació `idAula != 0`, després UPDATE `aula`; si `idAula=0`, la primera escriptura pot haver tingut lloc i el mètode llança error sense rollback visible. No actualitza directament places, factures ni preu en aquestes sentències. Classificar avís només si l'efecte real modifica condicions comunicades. | UC-114/115/129 segons efecte real; **V114-02**. |
...
| A114-04 · Afegir edicions des de CSV | [pàgina d'importació](../../codi-drive/intranet-actual/cursos-afegir-modificar-edicions.php), [JS L50–115](../../codi-drive/intranet-actual/js/cursos-afegir-modificar-edicions.js#L50-L115), [JS L210–310](../../codi-drive/intranet-actual/js/cursos-afegir-modificar-edicions.js#L210-L310), [`inserirCurs.php` L16–33](../../codi-drive/intranet-actual/ajax/cursos/inserirCurs.php#L16-L33) | Després de validar fitxer/data/tràmit en client i analitzar CSV, l'usuari selecciona files; el JS envia una petició `inserirCurs.php` per fila i una altra petició `inserirNumTramit.php`, **sense esperar la resolució de totes les insercions al codi JS examinat**. `inserirCurs.php` rep entre altres `idCurs`, any/mes, `idPreu`, `public`, dates i hores. | Cos **ja recuperat i contrastat**: `insertCurs()` executa tres INSERT successius `curs`, `aula`, `cursos` sense transaction/rollback visible dins del mètode; el tràmit és una crida a part. Tractar creació nova i versió d'edició existent com a variants, deduplicar reimportació i provar errors parcials reals. `inserirCurs.php` rep `idPreu`, i no s'ha d'equiparar a l'edició de `ID_PREU` ja assignat a una edició existent. | UC-114/106/115 i cas d'importació a classificar; **V114-04/05**. |

**ACTUALITZACIÓ del lot:** s'ha recuperat íntegrament `Intranet.php` amb la lectura del blob GitHub i s'han verificat el cos dels mètodes de desar edició/aula, `insertCurs()` i `desarCanvisEstatEnviarMsg_PreviIniciCursos()`. Les consultes SQL corresponents també són al constructor de la classe. **No** s'han verificat el desplegament, les transaccions externes, tots els handlers JS ni els resultats reals de BD/correus/Moodle; això no equival a considerar el cas provat.
...
note right
  Aquest és el wrapper AJAX inicial.
  Els UPDATE, la resposta de debug i
  les alternatives del mètode complet
  consten als diagrames de la fitxa UML.
  Permisos desplegats pendents de test.
end note
...
**Abans de declarar UC-114 cobert:** els cossos d'`Intranet::desarCanvisDadesEdicio`, `desarCanvisDadesAulaEdicio`, `desarCanvisEstatEnviarMsg_PreviIniciCursos` i `insertCurs` ja han estat recuperats i contrastats. **Resta** traçar les accions i handlers JS de tots els apartats/modal de les pantalles, la via real de canviar `ID_PREU` en BD, l'efecte de cada integració en entorn de proves i el desplegament actiu; executar les proves V114/127 amb BD, correus, Moodle i callbacks, i documentar captures i discrepàncies.
...
**Rectificació documental posterior al primer contrast (22/09/2026):** no prendre les afirmacions de «cos no recuperat» d'alguna cel·la històrica com a estat vigent. Les [fitxes funcionals UC-114](../06-fitxes-funcionals/uc-114.md#23-fitxa-funcional-de-les-accions-reals-dedició--contrast-del-codi-complet-22092026) i [UC-127](../06-fitxes-funcionals/uc-127.md#23-fitxa-funcional-contrastada--gestió-de-lestat-dedició-22092026) i les seves [UML UC-114](uc-114-versionar-producte-edicio.md#7-diagrames-dactivitat-contrastats-per-acció-de-la-pantalla) i [UC-127](uc-127-canvi-estat-edicio-operacions-afectades.md#7-diagrames-dactivitat-de-lestat-dedició-amb-el-php-complet) incorporen els nous resultats. Els fluxos parcials inicials d'aquest informe són història de la progressió de l'auditoria, no l'única representació actual.
