...
| AL-11 | Icona informació d'inscripció i navegació acadèmica | [JS L837–845 i L1009–1072](../../codi-drive/intranet-actual/js/alumnes-mostrar-alumne.js#L1009-L1072); mostra modal i enllaços a curs, Moodle nou/antic, participants, qualificacions i seguiment segons icona. | UC-042/124: autorització per inscripció i destí; T-AL-F06 consulta i permisos d'altres inscripcions i campus. |
...
**Conjunt de proves de la pàgina:** T-AL-08–17 i T-AL-F01–F12, més T-AL-11/12 de baixa/canvi i T-125-INTRA de consentiment, a executar **en un entorn amb dades sintètiques i autorització**. Cobertura mínima: una persona amb diverses inscripcions, una de grup amb pagador diferent, una de pack, una de regal, factura existent/absent, modal certificat i les variants d'error. Cada test ha de registrar: commit desplegat, fitxer JS minificat real, actor/rol, dades de prova sintètiques, passos, evidència de resposta tipificada, lectura BD abans/després, Moodle/correu quan pertoqui, resultat real i responsable de validació.

**Bloquejos externs de tancament funcional:** (a) reproduir les captures mancants de formularis/confirmació **amb dades sintètiques o ocultes**; (b) verificar els permisos dels endpoints i el deployed asset; (c) provar el resultat i reversió d'errors sense tocar alumnes reals; (d) resoldre per un canal privat la revisió dels fitxers generats publicats i la neteja de temporals; (e) implantar els canvis del backend/JS i repetir la suite. Cap d'aquests punts es considera realitzat per la mera existència de diagrames.
...
**Resultat declarat:** `DOCUMENTALMENT_INVENTARIAT_AL_TALL_MAIN`; `FUNCIONALMENT_PENDENT_DE_TEST`; `IMPLEMENTACIÓ_FINAL_NO_ACREDITADA`; `SEGURETAT_DE_CERTIFICATS_PENDENT_DE_REVISIÓ_PRIORITÀRIA`. **No** ampliar aquesta etiqueta a totes les aplicacions ni als 142 UC.
