...
**CASOS SEPARATS CONFIRMATS:** (A) crear inscripció a PrisMa; (A2) importar matrícules en lot **a la intranet**, amb traça de l'entrada i del destí per contrastar; (B) preparar pujada a Moodle d'inscripcions ja existents, i dins de Moodle, (B1) pujada d'alumnes als cursos i (B2) pujada d'aules obertes: **B1 i B2 requereixen dos casos d'ús i diagrames independents**, amb identificadors pendents de comprovar contra el catàleg. Els diagrames d'activitat del lot 04 cobreixen només UNA ACCIÓ de B1 i no constitueixen el diagrama acabat d'UC-113.

**IMPORTADOR EXISTENT CONFIRMAT PER NEGOCI:** PrisMa **ja disposa d'un importador de matrícules en lot A LA INTRANET**. L'existència funcional no implica que el seu parser, pantalla, sistema destí o classe hagin estat identificats en l'arbre consultat. `enrollment_import_run/item` i `EnrollmentImportService` són **model/servei SIF proposats per adaptar l'importador existent**, no prova que calgui desenvolupar un importador des de zero. No confondre l'importador existent amb el generador de CSV de la pantalla Moodle. Vegeu la [fitxa funcional consolidada](../06-fitxes-funcionals/uc-113.md) i els [diagrames UC-113 actual/final](uc-113-activitats-alta-manual-i-importador-lot.md).
...
**Mantenir UC-113 amb el seu contracte propi:** localitzar les rutes reals de l'alta manual i de l'**importador en lot existent**, els formats i permisos observables; migració `enrollment_import_run/item` = SQL DEFINIT i `EnrollmentImportService`/`Repository` són una **possible adaptació SIF** no acreditada al PHP d'aquest commit, no la mesura d'existència de l'importador real. Fer proves de duplicats entre lots, files fallides, historial i cap factura/CHARGE inferit d'una marca acadèmica. No construir un parser a partir del format de CSV de Moodle: aquest format representa dades destinades a un altre sistema.
