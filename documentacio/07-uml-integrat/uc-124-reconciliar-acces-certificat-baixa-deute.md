...
## 7. Traça real de la pantalla de consulta i descàrrega de certificats

**Punt d'entrada real:** [fitxa alumne i els diagrames AL-CERT actual/final](00-captures-auditoria-alumnes-consulta-modifica-2026-09-22.md#7-cerca-avançada-dades-personals-observacions-i-certificats--contrast-sense-captures-noves), [`alumnes-mostrar-alumne.js` L881–900 i L2327–2487](../../codi-drive/intranet-actual/js/alumnes-mostrar-alumne.js#L2327-L2487), [`mostraModalConsultaCertificat.php`](../../codi-drive/intranet-actual/ajax/alumnes/mostraModalConsultaCertificat.php) i [`mostrarCertificat.php`](../../codi-drive/intranet-actual/ajax/alumnes/mostrarCertificat.php).

**ACTUAL verificat per lectura de codi:** una icona de certificat digital o de cursant obre el modal per `idInsc`; [`Intranet::modalConsultaCertificat_resultatCerca()` L9892–9938](../../codi-drive/intranet-actual/Intranet.php#L9892-L9938) construeix `INSCRIT` o variants `DIGITAL/PAPER/SOBRE`; [`generaCertificat()` L9955–10320](../../codi-drive/intranet-actual/Intranet.php#L9955-L10320) prepara HTML, consulta dades acadèmiques i, si es demana descarregar, renderitza PDF temporal i retorna un nom generat amb identificació de persona/curs. La vista/preview no acredita per si mateixa que la matrícula, superació, deute i pagador s'hagin reconciliat amb l'estat SIF; **el mètode llegat de renderització no constitueix el servei de conciliació UC-124**.

**FINAL proposat, sense imposar un bloqueig per impagament no acordat:** abans d'habilitar una modalitat de certificat, consultar inscripció, progrés/acreditació i la política acadèmica/econòmica **que estigui aprovada**; si manca decisió, marcar revisió pendent en lloc de deduir automàticament prohibició per una factura d'empresa impagada. Autoritzar consulta/descàrrega per objecte al servidor, servir PDF des de magatzem privat amb nom opac, registrar lliurament i no modificar factura, saldo ni cobrament per generar un certificat.

**Incidència separada de privacitat:** l'arbre públic del repositori inclou un volum notable d'arxius de certificat generat amb identificador personal al nom. No s'han obert arxius de tercers ni s'han reproduït identitats; [registre d'incidència documental](00-captures-auditoria-alumnes-consulta-modifica-2026-09-22.md#8-incidència-de-protecció-de-dades-detectada-al-repositori-sense-reproduir-cap-document). Retirar dades personals de la vista actual no neteja l'historial de Git ni les còpies publicades: cal revisar-ho pel canal intern corresponent. Aquesta incidència no prova que el servei de certificació de producció tingui el mateix contingut o les mateixes condicions d'accés que el repositori.

**Proves pendents:** UC124-C1 cursant vs superat; C2 certificat ja existent; C3 certificat amb grup/pagador diferent; C4 baixa i canvi de curs; C5 accés al `idInsc` d'una altra persona; C6 generació/descàrrega i neteja després d'error; C7 PDF sense identificadors al nom públic. Cap prova executada ni decisió de bloqueig acadèmic nova en aquest lot.
