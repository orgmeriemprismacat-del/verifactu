...
## 6. Pantalla independent «Consulta / Anul·la factura» — fonts actual/final

[Auditoria amb vuit accions i diagrames d'activitat ACTUAL/FINAL](02-auditoria-pantalla-alumnes-factura-2026-09-25.md) i [fitxa funcional UC-007, apartat 23](../06-fitxes-funcionals/uc-007.md#23-pantalla-independent-consulta--anulla-factura--auditoria-daccions). La pantalla `alumnes-factura.php` obre consulta i previsualització per ID de factura; el modal construït per [`modalPrevisualitzaFactura_Factures()` L15199–15221](../../codi-drive/intranet-actual/Intranet.php#L15199-L15221) crida `generaFactura(factRel,false)` per mostrar el document, sense acreditar una emissió fiscal nova. La descàrrega al [JS L658–707](../../codi-drive/intranet-actual/js/alumnes-factura.js#L658-L707) fa una segona petició al **mateix endpoint de descàrrega**, tot i que el text de gestió d'errors parla d'eliminar fitxers; no atribuir a aquesta petició una neteja de temporals que el JS no invoca. Les accions d'editar i «Anul·lar» estan inventariades per traçabilitat de pàgina, però **queden fora del contracte exclusivament consultiu d'UC-007**: [UC-005](uc-005-rectificar-factura.md) i [UC-006](uc-006-devolucio-saldo-compensacio.md). No hi ha captures de la pàgina ni proves executades; no tractar els vuit fluxos com a document fiscal lliurat o com a seguretat validada.
