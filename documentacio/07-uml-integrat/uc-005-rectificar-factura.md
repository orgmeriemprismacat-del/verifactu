...
## 7. Ruta de la intranet llegada «Consulta / Anul·la factura» i efecte real

[Auditoria de pantalla, accions AF-04/05/06 i diagrames AF-EDIT/AF-ANUL](02-auditoria-pantalla-alumnes-factura-2026-09-25.md). La pàgina `alumnes-factura.php` exposa edició directa de dades fiscals per [`guardarDadesFactura_Factures()` L14811–14830](../../codi-drive/intranet-actual/Intranet.php#L14811-L14830) que executa `updDadesFact` sobre la taula llegat `factures`. L'acció «Anul·lar» invoca [`anularFactura()` L14931–15193](../../codi-drive/intranet-actual/Intranet.php#L14931-L15193): a partir de l'import rebut per GET, calcula número tipus R (`últim ordre+1`), crea una fila de `factures` amb import negatiu i modifica resums econòmics i observacions de registres antics. **La presència d'una fila negativa no acredita per si sola** creació/registre d'una rectificativa fiscal a l'API SIF, versió de document, hash chain ni enviament AEAT. El nou flux ha de classificar la necessitat real de correcció abans d'emetre document vinculat a l'original per servei fiscal central, registrar idempotència i mantenir història immutable; no assumir que la paraula «anul·lar» estableix la classe fiscal aplicable. Reconciliar import/titular i classificació per grup amb UC-006/105; no confondre l'UPDATE `PAGAMENT` amb devolució bancària real. **Proves AF-T04–06 no executades:** doble ordre/concurrència, error parcial, grup i canvis sobre factura ja emesa.
