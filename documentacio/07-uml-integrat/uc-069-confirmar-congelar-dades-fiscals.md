...
| Evidència d'acceptació | **DISSENY:** `UUID_OPERATION`, versió/hash del snapshot, qui confirma i amb quin rol, moment, `REQUEST_ID/CORRELATION_ID` i nova acceptació si un camp material canvia. La migració `commercial_operation` contempla snapshots fiscal/comercial, però no s'ha acreditat el writer/confirmador complet. |
