...
- `go-no-go-preproduction.php` agrega una bateria bloquejant de preproduccio: entorn no productiu, preflight base, runner de proves, migracions, connexio SIF, connexio legacy, clau Redsys, taules fiscals minimes i circuits preparats de curs, pagament manual, fraccions manuals, rectificatives manuals, factura manual, migracio historica, packs i regals.
...
- [x] Afegir `ManualInstallmentPaymentPayloadBuilder` per construir fraccions manuals contra una factura SIF existent amb `movement_type = CHARGE`, `method = MANUAL`, `source_channel = INTRANET`, assignacio `INSTALLMENT_PAYMENT` i idempotencia `MANUAL|FRACCIO|ID_INSC:{ID_INSC}|DATA:{DATA}|IMPORT:{IMPORT}|USUARI:{USUARI}`.
- [x] Afegir `ManualInstallmentPaymentService` per localitzar factura per `UUID_FACTURA` o `NUM_VISIBLE` i registrar cada fraccio via `PaymentService::registerPayment()`, sense crear factura nova, sense registre fiscal nou i sense sync legacy.
- [x] Afegir `sif/scripts/preview-manual-installment.php` i `sif/scripts/process-manual-installment.php`: preview dry-run del payload de fraccio i processador CLI no productiu per fraccions manuals contra factura existent.
- [x] Afegir proves `ManualInstallmentPaymentPayloadBuilderTest`, `ManualInstallmentPaymentServiceTest`, `ManualInstallmentPaymentPreviewScriptTest` i `ManualInstallmentPaymentPreproductionScriptTest` per validar dues fraccions successives, reintent idempotent, transicio `PARTIAL`/`PAID`, absencia de duplicat fiscal i contracte dels scripts.
- [x] Afegir `ManualRectificationPayloadBuilder` per construir una factura rectificativa manual `series = R`, `type = R1`, receptor copiat de la factura original, linia/totals amb import positiu o negatiu, relacio `RECTIFIES` i idempotencia per factura original, mode, motiu i import.
- [x] Afegir `RectificationRepository` i `ManualRectificationService` per emetre la rectificativa amb `InvoiceService::issueInvoice()`, inserir `factura_rectificacio`, marcar l'original com `RECTIFIED` i retornar la relacio amb `UUID_FACTURA`/`NUM_VISIBLE` original.
- [x] Afegir `sif/scripts/preview-manual-rectification.php` i `sif/scripts/process-manual-rectification.php`: preview dry-run del payload `R` i processador CLI no productiu sense pagaments ni sync legacy.
- [x] Afegir proves `ManualRectificationPayloadBuilderTest`, `ManualRectificationServiceTest`, `ManualRectificationPreviewScriptTest` i `ManualRectificationPreproductionScriptTest` per validar serie `R`, vincle `factura_rectificacio`, estat `RECTIFIED`, reintent idempotent, absencia de pagaments i contracte dels scripts.
- [x] Afegir `ManualRefundPayloadBuilder` per construir el payload `registerPayment()` de devolucio economica contra una factura SIF existent: `movement_type = REFUND`, `source_channel = INTRANET`, assignacio `INVOICE_REFUND`, import positiu i idempotencia `REFUND|REF:{REFERENCIA}` o fallback per factura/data/import/banc.
- [x] Afegir `ManualRefundService` per localitzar factura per `UUID_FACTURA` o `NUM_VISIBLE` i registrar la devolucio via `PaymentService::registerPayment()`, sense crear factura, sense nou registre fiscal, sense hash chain i sense sync legacy.
- [x] Afegir `sif/scripts/preview-manual-refund.php` i `sif/scripts/process-manual-refund.php`: preview dry-run del payload `REFUND` i processador CLI no productiu per devolucions manuals contra factura existent.
- [x] Afegir proves `ManualRefundPayloadBuilderTest`, `ManualRefundServiceTest`, `ManualRefundPreviewScriptTest` i `ManualRefundPreproductionScriptTest` per validar idempotencia, estats `PARTIALLY_REFUNDED`/`REFUNDED`, absencia de nous registres fiscals i contracte dels scripts.
- [x] Afegir `ClaimPaymentPayloadBuilder` i `ClaimPaymentService` per registrar cobraments de reclamacio/morositat contra una factura SIF existent amb `PaymentService::registerPayment()`, assignacio `CLAIM_PAYMENT`, idempotencia `CLAIM|REF:{REFERENCIA}` o fallback per factura/data/import/usuari, sense emetre factura ni crear registre fiscal nou.
- [x] Afegir `sif/scripts/preflight-claim-payment.php`, `preview-claim-payment.php` i `process-claim-payment.php`: preflight nomes SIF, preview dry-run del payload `CLAIM_PAYMENT` i processador CLI no productiu per factura existent.
- [x] Afegir proves `ClaimPaymentPayloadBuilderTest`, `ClaimPaymentServiceTest`, `ClaimPaymentPreviewScriptTest` i `ClaimPaymentPreproductionScriptTest` per validar idempotencia, absencia de nous registres fiscals, assignacio `CLAIM_PAYMENT` i contracte dels scripts.
- [x] Afegir `ManualInvoicePayloadBuilder` per normalitzar factures manuals d'intranet: força `source_channel = INTRANET`, `source_type = MANUAL`, `series = A`, `type = F1`, exigeix usuari intern, deriva idempotencia per referencia o per usuari/data/hash del payload, i admet bloc `payment` inicial quan la factura manual neix cobrada.
- [x] Afegir `ManualInvoiceService` per cridar `InvoiceService::issueInvoice()` amb el payload manual normalitzat, sense escriure a legacy ni cridar `registerPayment()` directament.
- [x] Afegir `sif/scripts/preview-manual-invoice.php` i `sif/scripts/process-manual-invoice.php`: preview dry-run del payload manual i processador CLI no productiu amb dependencies de pagament dins `InvoiceService` per permetre `issueInvoice(payment)`.
- [x] Afegir proves `ManualInvoicePayloadBuilderTest`, `ManualInvoiceServiceTest`, `ManualInvoicePreviewScriptTest` i `ManualInvoicePreproductionScriptTest` per validar emissio pendent, emissio amb cobrament inicial, reintent idempotent, usuari intern obligatori, absencia de legacy sync i contracte dels scripts.
- [x] Afegir `HistoricalInvoicePayloadBuilder`, `HistoricalInvoiceMigrationRepository` i `HistoricalInvoiceMigrationService` per migrar factures historiques com a `NO_VERIFACTU`: conserva `NUM_VISIBLE`, serie/any/numero, billing, linies, relacions `HISTORIC_LINK` i document historic opcional amb hash, sense crear `factura_registres`, `fiscal_queue`, `fiscal_sequence` ni hash chain.
- [x] Afegir `sif/scripts/preview-historical-invoice-migration.php` i `process-historical-invoice-migration.php`: preview dry-run i processador CLI no productiu per importar una factura historica des d'un JSON de payload revisat.
- [x] Afegir proves `HistoricalInvoicePayloadBuilderTest`, `HistoricalInvoiceMigrationServiceTest`, `HistoricalInvoiceMigrationPreviewScriptTest` i `HistoricalInvoiceMigrationPreproductionScriptTest` per validar `NO_VERIFACTU`, reintent idempotent, absencia de registre fiscal/cua AEAT i contracte dels scripts.
- [x] Afegir `CreditBalancePayloadBuilder`, `CreditBalanceRepository` i `CreditBalanceService` per crear saldo a `credit_balance` sense factura nova ni efectes fiscals, i per aplicar-lo posteriorment a una factura SIF existent com a `payment_transaction.TIPUS_MOVIMENT = COMPENSATION` i `METODE = COMPENSACIO`.
- [x] Fer que l'aplicacio de saldo sigui transaccional i idempotent: bloqueja el `credit_balance`, localitza factura per `UUID_FACTURA` o `NUM_VISIBLE`, rebutja imports superiors al saldo disponible o al pendent de la factura, crea `payment_allocation` amb `CREDIT_COMPENSATION` i resta `IMPORT_DISPONIBLE` nomes si no existia ja el pagament.
- [x] Afegir `sif/scripts/preview-credit-balance.php`, `process-credit-balance.php`, `preview-credit-compensation.php` i `process-credit-compensation.php`: els preview no escriuen, la creacio de saldo no registra pagaments i la compensacio no emet factura ni sincronitza legacy.
- [x] Afegir proves `CreditBalancePayloadBuilderTest`, `CreditBalanceServiceTest`, `CreditBalancePreviewScriptTest`, `CreditBalancePreproductionScriptTest`, `CreditCompensationPreviewScriptTest` i `CreditCompensationPreproductionScriptTest` per validar creacio de saldo, compensacio parcial/total, idempotencia, estats `ACTIVE`/`USED` i contracte dels scripts.
...
- [ ] Executar `SIF-PAY-001` amb PHP/MySQL de test, `preflight-manual-payment.php`, `preview-manual-payment.php`, `process-manual-payment.php`, `preflight-claim-payment.php`, `preview-claim-payment.php`, `process-claim-payment.php`, `preview-manual-installment.php`, `process-manual-installment.php`, `preview-manual-rectification.php`, `process-manual-rectification.php`, `preview-manual-refund.php`, `process-manual-refund.php`, `preview-manual-invoice.php`, `process-manual-invoice.php`, `preview-historical-invoice-migration.php`, `process-historical-invoice-migration.php`, `preview-credit-balance.php`, `process-credit-balance.php`, `preview-credit-compensation.php`, `process-credit-compensation.php` i pantalla real de `Passar pagaments` abans d'activar el flux operatiu.
...
- [x] Afegir `RedsysGroupInvoiceService` i els scripts `preflight-redsys-group.php`, `preview-redsys-group.php` i `process-redsys-group.php`: processament manual de preproduccio d'un `DS_ORDER` ja `VALIDATED`, payload `GRUP`, cobrament inicial Redsys i `--sync-legacy` opcional nomes despres d'exit SIF.
- [x] Afegir `ManualGroupInvoicePayloadBuilder`, `ManualGroupInvoiceService` i els scripts `preflight-manual-group.php`, `preview-manual-group.php` i `process-manual-group.php`: transferencia validada a `Passar pagaments`, idempotencia `TRANSFERENCIA|GRUP|IDPAG:{IDPAG}|...`, import manual igual al total fiscal del grup i sync legacy opcional post-SIF.
- [x] Afegir proves `RedsysGroupInvoiceServiceTest`, `RedsysGroupPreviewScriptTest`, `RedsysGroupPreproductionScriptTest`, `ManualGroupInvoicePayloadBuilderTest`, `ManualGroupInvoiceServiceTest`, `ManualGroupPreviewScriptTest` i `ManualGroupPreproductionScriptTest`.
- [ ] Validar SQL final de `descomptes_grup` i incorporar-lo al snapshot de grup abans d'activacio real del circuit. Fins aquest punt, el builder respecta imports/descomptes explicits del snapshot i el repositori nomes carrega imports ja presents a `inscripcions`.
...
- [x] Afegir `RedsysUsocInvoiceService` per processar manualment en preproduccio la factura `USOC_ALUMNE` des d'una notificacio Redsys `VALIDATED`, exigint `--usoc-amount=AMOUNT` explicit i retornant `entity_invoice_pending` amb `student_invoice_uuid` per a la futura factura `USOC_ENTITAT`.
- [x] Afegir `sif/scripts/preflight-redsys-usoc.php`, `sif/scripts/preview-redsys-usoc.php` i `sif/scripts/process-redsys-usoc.php`: preflight de nomes lectura, preview dry-run del payload `USOC_ALUMNE` i processador CLI no productiu. En aquest tall no hi ha `--sync-legacy` i no s'emet `USOC_ENTITAT`.
- [x] Afegir proves `RedsysUsocInvoiceServiceTest`, `RedsysUsocPreviewScriptTest`, `RedsysUsocPreproductionScriptTest` i `RedsysUsocPreflightScriptTest` per validar idempotencia de la factura d'alumne, rebuig de notificacio no validada, exigencia d'import USOC explicit i contracte dels scripts.
- [x] Afegir `UsocEntityInvoiceService` per processar `USOC_ENTITAT` en un circuit separat amb entrada explicita: `idpag`, `student_amount`, `amount`, `student_invoice_uuid` i `billing`. El servei carrega snapshot USOC validat i crida `issueInvoice()` sense bloc `payment`.
- [x] Afegir `sif/scripts/preflight-usoc-entity.php`, `sif/scripts/preview-usoc-entity.php` i `sif/scripts/process-usoc-entity.php`: preflight de nomes lectura, preview dry-run amb `--payload-file=payload.json` i processador CLI no productiu. En aquest tall no hi ha cobrament inicial ni sync legacy.
- [x] Afegir proves `UsocEntityInvoiceServiceTest`, `UsocEntityPreviewScriptTest`, `UsocEntityPreproductionScriptTest` i `UsocEntityPreflightScriptTest` per validar factura pendent de cobrament, billing explicit abans de consultar legacy, absencia de `payment_transaction` i contracte dels scripts.
- [ ] Confirmar dades fiscals completes de l'entitat USOC abans d'activar cap factura `USOC_ENTITAT` real.
- [ ] Executar el circuit Redsys USOC d'alumne amb PHP/MySQL de test: `preflight-redsys-usoc.php`, `preview-redsys-usoc.php DS_ORDER --usoc-amount=AMOUNT` i `process-redsys-usoc.php DS_ORDER --usoc-amount=AMOUNT`, amb notificacio Redsys validada, reintent idempotent, descompte USOC congelat i `entity_invoice_pending` revisat.
- [ ] Executar el circuit USOC entitat amb PHP/MySQL de test: `preflight-usoc-entity.php`, `preview-usoc-entity.php --payload-file=payload.json` i `process-usoc-entity.php --payload-file=payload.json`, amb billing USOC real confirmat, factura pendent de cobrament, reintent idempotent i absencia de pagament inicial.
...
- [x] Afegir suport a `RedsysCourseInvoiceService` per rebre un snapshot `discount` explicit i injectar-lo abans de construir el payload fiscal de curs normal.
- [x] Afegir `--discount-file=discount.json` a `preview-redsys-course.php` i `process-redsys-course.php` per provar en preproduccio un curs Redsys amb codi promocional ja validat pel canal, sense consultar encara `promocions`/`descomptes` des del SIF.
- [x] Afegir proves `RedsysCourseInvoiceServiceTest`, `RedsysCoursePreviewScriptTest` i `RedsysCoursePreproductionScriptTest` per validar persistencia de `DESC_*`, preview dry-run i processador CLI amb snapshot de descompte explicit.
- [x] Afegir suport equivalent a `ManualCourseInvoiceService` per rebre un snapshot `discount` explicit quan el curs normal es cobra manualment a `Passar pagaments`.
- [x] Afegir `--discount-file=discount.json` a `preview-manual-course.php` i `process-manual-course.php` per provar transferencies manuals amb codi promocional ja validat pel canal, sense revalidar `promocions` dins el SIF.
- [x] Afegir proves `ManualCourseInvoiceServiceTest`, `ManualCoursePreviewScriptTest` i `ManualCoursePreproductionScriptTest` per validar persistencia de `DESC_*`, cobrament `INTRANET` i contracte dels scripts manuals amb snapshot de descompte explicit.
- [x] Afegir `DiscountSnapshotFileReader` com a lector compartit del fitxer `discount.json` per als circuits Redsys/manuals de curs, amb prova unitària per ruta buida, JSON valid, fitxer absent, JSON invalid i JSON no objecte.
...
- [ ] Executar el circuit de pagaments fraccionats manuals amb PHP/MySQL de test: `preview-manual-installment.php` i `process-manual-installment.php`, amb dues fraccions sobre la mateixa factura, reintent idempotent, estat `PARTIAL` despres de la primera fraccio i `PAID` quan la suma assignada arriba al total.
- [ ] Executar el circuit de rectificatives manuals amb PHP/MySQL de test: `preview-manual-rectification.php` i `process-manual-rectification.php`, amb import negatiu i positiu, mode `DIFERENCIES`/`SUBSTITUCIO`, reintent idempotent, vincle `factura_rectificacio`, estat original `RECTIFIED` i absencia de `payment_transaction`.
- [ ] Executar el circuit de factura manual amb PHP/MySQL de test: `preview-manual-invoice.php --payload-file=payload.json` i `process-manual-invoice.php --payload-file=payload.json`, amb factura pendent, factura amb `payment` inicial, reintent idempotent, usuari intern obligatori, registre fiscal/AEAT creat i absencia de legacy sync.
- [ ] Executar el circuit de migracio historica amb PHP/MySQL de test: `preview-historical-invoice-migration.php --payload-file=payload.json` i `process-historical-invoice-migration.php --payload-file=payload.json`, amb factura historica `NO_VERIFACTU`, document antic amb hash si existeix, relacio `HISTORIC_LINK`, reintent idempotent i absencia total de `factura_registres`, `fiscal_queue`, `fiscal_sequence` i canvi de hash chain.
- [ ] Executar el circuit de saldo/compensacio amb PHP/MySQL de test: `preview-credit-balance.php`, `process-credit-balance.php`, `preview-credit-compensation.php` i `process-credit-compensation.php`, amb prova de saldo parcial, saldo consumit, reintent idempotent, bloqueig d'import superior al disponible i absencia de factura/registre fiscal nou.
...
- [ ] Executar els circuits de grup amb PHP/MySQL de test i dades legacy de preproduccio: `preflight-redsys-group.php`, `preview-redsys-group.php DS_ORDER`, `process-redsys-group.php DS_ORDER`, `preflight-manual-group.php`, `preview-manual-group.php IDPAG AMOUNT MOVEMENT_DATE` i `process-manual-group.php IDPAG AMOUNT MOVEMENT_DATE`, amb reintent idempotent, `VISIBLE_ALUMNE = 0` i revisio de sync legacy opcional.
...
- [ ] Executar curs normal amb codi promocional i promocio temporal amb PHP/MySQL de test: `preview-redsys-course.php DS_ORDER --discount-file=discount.json`, `process-redsys-course.php DS_ORDER --discount-file=discount.json`, `preview-manual-course.php IDPAG AMOUNT MOVEMENT_DATE --discount-file=discount.json`, `process-manual-course.php IDPAG AMOUNT MOVEMENT_DATE --discount-file=discount.json`, callback duplicat/reintent manual i verificacio dels camps `DESC_*` immutables.
