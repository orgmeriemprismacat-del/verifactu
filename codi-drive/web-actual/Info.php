...
class Info extends Curs {
   protected $presentacio; /**< string Presentació del curs */
   protected $autoria; /**< string Autoria del curs */
   protected $destinataris; /**< string Destinataris del curs */
   protected $objectius; /**< string Objectius del curs */
   protected $programa; /**< string Programa del curs */
   protected $valoracions; /**< string Valoracions del curs */
   protected $article; /**< Article Article relacionat amb el curs */
   protected $cursos_relacionats; /**< LlistatCursos Els cursos relacionats amb el curs */
   protected $packs_relacionats; /**< LlistatCursos Els packs relacionats amb el curs */
   protected $docents; /**< Array Els docents que porten el curs */
   ...
   */
   public function __construct($url ,$dispositiu) {
      ...
      VALORACIONS, ID_ARTICLE, TIPUS_CURS, SINCRON FROM informacio WHERE ID_AMIGABLE=? AND ESTAT=1";
      ...
      $stmInfo->bind_result($idInfo, $codi, $titol, $desc, $idVideo, $idImgLarge, $idImgSmall, $pres, $autoria, $dest, $obj, $prog, $val, $idArticle, $tipusCurs, $sincron);
      $stmInfo->fetch();
      $connexio->closeStmt();
      ...
      // echo $consultaHoresPreu
      $stmtHoresPreu->execute();
      $stmtHoresPreu->store_result();
      // echo $consultaHoresPreu."<br />";
      ...
      if ($this->estat==1) {
        ...
        if ($cdd!=null and $cdd!='')
         ...
         $idPerfils = null;
         // echo $consultaPerfil."<br>";
         // echo $codi."<br>".$hores."<br>".$cursEscolar."<br>".$gtaf."<br>";
         ...
         $consultaNivellTemaRel = "SELECT f.ID_NIVELL, f.ID_REL, f.ID_PACK_REL FROM filtres AS f WHERE ID_INFO LIKE ? AND f.DATAI<=CURRENT_TIME AND (f.DATAF IS NULL OR CURRENT_TIME<=f.DATAF)";
         ...
         $stmtNivTemaRel->bind_result($idNivell, $idRel, $idPackRel);
         ...
         $cnsEd = "SELECT DATAI, DATAF, CURS_ESCOLAR, FISS, GTAF, DATA_RESOL, ANY, MES, c.CURS,
            ...
            AND ORDRE_TUTOR is not NULL)) GROUP BY MES ORDER BY ANY, MES";
   ...
   */
   public function mostrarImgLargeInfo() {
      if ( (!$this->__isMobile()) || ($this->__isMobile() && $this->obtenirVideo()== null) ) {
         $banderola = '';
         if ( $this->cursEsMixt() )
            $banderola = "<div class='noComencat'>
            <div class='border'></div>
            <div class='nom'>MIXT EN LÍNIA</div>
            </div>";
        $banderola .= $this->__etiquetaNomesEstiu();

         if (!$this->__isMobile())
            $mostrar = "<div class='info-banner position-relative mb-4' id='img_llarga'>".$banderola."<picture>";
         else
            $mostrar = "<div class='info-banner mb-0' id='img_llarga'>".$this->__etiquetaNomesEstiu()."<picture>";
         ...
         $linkImgWeb=substr($linkImg, 0, -4).".webp?ver=".$versioImg;
         ...
         $mostrar = "<div class='info-banner mb-4 position-relative'>".$this->__etiquetaNomesEstiu()."<picture>
   ...
   */
   protected function __obtenirPresentacio() {
   ...
   */
   protected function __obtenirAutoria() {
      return $this->autoria;
   }
   ...
   */
   protected function __obtenirDestinataris() {
   ...
   */
   protected function __obtenirDocents() {
   ...
   */
   protected function __obtenirDocent($posicio) {
   ...
   */
   protected function __mostrarApartatVista($vista, $idTab, $contingutOrd, $obert, $idAccord, $nomApartat, $contingutMov) {
      $show = "";
      $collapsed = " collapsed";
      $ariaExpanded = "false";
   ...
   */
   protected function __mostrarDescripcio( $vista ) {
      ...
      $cnsRept = "SELECT ID_URL, TITOL FROM reptes WHERE CURS_ORIG=? AND ESTAT = 1";
      $stmtRept = $connexio->prepare($cnsRept);
      $stmtRept->bind_param("s", $codiCurs);
      $codiCurs=$this->obtenirCodi()->obtenirText();
      $stmtRept->execute();
      $stmtRept->store_result();
      $stmtRept->bind_result($idUrlRept, $titolRept);
      if ($stmtRept->num_rows() > 0)  {
        $stmtRept->fetch();
        $urltast = new Url($idUrlRept);
        $linkTastet = $urltast->obtenirLink();
        $textTastet = new Text($titolRept);
        $txtTitolTastet = $textTastet->obtenirTextHTML();

        $cntTastet = "<p class='mb-3'>Si no tens clar si aquest és el curs que estàs buscant o no,
        t'animem a provar-ne una versió reduïda i gratuïta fent el tastet
        <a href='".$linkTastet."' class='font-weight-bold'>".$txtTitolTastet."</a>.</p>";

        $contingut .= "<h2>Vols fer un tastet del curs?</h2>".$cntTastet;
      }
      $connexio->closeStmt();
      $connexio->desconectarBD();
   ...
   */
   protected function __mostrarPrograma( $vista ) {
      $contingutOrd = $contingutMov = "<h2>Programa</h2>".$this->obtenirPrograma()->obtenirText();
      $nomApartat = "<i class='fa-solid fa-cube mr-2'></i><span class='flex-1-0-auto'>Programa</span>";
   ...
   */
   protected function __mostrarDocents( $vista ) {
   ...
   */
   private function __mostrarPerfilEdicions($perfilEd, $curs_escolar) {
      ...
      if ($numPerfils > 0) {
         ...
         $mostrar.="title='Acreditació de perfils professionals'>";
   ...
   * @return Mostra la frase de "Les edicions de l'any YYYY/YYYY estan acollides al perfil XXXX. Per més informació...
   */
   private function __mostrarPerfilPendentsEdicions($perfilEd, $curs_escolar) {
      $cnt = 0;
      $numPerfils = count($perfilEd);
      ...
      if ($numPerfils > 0) {
         $linkPerfil = "https://www.prisma.cat/perfils-professionals/";
         $mostrar.="<p>Les edicions de l'<strong>any escolar ".$curs_escolar."</strong> d'aquest curs estan ";
         $mostrar.="pendents d'acreditar el perfil professional ".$textsPerfil.". ";
         $mostrar.="Podeu consultar-ne tota la informació a la nostra pàgina ";
         $mostrar.="d'<strong><a href='".$linkPerfil."' target='_self' ";
   ...
   */
   private function __mostrarEdicions( $vista ) {
      ...
      if ( $this->cursEsMixt() )
         $contEd.="<div id='accord_ed' class='d-flex flex-column mb-3'>
         <div class='panel-group' id='accordion' role='tablist' aria-multiselectable='true'>";
      else
         $contEd.="<ul class='llistes'>";

      $numero_edicions = count($this->__obtenirEdicions()); //echo $numero_edicions;
      $cnt = 0; $mesPendent='';
      while ($cnt < $numero_edicions) {
         $edicioActual=$this->__obtenirEdicio($cnt); //echo $edicioActual->obtenirMes()->obtenirText().".";
         ...
         if ( $this->cursEsMixt() ) {
            ...
            while ( $cntPestOberta < $numero_edicions && $this->__obtenirEdicio($cntPestOberta)->obtenirEstat()->obtenirText() == 'T' ) {
              $cntPestOberta++;
            }

            if ( $cnt == $cntPestOberta ) {
               $ariaExpanded = 'true';
               $collapseShow = 'show';
            }
            $contEd .= "<div class='panel panel-default'>
               <div class='panel-heading collapsed' role='tab' id='heading-".$cnt."' data-toggle='collapse'
               data-target='#collapse-".$cnt."' href='#collapse-".$cnt."' aria-expanded='".$ariaExpanded."' aria-controls='collapse-".$cnt."'>
                  <p class='panel-title d-flex flex-wrap align-items-center font-weight-bold'>
                     <i class='fat fa-calendar mr-2'></i>
                     <span class='flex-1-0-auto'>".$edicioActual->mostrarEdicioInfoMixt($nomCurs)."</span>
                     <i class='fas fa-plus ml-2'></i>
                  </p>
               </div>
               <div id='collapse-".$cnt."' class='panel-collapse active collapse ".$collapseShow."' role='tabpanel'
               aria-labelledby='heading-".$cnt."' aria-expanded='".$ariaExpanded."' style=''>
                  <div class='panel-body descripcio text-justify pb-3'>
                     ".$edicioActual->mostrarSessionsSincronesEdicioInfo()."
                  </div>
               </div>
            </div>";
         }
      ...
      if ($mesPendent!='')
         ...
         del curs al Departament d'Educació. Les edicions anteriors tenen data de resoluci&oacute;
      ...
      if (!$cursReconegut) { //no hi ha cap curs reconegut
         ...
         $contEd.="d’antelació.</p><p>Curs reconegut com a activitat de <strong><a rel='noopener' target='_blank' ";
      ...
      if ($cursEsc2=='') {
         ...
         if ($equals) {
           ...
           // echo "no equals".$cursEsc1." ".$cursEsc2."<br>";
             ...
             //comprovem si algun dels possibles perfils pendents que pot tenir el curs, existeixen com a perfil en el curs a la taula perfils_list
             $idPerfilsPendentsComprovar = explode('|',$idPerfilPendent);
             // print_r($idPerfilsPendentsComprovar);
             if ( $idPerfilPendent == null or $idPerfilPendent == '' )
                $numPerfilsPendents = 0;
                ...
                $numPerfilsPendents = count($idPerfilsPendentsComprovar);
             if ( $perfilEd2 == null or $perfilEd2 == '' )
                 $numPerfils = 0;
             else
                 $numPerfils = count($perfilEd2);
             ...
             for ($i=0; $i<$numPerfilsPendents; $i++) {
                ...
                while ( $j < $numPerfils && !$trobat ) {
                   // echo $codi." PERFIL: ".$idPerfils[$i]."<br />";
                   if ( $idPerfilsPendentsComprovar[$i] == $perfilEd2[$j] )
                      $trobat = true;
                   else
                      $j++;
                }
                ...
                if ( !$trobat ) {
                   // echo $codi." ".$idPerfilsPendentsComprovar[$i]." ";
                   $perfilsPendentsEd2[$cntPerfilPendEd2] = new Perfil($idPerfilsPendentsComprovar[$i]);
                   $cntPerfilPendEd2++;
                }
             }

             //si existeix algun perfil pendent pel curs escolar 2, mostrem el misatge
             if ( $cntPerfilPendEd2 > 0 ) {
               $contEd.=$this->__mostrarPerfilPendentsEdicions($perfilsPendentsEd2, $cursEsc2);
             }
         }
      }
      $connexio->desconectarBD();
      ...
      if ($firstEd->obtenirCodiFiss()!=null && $ultimaEdicio->obtenirCodiFiss()!=null) {
         ...
         $contEd.="de Drets Socials i Inclusió\">formació d’interès en serveis socials</a></strong> ";
         $contEd.="pel Departament de Drets Socials i Inclusió de la Generalitat de Catalunya.</p>";
      }
      else if (
        ...
        if ($stmt->num_rows() > 0) {
         ...
         pel Departament de Drets Socials i Inclusió de la Generalitat de Catalunya.";
         ...
         if ( $pendentRecFISS ) {
           ...
           pel Departament de Drets Socials i Inclusió\">Activitats de
   ...
   */
   protected function __mostrarValoracions( $vista ) {
   ...
   */
   public function mostrarMesInformacio() {
      ...
      if ( $this->cursEsMixt() ) $urlMet = "https://www.prisma.cat/metodologia-mixt-linia";
    ...
    if ( $this->tipus->obtenirText() != "S" ) {
      ...
      $cnsRept = "SELECT ID_URL FROM reptes WHERE CURS_ORIG=? AND ESTAT = 1";
      $stmtRept = $connexio->prepare($cnsRept);
      $stmtRept->bind_param("s", $codiCurs);
      $codiCurs=$this->obtenirCodi()->obtenirText();
      $stmtRept->execute();
      $stmtRept->store_result();
      $stmtRept->bind_result($idUrlRept);
      if ($stmtRept->num_rows() > 0)  {
        $stmtRept->fetch();
        $urltast = new Url($idUrlRept);
        $linkTastet = $urltast->obtenirLink();
        //
        $mostrar.="<button role='button' class='regala border-0 border-radius-2 text-center ";
        $mostrar.="position-relative negreta500' onclick=\"location.href='https://www.prisma.cat".$linkTastet."'\" ";
        $mostrar.="title=\"Vols fer un tastet del curs ".$this->obtenirTitol()->obtenirText()."? \">";
        $mostrar.="<svg style='width: 18px;margin-right: 3px;' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'><path fill=#4668A7 d='M257.5 27.6c-.8-5.4-4.9-9.8-10.3-10.6v0c-22.1-3.1-44.6 .9-64.4 11.4l-74 39.5C89.1 78.4 73.2 94.9 63.4 115L26.7 190.6c-9.8 20.1-13 42.9-9.1 64.9l14.5 82.8c3.9 22.1 14.6 42.3 30.7 57.9l60.3 58.4c16.1 15.6 36.6 25.6 58.7 28.7l83 11.7c22.1 3.1 44.6-.9 64.4-11.4l74-39.5c19.7-10.5 35.6-27 45.4-47.2l36.7-75.5c9.8-20.1 13-42.9 9.1-64.9v0c-.9-5.3-5.3-9.3-10.6-10.1c-51.5-8.2-92.8-47.1-104.5-97.4c-1.8-7.6-8-13.4-15.7-14.6c-54.6-8.7-97.7-52-106.2-106.8zM208 144a32 32 0 1 1 0 64 32 32 0 1 1 0-64zM144 336a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zm224-64a32 32 0 1 1 0 64 32 32 0 1 1 0-64z'/></svg>Tastet gratuït del curs</button>";
      }
    }
    else {
