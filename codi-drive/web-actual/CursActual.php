...
class CursActual {
   ...
   */
   public function __construct($codi, $curs_escolar, $dispositiu) {
      ...
      $consultaCurs = "SELECT TITOL, SHORT_DESC, ID_VIDEO, ID_IMG_LARGE, ID_IMG_PERFIL,
         ID_IMG_SMALL, ID_AMIGABLE FROM informacio WHERE CODI_CURS=? AND ESTAT=1";
      $stmtInfo=$connexio->prepare($consultaCurs);
      $stmtInfo->bind_param("s", $codi);
      $stmtInfo->execute();
      $stmtInfo->bind_result($titol, $shortDesc, $idVideo, $idImgLarge, $idImgPerf, $idImgSmall, $idAmig);
      ...
      if ($stmtInfo->num_rows() > 0) {
		...
		/*$any_actual = date(n);
		if ($any_actual <= 8)
			$curs_escolar_actual = $any_actual-1 ."/".$any_actual;
		else
			$curs_escolar_actual = $any_actual."/".$any_actual+1;		*/

         /* Es busca les hores i el id_preu de l'actual curs escolar de l'última edició realitzada del curs */
         $consultaHoresPreu = "SELECT HORES, GTAF, ID_PREU, CDD FROM curs WHERE
            CURS LIKE ? AND PUBLIC=1 AND CURS_ESCOLAR=?
            AND (CURS NOT LIKE '%JOR%') AND (CURS NOT LIKE '%0%')
            ORDER BY ANY, MES LIMIT 1";
         $stmtHoresPreu=$connexio->prepare($consultaHoresPreu);
         $stmtHoresPreu->bind_param("ss", $codi, $curs_escolar);
         $stmtHoresPreu->execute();
         $stmtHoresPreu->store_result();

         if ( $stmtHoresPreu->num_rows() > 0 ) {
            $stmtHoresPreu->bind_result($hores, $gtaf, $idPreu, $cdd);
            $stmtHoresPreu->fetch();
            $connexio->closeStmt();
            $this->estat = 1;
        }
        ...
        if ( $this->estat==1) {
            ...
            $stmtPerfil->bind_param("sdss", $codi, $hores, $curs_escolar, $gtaf);
            $stmtPerfil->execute();
            $stmtPerfil->bind_result($idPerfil);
            $cntPerfils=0;
            require_once 'Perfil.php';
            while ($stmtPerfil->fetch()) { //echo $codi."<br>"; echo $hores."<br>"; echo $curs_escolar."<br>"; echo $gtaf;
               $this->perfil[$cntPerfils] = new Perfil($idPerfil);
               $cntPerfils++;
            }
            $connexio->closeStmt();

            /* Es busca els nivells i les tematiques del curs */
           // $consultaNivellTema = "SELECT f.ID_NIVELL, f.ID_TEMA FROM informacio AS i INNER
            ...
            $stmtNivTema->bind_result($idNivell, $idTema);
            ...
            $cnt_nivells = 0;
            $consultaNivell = "SELECT NOM, ORDRE FROM nivells WHERE ID=?";
   ...
   */
   protected function obtenirVideo() {
      return $this->video;
   }
   ...
   */
   protected function obtenirTitol() {
      if ($this->titol==null)
   ...
   */
   protected function obtenirPerfil($posicio) {
   ...
   */
   protected function obtenirHores() {
      if ($this->hores==null)
   ...
   */
   public function mostrarCursRelacionat() {
      $mostrar = "<div class='col-3'>".$this->__mostrarCurs("div class='h3'")."</div>";
      return $mostrar;
   }
   ...
   */
   public function crearTastet(){
      ...
      if ($this->dispositiu=='ordinador') {
         ...
         $etiquetaDesc=$this->__etiquetaCursAmbDescompte();
         ...
         $mostrar .= "border-0 negreta'>".$etiquetaNou.$etiquetaDesc.$titol."</div>";
         $mostrar .= "<p class='modal-descripcio-curta'>".$shortDesc."</p></div>";
         $mostrar .= "<div class='col-md-4 d-flex flex-column border-left my-2'><a role='button' class='inscripcio border-0 ";
         $mostrar .= "border-radius-2 text-white position-relative text-center negreta500' ";
         $mostrar .= "title=\"Inscriu-te al curs ".$titol."\" href =\"".$urlInsc."\" target=\"_blank\">";
         $mostrar .= "<i class='fas fa-edit'></i> Inscripció</a><button role='button' ";
         $mostrar .= "class='regala border-radius-2 position-relative text-center negreta500' ";
         $mostrar .= "title=\"Regala el curs ".$titol."\" onclick=\"location.href='".$urlReg."'\">";
         $mostrar .= "<i class='fas fa-gift'></i> Regala aquest curs</button><ul>";
         ...
         $mostrar .= "<span class='negreta500'>".$preuDesc."-".$preuCar." €</span>";

         $cntPerfil=0;
         $trobat=false;
         while ($this->obtenirPerfil($cntPerfil)!=null && !$trobat) {
            $mostrar.="<li class='d-flex flex-row align-items-center'><i class='fas fa-tag'></i><span class='text'>Edicions amb perfil</span>";
            $mostrar.="<button class='mesinfo position-relative negreta500' ";
            $mostrar.="title='Totes les edicions que acrediten algun perfil del curs de PrisMa' ";
            $mostrar.="onclick=\"location.href='".$urlPerfil."'\"> + info </button></li>";
            $cntPerfil++;
            $trobat=true;
         }
   ...
   */
   public function mostrarCursTutor($dni_tutor, $midaPantalla) {
      ...
      $mostrar.="<div class='col-md-8 curs-tutoritat-tutor d-flex flex-column'>";
      $mostrar.="<div class='clear-both'>".$etiquetaNou.$etiquetaDesc."<a role='link' class='color-text' ";
      $mostrar.="href='".$linkCurs."' target='_self' class='color-text' ";
      $mostrar.="title=\"Informació del curs de PrisMa «".$titol."»\">";
      $mostrar.="<h3 class='media-heading'>".$titol."</h3></a>";
      $mostrar.="</div><div class='event-schedule'>";
      $mostrar.="<div class='float-left'><i class='far fa-clock'></i>";
      ...
      while ($cntNiv < count($this->nivells)) {
         ...
         if ($cntNiv!=0)
            $mostrar .= " | ";
         $mostrar .= "<span class='espai'>".$niv."</span>";
         $cntNiv++;
      }
      $mostrar .= "</div>";
      ...
      while ($this->obtenirPerfil($cntPerfil)!=null) {
         ...
         $mostrar.="<div role='link' title=\"Totes les edicions que han acreditat ";
         $mostrar.="el perfil «".$perfil->obtenirNomLlarg()->obtenirText()."»\" ";
         $mostrar.="class='perfil-curs ".$classPerfil." font-weight-bold float-left border-radius-2'";
         $mostrar.="onclick=\"location.href='https://www.prisma.cat/perfils-professionals'\">";
         $mostrar.=$perfil->obtenirNomCurt()->obtenirText()."</div>";
         $cntPerfil++;
      }
      ...
      if ($numDoc > 0) {
         ...
         $mostrar .= "<p class='descripcio-curta flex-1-0-auto'>".$descMesInfo."</p>";
         $mostrar .= "<p class='altres-tutoritzacio p-0'>També el ";
         if ($numDoc==1)
            $mostrar .= "pot ";
         else
            $mostrar .= "poden ";
      ...
      $mostrar.="</div><div class='col-md-4 p-0 d-flex flex-column'><div class='prisma-tutor-overlay'>";
      $mostrar.="<img role='img' src='".$linkImg."' alt=\"".$dscImg."\" ";
      $mostrar.="class='prisma-tutor-image prisma-event-image w-100'";

      if ($this->dispositiu=='ordinador') {
         $mostrar .= "onclick=\"mostrarTaset('".$codiCursMin."')\">";
         $mostrar .= "<div class='prisma-tutor-middle position-absolute'>";
         $mostrar .= "<button role='button' id='boto_veure_tastet_".$codiCursMin."' ";
         $mostrar .= "class='prisma-tutor-text border-0' aria-label=\"Mostra tastet ".$titol."\" ";
         $mostrar .= "type='button' title=\"Mostra un tastet del curs «".$titol."»\" ";
         $mostrar .= "data-html='true' onclick=\"mostrarTaset('".$codiCursMin."')\" ";
         $mostrar .= "data-placement='bottom' data-keyboard='true'>Mostra'n un tastet</button></div>";
      }
      else
   ...
   */
   public function mostrarCursPerfil() {
      $linkUrl=$this->obtenirUrl()->obtenirLink();
      ...
      $mostrar.="<div class='curs-perfil-container d-flex flex-column flex-md-row border-radius-2 mb-4'>
      <div class='col-12 col-md-8 curs-perfil pt-2 pr-2 pb-1 pl-3'>
      <div class='mb-2 mt-1'>".$etiquetaNou.$etiquetaDesc.$etiquetaCDD."
      <a role='link' target='_self' class='color-text' href='".$linkCurs."' ";
      $mostrar.="title=\"Informació del curs de PrisMa «".$titol."»\">";

      $mostrar.="<h4 class='font-weight-bold my-0'>".$titolHTML."</h4></a></div>";
      $mostrar.="<div class='event-schedule d-flex flex-wrap mb-2'>
      <div class='element mr-3'><i class='far fa-clock mr-2'></i><span>";
      ...
      $mostrar.="<a role='link' class='consulta-edicions font-weight-normal' ";
      ...
      $mostrar.="<div class='col-12 col-md-4 prisma-perfil-overlay position-relative p-0'>
      ".$this->__etiquetaNomesEstiu()."
      <img role='img' src='".$linkImg."' alt=\"".$dscImg."\" class='h-100 w-100 ".$classeImatgeGran."'";

      if ($this->dispositiu=='ordinador') {
         $mostrar.="onclick=\"mostrarTaset('".$codiCursMin."')\">";
         $mostrar.="<div class='prisma-perfil-middle position-absolute'>";
         $mostrar.="<button role='button' id='boto_veure_tastet_".$codiCursMin."' ";
         $mostrar.="class='negreta500 border-radius-2' aria-label=\"Mostra tastet ".$titol."\" ";
         $mostrar.="type='button' data-html='true' title=\"Tastet del curs ".$titol."\" ";
         $mostrar.="onclick=\"mostrarTaset('".$codiCursMin."')\" data-placement='bottom' ";
   ...
   */
   private function __mostrarCurs($tagTitol) {
      ...
      $mostrar="<div class='prisma-related-course border-radius-2'><div class='prisma-related-course-overlay'>";
      ...
      // $mostrar .= "<img role='img' src='".$linkImg."' alt=\"".$dscImg."\" ";
      // $mostrar .= "class='prisma-related-course-image w-100'";
      $mostrar .= "<picture><source type='image/webp' data-srcset='".$linkImgWeb."' ";
      $mostrar .= "alt=\"".$dscImg."\" class='prisma-related-course-image w-100' ";
      if ($this->dispositiu=='ordinador') {
         $mostrar .= "onclick=\"mostrarTaset('".$codiMin."')\">
         <source type='image/jpeg' data-srcset='".$linkImg."'
         alt=\"".$dscImg."\" class='prisma-related-course-image w-100'
         onclick=\"mostrarTaset('".$codiMin."')\">";
         $mostrar .= "<img data-src='".$linkImg."' alt=\"".$dscImg."\" ";
         $mostrar .= "class='prisma-related-course-image w-100 lazyload' ";
         $mostrar .= "onclick=\"mostrarTaset('".$codiMin."')\"></picture>";
         $mostrar .= "<div class='prisma-course-middle position-absolute'>";
         $mostrar .= "<button role='button' id='boto_veure_tastet_".$codiMin."' ";
         $mostrar .= "class='prisma-course-text border-radius-2 font-weight-bold' data-placement='bottom' ";
         $mostrar .= "type='button' data-html='true' title=\"Mostra el tastet del curs ".$titol."\" ";
         $mostrar .= "onclick=\"mostrarTaset('".$codiMin."')\" aria-label=\"Mostra tastet ".$titol."\" ";
         $mostrar .= "data-keyboard='true'>Mostra'n un tastet</button></div>";
      }
      else {
         $mostrar .= "onclick=\"location.href='https://www.prisma.cat".$linkUrl."'\">
      ...
      $mostrar .= "</div><div class='related-course-content border-radius-2'>";
      $mostrar .= "<div class='espai-titol-etiqueta'><div class='titol flex-1-0-auto'>".$etiquetaNou.$etiquetaDesc;
      $mostrar .= "<a role='link' class='color-text' href='https://www.prisma.cat".$linkUrl."' ";
      $mostrar .= "title=\"Mostra la informació del curs ".$titol."\">";
      $mostrar .= "<".$tagTitol.">".$titol."</".$tagTitol."></a></div>";
      $mostrar .= "<div class='perfil-curs-related'>";
      $cntPerf=0;
      while ($this->obtenirPerfil($cntPerf)!=null) {
         ...
         $mostrar .= "<div role='button' title=\"Mostra totes les edicions dels cursos ";
         $mostrar .= "que acrediten el perfil «".$perfilEd->obtenirNomLlarg()->obtenirText()."»\" ";
         $mostrar .= "class='perfil-curs text-center ".$classPerfil." border-radius-2' ";
         $mostrar .= "onclick=\"location.href='https://www.prisma.cat/perfils-professionals'\">";
         $mostrar .= $perfilEd->obtenirNomCurt()->obtenirText()."</div>";
         $cntPerf++;
      }
      ...
      $mostrar .= "<div style='clear: both;'></div></div></div></div>";
      return $mostrar;
   }
   ...
   */
   ...
   /**
   * @brief Comprova si el curs és només per l'estiu
   * @return Retirna si el curs és només per l'estiu
   */
   public function __esCursPerNomesEstiu() {
      $nomestiu = 0;
      ...
      $curs_escolar1 = $any_anterior."/".date("Y");
   	$curs_escolar2 = date("Y")."/".$any_seguent;

      if ($num_mes_actual>=1 && $num_mes_actual<8)
   		$cursEscolar = $curs_escolar1;
      else
         $cursEscolar = $curs_escolar2;

      /* Busco les edicions disponibles */
      $cnsEd = "SELECT MES, ANY FROM curs WHERE PUBLIC=1 AND
      	CURS!='PROVA' AND CURS NOT LIKE '%0%' AND CURS = ? AND ANY = ?
         GROUP BY ANY, MES LIMIT 1";
      $stmtEd = $connexio->prepare($cnsEd);
      $stmtEd->bind_param("sd", $cursEd, $anyNow);

      $anyNow = date('Y');
      $cursEd = $this->obtenirCodi()->obtenirText();

      //si la primera edició és 06, 07 o 08, mostrar etiqueta
      $stmtEd->execute();
  		$stmtEd->bind_result($mesFirstEd, $anyFirstEd);
  		$stmtEd->fetch();

      if ( $mesFirstEd == '07' || $mesFirstEd == '08' ) {
         $nomestiu = 1;
         $connexio->closeStmt();

         /* si l'edició encara esta oberta perfecte.
         Si l'edició ja no està oberta
            s'ha de comprovar si existeix una edició futura.
               si no està oberta es queda com abans
               si nhi ha alguna doberta, es busca la propera edició del curs escolar*/

         // Buscar si l'edició del $mesFirstEd + $anyNow encara esta oberta
         $cnsEdOberta = "SELECT MES, ANY FROM curs AS c INNER JOIN aula AS a ON c.ID_AULA=a.ID_AULA
         INNER JOIN rel_cuho AS r ON r.ID_CUHO=a.ID_CUHO INNER JOIN honoraris AS h
         ON r.ID_HONO=h.ID WHERE PUBLIC=1 AND c.CURS!='PROVA' AND c.CURS NOT
         LIKE '%0%' AND c.ESTAT!='0' AND r.ACTIU=1 AND (DATEDIFF(DATAI + ?,CURRENT_DATE)>0)
         AND c.CURS NOT LIKE '%JOR%' AND (a.ID_CUHO=17 OR a.ID_CUHO=13 OR
         (a.ID_CUHO!=17 AND h.DNI_TUTOR='GENERIC') OR (a.ID_CUHO!=17 AND
         h.DNI_TUTOR!='GENERIC' AND AULA='A' AND perfil='tutor' AND ORDRE_TUTOR=1))
         AND MES = ? AND ANY = ? GROUP BY ANY, MES ORDER BY ANY, MES";
         $stmtEd2 = $connexio->prepare($cnsEdOberta);
         $stmtEd2->bind_param("dsd", $diesObets, $mesFirstEd, $anyNow);
         $stmtEd2->execute();
         $stmtEd2->store_result();
         if ( $stmtEd2->num_rows() == 0 ) { // Si no està oberta
               $connexio->closeStmt();
            // Buscar si existeix una edició en el curs escolar
            $cnsProxEdCursEsc = "SELECT MES, ANY FROM curs WHERE CURS = ?
            AND CURS_ESCOLAR = ? ORDER BY DATAI";
            $stmtProxEdCursEsc = $connexio->prepare($cnsProxEdCursEsc);
            $stmtProxEdCursEsc->bind_param("ss", $cursEd, $cursEscolar);
            $stmtProxEdCursEsc->execute();
            $stmtProxEdCursEsc->store_result();
            // echo $cursEd." ".$cursEscolar."<br />";
            if ( $stmtProxEdCursEsc->num_rows() > 0 ) { // Si no està oberta
               // Buscar la pròxima edició disponible.
               $stmtProxEdCursEsc->bind_result($mesProxEd, $anyProxEd);
   				$stmtProxEdCursEsc->fetch();
               // Si la pròxima edició disponible no és el 07 o el 08, $nomestiu = 0
               if ( $mesProxEd != '07' && $mesProxEd != '08' ) {
                  $nomestiu = 0;
               }
            }
         }
   ...
   * @return Si el curs és nou, mostra la etiqueta del curs nou
   */
   protected function __etiquetaNomesEstiu() {
      $etiquetaNou='';
      if ( $this->__esCursPerNomesEstiu() && !$this->__esCursNou() )
        $etiquetaNou = "<div class='noComencat'>
        <div class='border' style='border:16px solid #77002a!important;border-left: 16px solid transparent!important;opacity: .85;'></div>
        <div class='nom'>NOMÉS A L'ESTIU!</div>
        </div>";
      return $etiquetaNou;
   }
   ...
   */
   protected function __etiquetaCursAmbDescompte() {
      $etiqueta='';
      ...
      $consultaEd = "SELECT MES FROM curs WHERE CURS=? AND PUBLIC = 1
      ...
      $stmtEd->bind_param("s", $codiCurs, $diesObets, $limitEd);
      $stmtEd->execute();
      $stmtEd->bind_result($mesDesc);
      while ( $stmtEd->fetch() ){
   ...
   */
   private function __etiquetaNoHaComencat($textNoComencat) {
      ...
      $consultaData = "SELECT DATAI, MES FROM curs WHERE CURS=? ORDER BY ANY, MES LIMIT 1";
