...
if ( $altresFiltres[0] == 0 )
	...
	$inclouCursosEstiu = 1;
// if (count($filtres[3])>0) $nivellsFiltres[$i]=4;
...
$cns="SELECT CODI_CURS FROM informacio as i INNER JOIN curs as c ON CODI_CURS=CURS
	...
	WHERE f.DATAI<=CURRENT_TIME AND (f.DATAF IS NULL OR CURRENT_TIME<=f.DATAF) AND ".$filtres." i.ESTAT=1 AND PUBLIC=1 AND
	(c.ESTAT!='0' AND c.ESTAT!='T') AND i.TIPUS_CURS != 'T' AND i.TIPUS_CURS != 'C'
...
if ( $stmt = $connexio->prepare($cns) ) {
	...
	while ($stmt->fetch()) {
		...
		if (
			...
			if ( $curs->obtenirEstat()==1 && ( $qualsevolEdicio || (!$qualsevolEdicio && $curs->inscripcionsObertes($ed[2], $ed[1])) ) ) {
				// echo "ESTAT 1	";
...
$cnsPack="SELECT i.ID_PACK FROM info_pack as i INNER JOIN packs as p ON p.ID_PACK = i.ID_PACK
	...
	i.ESTAT=1 AND c.PUBLIC=1 AND p.PUBLIC=1 AND (c.ESTAT!=0 AND c.ESTAT!='T')
...
if ( $stmt = $connexio->prepare($cnsPack) ) {
	...
	while ($stmt->fetch()) {
		...
		if ( $pack->obtenirEstat()==1 && ( $qualsevolEdicio || (!$qualsevolEdicio && $pack->inscripcionsObertes($ed[2], $ed[1])) ) ) {
			...
			$cursPertany=false;
			if ( $trobatT==1 && $trobatN==1 ) $cursPertany=true;
			if ( $cdd == 1 ) $cursPertany = false;
			if ( $mixtos == 1 && !$pack->cursEsMixt() ) $cursPertany = false;
			if ( $nousCursos==1 ) $cursPertany = false;
