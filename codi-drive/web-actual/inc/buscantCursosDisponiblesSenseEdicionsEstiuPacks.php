...
$edicions=''; $hores=''; $temes=''; $nivells=''; $altres='';
...
while ($i<count($filtres[0]) && !$qualsevolEdicio) {
	...
	if ($ed[1] == '07' && $ed[2] =='2024')
		$edicions .= " OR ( ANY = 2024 AND MES = '08' AND HORES = 15 )" ;
...
$diffMostrarTotsCursos = intval($date1->diff($date2)->days);
$signe = $date1->diff($date2)->format('%R');
if ($signe == "-") $diffMostrarTotsCursos = 0 - $diffMostrarTotsCursos;

// if ( $diffMostrarTotsCursos >= 0 )
// 	echo "No mostrar cursos només d'estiu";
// else
// 	echo "Mostrar cursos només d'estiu";
...
$cns="SELECT CODI_CURS FROM informacio as i INNER JOIN curs as c ON CODI_CURS=CURS
	...
	".$filtres." i.ESTAT=1 AND PUBLIC=1 AND (c.ESTAT!='0' AND c.ESTAT!='T')
...
$cnsPack="SELECT i.ID_PACK FROM info_pack as i INNER JOIN packs as p ON p.ID_PACK = i.ID_PACK
	...
	WHERE f.DATAI<=CURRENT_TIME AND (f.DATAF IS NULL OR CURRENT_TIME<=f.DATAF) AND ".$filtres." i.ESTAT=1 AND c.PUBLIC=1 AND p.PUBLIC=1 AND (c.ESTAT!=0 AND c.ESTAT!='T')
	GROUP BY i.ID_PACK ORDER BY ".$ordre;
if ( $stmt = $connexio->prepare($cnsPack) ) {
	$stmt->execute();
	$stmt->bind_result($idPack);
	while ($stmt->fetch()) {
		echo "CODICURS".$codiCurs."	"."dispositiu:".$dispositiu;
		$pack = new Pack($idPack, $dispositiu);
		echo $idPack."<br />";
		echo $pack->obtenirEstat()."<br />";
		echo $qualsevolEdicio."<br />";
		echo $pack->inscripcionsObertes($ed[2], $ed[1])."<br />";
