...
class Curs {
   ...
   protected $tipus; /**< Text El tipus d'un curs. ex. N */
   protected $sincron; /**< Int 1 és sincrón, 0 si no ho és, 2 és mixt. ex. N */
   ...
   */
   public function __construct($codi, $dispositiu) {
      ...
      $consultaCurs = "SELECT TITOL, SHORT_DESC, ID_VIDEO, ID_IMG_LARGE, ID_IMG_PERFIL,
         ID_IMG_SMALL, ID_AMIGABLE, TIPUS_CURS, SINCRON FROM informacio WHERE CODI_CURS=? AND ESTAT=1";
      $stmtInfo=$connexio->prepare($consultaCurs);
      $stmtInfo->bind_param("s", $codi);
      $stmtInfo->execute();
      $stmtInfo->bind_result($titol, $shortDesc, $idVideo, $idImgLarge, $idImgPerf, $idImgSmall, $idAmig, $tipusCurs, $sincron);
         ...
         if ( $stmtHoresPreu->num_rows() > 0 ) {
            ...
            $diesObets=0;
             ...
             $consultaHoresPreu = "SELECT HORES, CURS_ESCOLAR, GTAF, ID_PREU, CDD FROM curs WHERE
                DATAI+?>CURRENT_DATE AND CURS LIKE ? AND ESTAT != '0' AND PUBLIC=1
                AND (CURS NOT LIKE '%JOR%') AND (CURS NOT LIKE '%0%')
   ...
   */
   ...
   /*
   * @brief Obtens el tipus del curs
   ...
   */
   ...
   /*
   * @brief Obtens el perfils del curs
   * @return Si el curs té un perfil, retorna els perfils del curs. Altrament, null.
   */
   public function obtenirPerfils() {
      return $this->perfil;
   }
   /*
   * @brief Obtens el perfils pendents del curs
   * @return Si el curs té un perfil pendents, retorna els perfils del curs. Altrament, null.
   */
   public function obtenirPerfilsPendents() {
      return $this->perfil_pendent;
   }
   ...
   * @return Si el curs té un nivell i la posicio existeix a la llista de nivells, retorna el perfil del curs. Altrament, null.
   */
   public function obtenirNivell($posicio) {
   ...
   */
   ...
   public function cursEsMixt() {
     return ( $this->sincron  == 2 );
   }
   public function cursEsTaller() {
     return ( $this->tipus->obtenirText()  == 'T'  );
   }
   ...
   */
   public function inscripcionsObertes($any, $mes) {
      ...
      * @return Obtens true si les inscripcions de l'any $any i del mes $edicio estan
      obertes. Altrament, retorna false
      */
      public function inscripcionsObertesEstiu($any, $mes) {
         $oberta = true;
         $codiCurs = $this->codi->obtenirText();
         $hores = $this->hores->obtenirNumero();
         ...
         $cnsEd = "SELECT MES, ANY FROM curs AS c INNER JOIN aula AS a ON
            ...
            INNER JOIN honoraris AS h ON r.ID_HONO=h.ID WHERE PUBLIC=1 AND
            c.CURS!='PROVA' AND c.CURS NOT LIKE '%0%' AND c.ESTAT!='0' AND r.ACTIU=1
            AND (c.MES = '06' OR c.MES = '07' OR c.MES = '08') AND c.ANY >= ?
            AND (".$dateDifss.") AND c.CURS NOT LIKE '%JOR%' AND c.PUBLIC = 1 AND
            (a.ID_CUHO=17 OR a.ID_CUHO=13 OR (a.ID_CUHO!=17 AND h.DNI_TUTOR='GENERIC')
            OR (a.ID_CUHO!=17 AND  h.DNI_TUTOR!='GENERIC' AND AULA='A' AND perfil='tutor'
            )) AND ANY=? AND MES=? AND c.CURS=? GROUP BY ANY, MES";
         $stmtEd = $connexio->prepare($cnsEd);
         $stmtEd->bind_param("ddss", $anyActual, $any, $mes, $codiCurs);
   ...
   public function crearTastet(){
      ...
      */
      ...
      if ($this->dispositiu=='ordinador') {
         ...
         $etiquetaTaller=$this->__etiquetaTaller();
         ...
         $mostrar .= "border-0 font-weight-bold'>".$etiquetaNou.$etiquetaTaller.$etiquetaDesc.$etiquetaCDD.$titolHTML."</div>";
         ...
         if ( !$this->esCursSubvencionat() ) {
           ...
           if ( $codiCurs == 'SUI' || $codiCurs == 'SDA' ) $idPreu = 17;
           $stmtPreus->execute();
   ...
   */
   public function mostrarCursTutor($dni_tutor, $midaPantalla) {
      ...
      $mostrar.=$this->__etiquetaNomesEstiu();
      ...
      if ($this->dispositiu=='ordinador') {
         ...
         data-placement='bottom' data-keyboard='true'>Mostra'n un resum</button></div>";
      }
      else
   ...
   */
   public function mostrarCursPerfil($midaPantalla) {
      ...
      $versioImg = $this->obtenirImgPerfil()->obtenirVersio();
      $linkImg .= "?ver=".$versioImg;
      $shortDesc=$this->obtenirShortDesc()->obtenirText();
      ...
      $mostrar.="</div></div>";
      $mostrar.="<div class='col-md-4 prisma-perfil-overlay position-relative'>";
      $mostrar.=$this->__etiquetaNomesEstiu();
      $mostrar.="<img role='img' src='".$linkImg."' alt=\"".$dscImg."\" ";
      ...
      if ($this->dispositiu=='ordinador') {
         ...
         $mostrar.="data-keyboard='true'>Mostra'n un resum</button></div>";
      }
      else
         $mostrar.="onclick=\"location.href='https://www.prisma.cat".$linkUrl."'\">";
    ...
    */
    public function mostrarCursSubvencionat($midaPantalla) {
       ...
       $etiquetaCDD=$this->__etiquetaCDD2();
       $etiquetaSubv=$this->__etiquetaCursSubvencionat();
       $etiquetes = $etiquetaNou.$etiquetaSubv.$etiquetaCDD;
       //   $etiquetaDesc=$this->etiquetaCursAmbDescompte();
   ...
   public function mostrarCursBescanvia() {
      ...
      $mostrar.="<div class='col-12 col-sm-6 col-lg-3 pl-2 pr-2 mb-4'>";
         $mostrar.="<div class='curs-regal border rounded'>";
            $mostrar.="<div class='curs-regal-overlay position-relative'>";
               /* Quan encara no ha passat la primera edicio i fins a dues setmanes abans de l'inici
               del curs apareix "A partir de X" on X es el mes escrit i es la primera edicio  */
               $mostrar.=$this->__etiquetaNoHaComencat();
               $mostrar.=$this->__etiquetaNomesEstiu();
      ...
      // $etiquetaCDD=$this->__etiquetaCDD2();
   	...
   	$mostrar.="<div class='col-12 col-sm-6 col-lg-3 pl-2 pr-2 mb-4'>";
   		$mostrar.="<div id='descompte-grup-".$codiMin."' class='curs-descompte-grup border rounded'>";
   			$mostrar.="<div class='curs-descompte-grup-overlay position-relative'>";
   				/* Quan encara no ha passat la primera edicio i fins a dues setmanes abans de l'inici
   				del curs apareix "A partir de X" on X es el mes escrit i es la primera edicio  */
   				$mostrar.=$this->__etiquetaNoHaComencat();
          $mostrar.=$this->__etiquetaNomesEstiu();
   			...
   			$mostrar.="<div class='curs-descompte-grup-titol bg-white'>";
   				$mostrar.="<div class='espai-titol-etiqueta d-flex flex-column pr-3 pl-3 pt-2'>";
   					$mostrar.="<div class='titol flex-grow-1'>".$etiquetaNou.$etiquetaDesc.$etiquetaCDD;
   							$mostrar.="<div class='negreta500'>".$titolHTML."</div>";
   					$mostrar.="</div>";
                  if ( $this->perfil_pendent == null)
   					  $mostrar.="<div class='perfil-curs-related mt-2 d-flex flex-column flex-md-row'>";
   ...
   */
   public function mostrarCursBlocPerfils() {
      ...
      <div class='prisma-related-course-overlay position-relative'>";
      ...
      $mostrar .= "<div class='espai-titol-etiqueta d-flex flex-column'><div class='titol flex-1-0-auto'>";
   ...
   */
   public function mostrarCursRelTrobada() {
      ...
      <div class='titol flex-1-0-auto'>".$etiquetaNou.$etiquetaDesc."
   ...
   */
   private function __mostrarCurs($tagTitol) {
      ...
      $etiquetaCDD=$this->__etiquetaCDD2();
      $etiquetaSubv=$this->__etiquetaCursSubvencionat();
      $etiquetaMixt=$this->__etiquetaMIXT2();

      $etiquetes = $etiquetaNou.$etiquetaTaller.$etiquetaDesc.$etiquetaSubv.$etiquetaCDD.$etiquetaMixt;
  ...
  */
   ...
   */
   public function __esCursPerNomesEstiu() {
      ...
      // CALCULAR EL CURS ESCOLAR
      // if ( date('n') >= 1 && date('n') < 8 ) $curs_escolar = date('Y')-1."/".date('Y');
      // else $curs_escolar = date('Y')."/".date('Y')+1;

      /* Busco les edicions disponibles */
      // $cnsEd = "SELECT MES, ANY FROM curs AS c INNER JOIN aula AS a ON
      // 	c.ID_AULA=a.ID_AULA INNER JOIN rel_cuho AS r ON r.ID_CUHO=a.ID_CUHO
      // 	INNER JOIN honoraris AS h ON r.ID_HONO=h.ID WHERE PUBLIC=1 AND
      // 	c.CURS!='PROVA' AND c.CURS NOT LIKE '%0%' AND c.PUBLIC!='1' AND r.ACTIU=1
      // 	AND c.CURS NOT LIKE '%JOR%' AND
      // 	(a.ID_CUHO=17 OR a.ID_CUHO=13 OR (a.ID_CUHO!=17 AND h.DNI_TUTOR='GENERIC')
      // 	OR (a.ID_CUHO!=17 AND  h.DNI_TUTOR!='GENERIC' AND AULA='A' AND perfil='tutor'))
      //    AND c.CURS = ? AND (DATEDIFF(DATAI + ?,CURRENT_DATE)>0) GROUP BY ANY, MES LIMIT 1";
      ...
      $cnsEd = "SELECT MES, ANY FROM curs WHERE PUBLIC=1 AND
         ...
         GROUP BY ANY, MES ORDER BY MES LIMIT 1";
      $stmtEd = $connexio->prepare($cnsEd);
      ...
      //if ( $mesFirstEd == '06' || $mesFirstEd == '07' || $mesFirstEd == '08' )
	  if ( $mesFirstEd == '07' || $mesFirstEd == '08' ) {
         ...
         /*  a partir d'ara els cursos que només son d'estiu, poden deixar de ser només d'estiu
            si continua siguent només estiu però estem més endavant del 07,
          per tant tenim informació del proxim curs escolar, revisem si segueix sent un curs de només estiu. */
         if ( $nomestiu == 1 && $num_mes_actual >= 7 ) {
            // consulto si existeix una edicio en el PROXIM curs ESCOLAR que no siguin el 06, 07 i 08.
            $cnsProxEdCursEsc = "SELECT MES, ANY FROM curs WHERE CURS = ?
            AND CURS_ESCOLAR = ? AND MES != '06' AND MES != '07' AND MES != '08' ORDER BY DATAI";
            $stmtProxEdCursEsc = $connexio->prepare($cnsProxEdCursEsc);
            $stmtProxEdCursEsc->bind_param("ss", $cursEd, $curs_escolar2);
            $stmtProxEdCursEsc->execute();
            $stmtProxEdCursEsc->store_result();
            // echo $cursEd." ".$cursEscolar."<br />";
            if ( $stmtProxEdCursEsc->num_rows() > 0 ) { // Si no està oberta
  ...
  */
  ...
  /**
  * @brief Mostra la etiqueta en format banderola
  * @return Si el curs es MIXT EN LINIA mostra la etiqueta de «MIXT EN LINIA» en format banderola
  */
  protected function __etiquetaMIXT() {
     $etiqueta = '';
     if ( $this->cursEsMixt() ) {
        $objTxt = new Text("MIXT EN LÍNIA");
        $txt = $objTxt->obtenirTextHTML();
        $etiqueta = "<div class='etiquetaMixt position-absolute'>
        <div class='nom'>".$txt."</div></div>";
     }
     return $etiqueta;
  }
  ...
  /**
  * @brief Mostra la etiqueta en format etiqueta NOU
  * @return Si el curs és MIXT EN LINIA, mostra la etiqueta de «MIXT EN LINIA» en format  etiqueta MIXT EN LINIA
  */
  protected function __etiquetaMIXT2() {
     $etiqueta = '';
     if ( $this->cursEsMixt() ) {
        $objTxt = new Text("MIXT EN LÍNIA");
        $txt = $objTxt->obtenirTextHTML();
        $etiqueta="<div class='nou mixt text-white float-left'>".$txt."</div>";
     }
     return $etiqueta;
  }
  ...
  /**
  * @brief Mostra la etiqueta en format etiqueta NOU
  * @return Si el curs és taller, mostra la etiqueta de «TALLER» en format  etiqueta
  */
  protected function __etiquetaTaller() {
     $etiqueta = '';
     if ( $this->cursEsTaller() ) {
        $objTxt = new Text("TALLER");
        $txt = $objTxt->obtenirTextHTML();
        $etiqueta="<div class='nou taller text-white float-left'>".$txt."</div>";
     }
     return $etiqueta;
  }
  ...
  * @return Si el curs és taller, mostra la etiqueta de «TALLER» en format  etiqueta
  */
  protected function __etiquetaNovetat() {
     $etiqueta = '';
     if ( $this->cursEsTaller() ) {
        $objTxt = new Text("NOVETAT");
        $txt = $objTxt->obtenirTextHTML();
        $etiqueta="<div class='nou float-left'>".$txt."</div>";
     }
     return $etiqueta;
  }
  ...
  */
   ...
   */
   public function etiquetaCursAmbDescompte() {
      ...
      $stmtEd->bind_param("sd", $codiCurs, $diesObets);
   ...
   */
   private function __etiquetaNoHaComencat() {
      $etiqueta='';
      ...
      // codi per marcar ultima edicio als cursos de 15 hores
      if ( $this->cursEsMixt() ) {
         $etiqueta.="<div class='noComencat'>";
         $etiqueta.="<div class='border'></div>";
         $etiqueta.="<div class='nom'>ÚLTIMES EDICIONS!</div>";
         $etiqueta.="</div>";
      }
