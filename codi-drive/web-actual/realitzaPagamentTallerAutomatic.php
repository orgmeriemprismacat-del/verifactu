	...
	try {
	   ...
	   $codiResposta = $miObj->getParameter("Ds_Response");
		...
		if (intval($codiResposta)>=0 && intval($codiResposta)<=99) {
			...
			$cnsCurs = "SELECT TITOL, HORES, DATAI, LLOC2 FROM info_taller INNER JOIN informacio ON info_taller.CODI_CURS = informacio.CODI_CURS
			WHERE ANY=? AND MES=? AND informacio.CODI_CURS=?";
			if ( $stmt=$connexio->prepare($cnsCurs) ) {
				$stmt->bind_param("dss", $any, $mes, $codiCurs);
				$stmt->execute();
				$stmt->bind_result($titol, $hores, $datai, $lloc);
				$stmt->fetch();
				$connexio->closeStmt();
			}
			else {
			...
			$mailSuport->addSubject($subject);
			$mailSuport->addTo("suport.informatic@prisma.cat");
			$mailSuport->addMissatgeTiquet($missPreDiv, $factura, '');
			$mailSuport->sendMessage();
		   ...
		   //Si el codi curs té numeros es tracta d'una jornada
			$concepte1 = "Taller ".$titol;
			...
			if ( floatval($pagats) >= floatval($apagar) ) {
				$updateBD = "UPDATE inscripcions SET `DATA PAG`=CURRENT_TIME WHERE IDPAG=? AND (`INSC CURS`='0' OR `INSC CURS`='1' OR `INSC CURS`='M')";
				if ( $stmtUpdate=$connexio->prepare($updateBD) ) {
			...
			<p>Hem rebut correctament el pagament de la teva inscripció al taller presencial <strong style='color: #496baa'>".$titol."</strong> que es
			realitza el <strong>".$dataIniciLlarga."</strong> a l'espai de <strong>".$lloc."</strong>. </p>
			...
			if ( $inscrit == 'M' && $pendentPagar <= 0 ) {
				...
				<div style='background-color: #e8ecf5; border: 1px solid #D7DEEE; border-radius: 2px; padding: 5px 25px; margin-bottom: 20px'>
					<p><strong>N. de comanda:</strong> ".$ordre."</p>
					<p><strong>Concepte:</strong> ".$dniTitularPag." | ".$titol."</p>
					<p><strong>Data:</strong> ".$dataIniciLlarga."</p>
