...
class PaginaDescomptes {
	...
	*/
	private function __getHoresCursos() {
		...
		$stmtEx = $connexio2->prepare($cnsExsiteixCurs);
   ...
   */
	private function __getTitle() {
		...
		$container_titol = "<div class='titol my-4'>
			<h1>Descomptes PrisMa</h1>
			<div class='d-flex flex-column flex-sm-row align-items-left'>
				<div class='subtitol flex-grow-1 py-2'>Des de l’Associació PrisMa oferim diferents tipus de descomptes (no acumulables)</div>
			</div>
		</div>";
   ...
   */
	...
	public function __getContentPage() {
		$icon = "<span class='cnt-icon prisma ml-2'><img class='icon' src='https://www.prisma.cat/img/pagines/prisma-icon.png'></span>";
		$cntDescAlumnPrisma = "<h2>Alumnes PrisMa".$icon."</h2>".$this->__getSection(1);

		$icon = "<span class='cnt-icon carnet-jove ml-2'><img class='icon' src='https://www.carnetjove.cat/bundles/undfmain/images/img26.jpg?v1.3.9.0-utmp'></span>";
		$cntDescCarnetJove = "<h2>Carnet Jove".$icon."</h2>".$this->__getSection(2);

		$icon = "<span class='cnt-icon ml-2'><i class='icon fas fa-address-card'></i></span>";
		$cntDescSocials = "<h2>Socials".$icon."</h2>".$this->__getSection(3);

		$icon = "<span class='cnt-icon union ml-2'><img class='icon' src='https://usoc.cat/web/wp-content/uploads/ico-elecciones-1.png'></span>";
		$cntDescUsoc = "<h2>Unió Sindical Obrera de Catalunya".$icon."</h2>".$this->__getSection(4);

		$icon = "<span class='cnt-icon descomptes ml-2'><img class='icon' src='https://www.prisma.cat/img/pagines/descompte-grups-icon.png'/></span>";
		...
		$page = "<div class='separacio-peu border border-radius-2 bg-white px-4 py-2'>
			<div id='cntPage' class='cntPage pt-1'>
	...
	public function __getSection( $type ) {
		...
		if ( $type == 1 ) {
			//Alumnes PrisMa
			$cnt = "<p>Les persones que <strong>esteu fent o hàgiu fet un curs a PrisMa</strong> gaudireu d’un <strong>descompte</strong> en el preu dels cursos:</p>";
			...
			//Carnet jove
			$cnt = "<p>Les persones que tingueu un <strong>Carnet Jove</strong> vigent gaudireu d’un <strong>descompte</strong> en el preu dels cursos:</p>";
			...
			//Socials
			$cnt = "<p>Oferim un <strong>descompte del 20 %</strong> sobre el preu del curs a les persones que tingueu l'<strong>acreditació</strong> de <strong>família nombrosa</strong>, de <strong>família monoparental</strong>, de <strong>discapacitat del 33 % o més</strong>, o de situació de <strong>violència de gènere</strong>.</p>";
			$table = $this->calcTableDesc( 5 );
			$cnt .= "<div class='d-flex justify-content-center align-items-center my-3'>".$table."</div>";
			$cnt .= "<p>En el moment de fer la inscripció caldrà que adjunteu un document que justifiqui el descompte (carnet, resolució, etc.) i esperar a la validació des de PrisMa per rebre les dades per fer el pagament amb el descompte. Les acreditacions es mantindran mentre siguin necessàries per a la confirmació de l'import del curs.</p>";
		}
		else if ( $type == 4 ) {
			//USOC
			$cnt = "<p>Oferim un <strong>descompte del 20 %</strong> sobre el preu del curs a les persones que estigueu <strong>afiliades a la Unió Sindical Obrera de Catalunya (USOC)</strong>.</p>";
			...
			$cnt = "<p>Si <strong>tres persones o més</strong> us inscriviu com a grup en un dels nostres cursos en línia, se us aplicarà un <strong>descompte</strong> segons el nombre d’integrants del grup:</p>";
			$cnt .= "<div class='d-flex justify-content-center align-items-center my-3'>".$table."</div>";
			$cnt .= "<p>A més, si sou 15 alumnes o més podreu gaudir d’una aula exclusiva per al vostre grup.</p>";
			$cnt .= "<p>Més informació i formulari d’inscripció grupal a la pàgina
			<a href='https://www.prisma.cat/descompte-curs-grup' target='_self' class='font-weight-bold' title='Descompte per a grups!'>Descomptes per a grups i centres escolars</a>.</p>";
		}
