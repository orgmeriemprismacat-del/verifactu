<?php
	...
	/* TO DO: DFI DESCOMENTAR */

	/* TO DO: a eliminar */
	$dniTitularPag = "77922662L";
	$importPag = 52.5;
	$order = 1;
	$codiRegal = 'I8EDSAWFNB';
	$nomTitularPag = "Meriem Abjil Bajja";
	/* TO DO: FI a eliminar */
	...
	try {
	   ...
	   // $codiResposta = $miObj->getParameter("Ds_Response");
		...
		$mailProves->addTo("meriem.prisma.cat@gmail.com");
		...
		if (intval($codiResposta)>=0 && intval($codiResposta)<=99) {
			...
			/* TO DO: a eliminar */
			echo "DADES CURS<br />".$nomCurs."<br>".$codiCurs."<br>".$nomC."<br>".$nifC."<br>".$mailC."<br>".$adrecaC."<br>".$pobleC."<br>".$cpC."<br>".$factRel."<br />";
			$nomFromProves = "Gestió PrisMa";
			$correuFromProves = "gestio@prisma.cat";
			$correuReplyProves = "gestio@prisma.cat";
			...
			$mailProves = new Mail();
			$mailProves->addHeaders($nomFromProves, $correuFromProves, $correuReplyProves);
			$mailProves->addSubject("CODI");
			$mailProves->addTo("meriem.prisma.cat@gmail.com");
			$mailProves->addMissatgeTiquet("hola", $codiRegal, '');
			$mailProves->sendMessage();
			...
			$mailProves = new Mail();
			$mailProves->addHeaders($nomFromProves, $correuFromProves, $correuReplyProves);
			$mailProves->addSubject("concepte1");
			$mailProves->addTo("meriem.prisma.cat@gmail.com");
			$mailProves->addMissatgeTiquet("hola", $concepte1, '');
			$mailProves->sendMessage();
			/* TO DO: FI a eliminar */

			require_once 'Curs.php';
			/* TO DO: a eliminar */
			echo "codiCurs1".$codiCurs."<br />";
			echo "intval".intval($codiCurs)."<br />";
			/* TO DO: FI a eliminar */
			...
			$mailProves = new Mail();
			$mailProves->addHeaders($nomFromProves, $correuFromProves, $correuReplyProves);
			$mailProves->addSubject("hores");
			$mailProves->addTo("meriem.prisma.cat@gmail.com");
			$mailProves->addMissatgeTiquet("hola", $hores, '');
			$mailProves->sendMessage();
			/* TO DO: FI a eliminar */

			if ($factRel==0) { //No ha pagat
				echo $insertBD."<br>".$factura."<br>".$tipusFact."<br>".$anyFiscal."<br>".$ordreFact."<br>
				".$numFact."<br>".$dataActual."<br>".$dataActual."<br>".$order."<br>".$rao."<br>".$cif."<br>
				".$adrecaC."<br>".$cpC."<br>".$pobleC."<br>".$concepte1."<br>".$concepte2."<br>".$importPag."<br>
				".$entitat."<br>".$formaPagament."<br>".$codiCurs."<br>".$hores."<br>";
				...
				$mailProves = new Mail();
				$mailProves->addHeaders($nomFromProves, $correuFromProves, $correuReplyProves);
				$mailProves->addSubject("insert");
				$mailProves->addTo("meriem.prisma.cat@gmail.com");
				$mailProves->addMissatgeTiquet("hola", $insertBD, '');
				$mailProves->sendMessage();
				/* TO DO: FI a eliminar */
				...
				$mailProves = new Mail();
				$mailProves->addHeaders($nomFromProves, $correuFromProves, $correuReplyProves);
				$mailProves->addSubject("upd");
				$mailProves->addTo("meriem.prisma.cat@gmail.com");
				$mailProves->addMissatgeTiquet("hola", $factura." ".$codiRegal, '');
				$mailProves->sendMessage();
				/* TO DO: FI a eliminar */
	      ...
	      if ($codiResposta == "0101") {
			...
			$nomTo = $nameUser;
			$correuTo = 'resguard.gestio@prisma.cat';
			$correuTo = 'meriem.prisma.cat@prisma.cat';
