...
*/
class InscripcioPack{
   private $idPack; /** Text IdPack d'una Inscripcio ex: ACRE */
   ...
   public function __construct($idUrl, $dispositiu) {
      ...
      $cnsInfo = "SELECT ID_PACK, CODI, TITOL FROM info_pack WHERE ID_URL=? AND ESTAT=1";

      require_once 'ConnexioBBDD_PreparedStatment.php';
      $connexio = new ConnexioBBDDSTMT();
      $connexio->connectarBD();
      if ( $stm = $connexio->prepare($cnsInfo) ) {
         $stm->bind_param("d", $idUrl);
         $stm->execute();
         $stm->store_result();
         if ( $stm->num_rows() <= 0 ) {
            $this->estat=0;
             throw new Exception('',2901);
         }
         else {
            $stm->bind_result($idPack, $codiPack, $titol);
            $stm->fetch();
         }
         $connexio->closeStmt();
      }
      else {
         throw new Exception('',2900);
      }
      ...
      if ($codiPack!=null AND $codiPack!='')
         $this->codi = new Text($codiPack);
      else
         $this->codi = null;
   ...
   */
   public function mostrar() {
      ...
      $cnsInfoOrig = "SELECT DATAI, DATAF, CURS_ESCOLAR, FISS, GTAF, DATA_RESOL, ANY, MES,
   		c.CURS, p.ID_CURS, HORES, i.ID, i.ID_AMIGABLE, c.ESTAT
      ...
      if ( $stmt = $connexio->prepare($cnsInfoOrig) ) {
         ...
         $idPack = $this->idPack->obtenirText();
         $stmt->execute();
      ...
      $textP = $objText->obtenirTextHTML();

      $mostrar .= "<h1 class='mb-4'>Formulari d'inscripci&oacute</h1>
      <h2 class='pt-3 mt-0'>".$textP.": <span class='nom-curs'>".$this->obtenirTitol()->obtenirTextHTML()."</span></h2>
      <ul class='llistes'>".$edicions."</ul>";
   ...
   */
   public function modalCorreuValid() {
      ...
      $mostrar = $this->__modalWarning('modalCorreuValid', 'modal-warning', 'modalCorreuValidTitle',
   ...
   * i el corresponent identificador $idBody i com a text del footer $textFooter
   */
   private function __modalWarning( $id, $classModal, $idTitol, $textTitol, $idBody, $textBody, $textFooter) {
      if ( $textFooter == '' ) {
         $textFooter = "<button role='button' data-dismiss='modal' class='boto-blau
            border-0 border-radius-2 text-center negreta500 text-white m-0'>
            Tanca</button>";
      ...
      <div class='modal fade in' id='".$id."' tabindex='-1' role='dialog' aria-labelledby='".$id."' aria-hidden='true'>
         <div class='modal-dialog modal-dialog-centered modal-notify ".$classModal."' role='document'>
            <div class='modal-content w-100 border-0'>
               <div class='modal-header bg-warning border-0 text-white'>
                  <p class='modal-title m-0 text-center' id='".$idTitol."'>".$textTitol."</p>
                  <button role='button' class='close text-white' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>×</span></button>
               </div>
               <div class='modal-body' id='".$idBody."'>".$textBody."</div>
