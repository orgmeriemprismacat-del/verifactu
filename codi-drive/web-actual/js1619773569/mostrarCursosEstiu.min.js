...
var marginTop;
var fraseNomDescompteAmic = "<div id='frase-descompte-amic' class='col-12' style='margin-top: -10px; margin-bottom: 20px;'>";
fraseNomDescompteAmic += "<div style='text-align: center; background: var(--bg-accord); padding: 10px'>";
//fraseNomDescompteAmic += "<p class='mb-2'><strong>Oferta <em>flash</em>!</strong> Fins a l’1 de juliol (inclòs), gaudeix d’un <strong>15 % de descompte</strong> en inscriure’t en <strong>qualsevol</strong> dels nostres <strong>cursos de juliol</strong>.</p>";
fraseNomDescompteAmic += "<p class='mb-0'><a href='https://www.prisma.cat/descompte-amic' target='_blank'><strong>Porta un amic i gaudiu d'un descompte!</strong></a> Fins al 31 de juliol, oferim un <strong>25 % de descompte</strong> a tots aquells que us inscrigueu amb un amic en les <strong>edicions d'estiu (juliol i agost) dels cursos de 40, 60 i 100 hores</strong>.</p>";
fraseNomDescompteAmic += "</div>";
var fraseNomJuliol = "<div id='frase-nomenament-juliol' class='col-12' style='margin-top: -10px; margin-bottom: 20px;'>";
fraseNomJuliol += "<p><i class='fas fa-exclamation-circle ml-0'></i>Recordem que els <strong>cursos de juliol</strong> compten per obtenir el <strong>nomenament i cobrament de juliol</strong>. Si voleu conèixer els requisits que demana el Departament d’Educació, consulteu la pàgina d’<a href='https://www.prisma.cat/nomenaments-juliol'><strong>Informació dels nomenaments de juliol</strong></a>.</p>";
fraseNomJuliol += "</div>";

var fraseNomJuny = "<div id='frase-nomenament-juny' class='col-12' style='margin-top: -10px; margin-bottom: 20px;'>";
fraseNomJuny += "<p><i class='fas fa-exclamation-circle ml-0'></i>Recordem que els <strong>cursos de 100 hores de juny</strong> compten per obtenir el <strong>nomenament i cobrament de juliol</strong>. Si voleu conèixer els requisits que demana el Departament d’Educació, consulteu la pàgina d’<a href='https://www.prisma.cat/nomenaments-juliol'><strong>Informació dels nomenaments de juliol</strong></a>.";
// fraseNomJuny += "<div style='text-align: center; background: var(--bg-accord); padding: 10px; font-weight: bold'>";
// fraseNomJuny += "<p class='mb-0'>Oferim un descompte del 25 % en tots els cursos de 100 h de l'edició de juny (fins al 28 de maig).</p>";
// fraseNomJuny += "</div>";
fraseNomJuny += "</div>";
...
reqTitolFiltres.done(function( titol ) {
	...
	var reqCursos = $.ajax({
		async: !0,
		url: "https://www.prisma.cat/ajax/existexCursNou.php",
		method: "GET",
		data: {
	...
	reqCursos.done(function( cursos ) {
		if (cursos.indexOf('1') <= -1) {
			$('.novetats').addClass('d-none');
		}
		else {
			$('.novetats').removeClass('d-none');
		}
	});
});
...
function mostrarBlocCursos() {
	$('#frase-descompte-amic').remove();
	$('#frase-nomenament-juliol').remove();
	$('#frase-nomenament-juny').remove();
	if ( iniciPack ) {
		packs = 1; iniciPack = 0;
	}
	var reqBlocCursos = $.ajax({
		url: "https://www.prisma.cat/ajax/mostrar_cursos_estiu.php",
		method: "GET",
		data: {
			filtres : JSON.stringify(filtres),
	...
	reqBlocCursos.done(function( cursos ) {
		...
		// $('.cnt-titol-filtres').after(fraseNomDescompteAmic);
		//
		// if ( filtres[0][0].split('|')[1] == '07' ) {
		// 	$('.cnt-titol-filtres').after(fraseNomJuliol);
		// }
		// if ( filtres[0][0].split('|')[1] == '06' ) {
		// 	$('.cnt-titol-filtres').after(fraseNomJuny);
		// }
		amagarLoadingModal();
