...
function efectuarPagament() {
	var codiCurs = $("#codiCurs").html().trim(),
		 codiRegal = $("#codiRegal").html().trim(),
		 titol = $("#titol").html().trim(),
		 nomTit = $("#nom-titular").html().trim(),
		 dni = $("#dni").html().trim(),
		 preu = $("#import").html().trim();

	if (codiCurs!='' && titol!='' && dni!='' && nomTit!='' && preu!='') {
		$("#form_enviar_dades").click(function(e) {
			$.ajax({
				url: "https://www.prisma.cat/ajax/efectuarPagamentRegalAutomaticProva.php?codiCurs="+codiCurs+"&codiRegal="+codiRegal+"&titol="+titol+"&dni="+dni+"&nomTit="+nomTit+"&import="+preu,
				cache: false,
				type: "GET",
				success: function(pagina) {
					$('#frm').submit();
		...
		$("#form_cancelar_dades").click(function(e) {
			...
			pagina += "<div class='d-flex flex-column border-0 align-items-center w-100 mb-4'>";
		...
		$.ajax({
			url: "https://www.prisma.cat/ajax/mostrarErrorEfectuarPagamentRegal.php",
			cache: false,
			type: "GET",
			success: function(pagina) {
				$('#cnt-pagament').html(pagina);
