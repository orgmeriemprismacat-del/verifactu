	...
	$.ajax({
		...
		success: function(pagina) {
			...
			$('.form-footer').on('blur', '.form-control', function() {
				if ($('#adreca-electronica').val().trim() == '')
			...
			function validacioCorreuButlleti() {
				var email = $('#adreca-electronica').val().trim();
				var error = "";
				if (email.length != 0) {
			...
			$('.form-footer').on('click', '#news', function() {
				if (validacioCorreuButlleti() != '') {
					...
					var correu = $('#adreca-electronica').val().trim();
					var comprovaSpam = $("#butlletiSpam").val().trim();
					$.ajax({
...
var vectUrlPagina = urlPagina.split('/');
urlPagina = "/" + vectUrlPagina[1] + "/" + vectUrlPagina[2];
var keyEncr = '';
for (var i=3; i<vectUrlPagina.length; i++) {
	keyEncr += vectUrlPagina[i];
	if (i<vectUrlPagina.length-1) keyEncr+='/';
}

function mostrarPaginaPagament() {
	$.ajax({
		url: "https://www.prisma.cat/ajax/mostrar_pagina_pagament_regal_automatic.php?keyEncr=" + keyEncr,
		cache: false,
		type: "GET",
		success: function(pagina) {
			$("#cnt-pagament-regal").html(pagina);
			...
			$('.form-dades').on('change', '#nom-titular', function() {
				validarNomTitular()
			});
			$('.form-dades').on('blur', '#nom-titular', function() {
				validarNomTitular()
			});
			$('.form-dades').on('focus', '#nom-titular', function() {
				eliminarError('nom_cognom_titular_erroni')
			});
			...
			$(".form-dades #form_enviar_dades").click(function(e) {
				var comprovacio = '',
					validDoc = '',
					validNomTitular = validarNomTitular(),
					validImport = validarImport();

				if ($("#doc .element-selected").html().trim().indexOf("NIF/NIE") >= 0)
					validDoc = validarNif();
				else
					validDoc = validarPass();

				if (validNomTitular.length != 0) comprovacio += "<li>Nom i cognoms del titular de la targeta</li>";
				if ($("#doc .element-selected").html().trim().indexOf("NIF/NIE") >= 0) {
					if (validDoc.length != 0) comprovacio += "<li>Dni amb lletra</li>";
				} else {
					if (validDoc.length != 0) comprovacio += "<li>Número de passaport</li>";
				}
				if (validImport.length != 0) comprovacio += "<li>Import</li>";
...
mostrarPaginaPagament();

var documentacio = '';

function validarNomTitular() {
	var error = validacioCampBuit("nom-titular", "nom_cognom_titular_erroni");
	return error;
}
...
function mostrarDocumentacio(id) {
	...
	input += "<span class='camp'>" + nomCamp + "</span><span class='req'>*</span>";
	input += "</label>";
	input += "<input type='text' class='form-control' id='" + idInput + "' name='" + idInput + "'>";
	input += "<span id='" + idError + "' class='d-flex justify-content-center ";
	input += "align-items-center px-2 position-absolute text-centrat text-white'></span>";
	input += "</div>";
	$('#input_doc').html(input);

	$('.form-dades').on('change', '#' + idInput, function() {
		validarNif();
		documentacio = $('#dni').val().trim();
	});
...
function validarNif() {
	var nif = $('#dni').val().trim();
	var error = "";

	var error = validacioCampBuit("dni", "dni_erroni");;
...
function validarPass() {
	return validacioCampBuit("dni", "dni_erroni");
}

function validarImport() {
	var error = validacioCampBuit("import", "import_erroni");
	if (error=='') {
		if ( parseFloat($('#validarImport').val()) <= 0) {
			error = "L'import ha der més gran que 0";
			mostrarError("import_erroni", error);
		}
		else
			eliminarError("dni_erroni");
	}
...
function afegirCSS() {
	$('body').append("<link rel='stylesheet' href='https://www.prisma.cat/css1619773569/pagamentRegal.min.css' />");
	$('body').append("<link rel='stylesheet' href='https://fonts.googleapis.com/css?family=Roboto:100,100i,300,400,500,700|Nunito+Sans&display=swap' />");
	$('body').append("<link rel='stylesheet' href='https://www.prisma.cat/css1606382329/font-awesome-prisma.min.css' />");
	$('body').append("<link rel='shortcut icon' type='image/x-icon' href='https://www.prisma.cat/favicon.ico'/>");
	$('body').append("<link rel='stylesheet' href='https://www.prisma.cat/css1606382329/404.min.css' />")
}
afegirCSS();
...
function openNav() {
	...
	$('.navBarPrisma').addClass('width-100');
...
function closeNav() {
	...
	$('.closebtn').css('display', 'none');
	$('.navBarPrisma').removeClass('width-100');
