...
urlPagina = "/" + vectUrlPagina[2]  + "/" + vectUrlPagina[3]+ "/" + vectUrlPagina[4];
var keyEncr = '';
for (var i=4; i<vectUrlPagina.length; i++) {
...
function mostrarConfirmacioInscripcio() {
	$.ajax({
		url: path + "ajax/mostrar_confirmacio_inscripcio_tastet_automatic.php?keyEncr=" + keyEncr,
		cache: false,
		type: "GET",
		success: function(confirmacio) {
			$("#cnt-inscripcio").html(confirmacio);

			var heightFooter = '418px';
			var heightHeader = '102px';
			var heightFooter = parseInt( $('.prisma-footer').css('paddingBottom') ) + parseInt( $('.prisma-footer').css('paddingTop') ) + $('.prisma-footer').height();
			var heightHeader = parseInt( $('#nav-header').css('paddingBottom') ) + parseInt( $('#nav-header').css('paddingTop') ) + $('#nav-header').height();

			var separacio = heightFooter + heightHeader;
			separacio = separacio + parseInt( $('#cnt-inscripcio').css('marginBottom') );

			$("#cnt-inscripcio").css('min-height', 'calc( 100% - ' + separacio + 'px)');
			...
			$('.form-dades').on('change', '#importPagare', function() {
				validarImport()
			});
			$('.form-dades').on('blur', '#importPagare', function() {
				validarImport()
			});
			$('.form-dades').on('focus', '#importPagare', function() {
...
function mostrarDocumentacio(id) {
	...
	input += "align-items-center px-2 position-absolute text-center text-white'></span></div>";
...
function validarImport() {
	...
	if (error=='') {
		if ( parseFloat($('#importPagare').val()) <= 0 && $('#frac').val()=='1') {
