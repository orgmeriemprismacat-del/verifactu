...
function mostrarInscripcio() {
	var requestPage  = $.ajax({
		async: !0,
		url: path + "ajax/mostrar_inscripcio.php",
		method: "GET",
		data: {
			url : urlPagina,
			tipus : tipus,
	...
	requestPage.done(function( cursos ) {
		...
		$('.form-dades').on('click', ':checkbox', function() {
			...
			if (
				...
				$(this).attr('id') == 'victima_violencia'
			)
			{
				checkCarnet = 0; checkUSOC = 0; checkDiscapacitat = 0; checkDiscapacitat = 0; checkFamMono = 0; checkFamNum = 0; checkVictViolencia = 0;
				...
				if ( this.checked ) {
					...
					else if ($(this).attr('id') == 'victima_violencia') {
						checkVictViolencia = 1;
						$('#cnt_input_carnet').removeClass('d-none');
					}
				}
			...
			else if ( $(this).attr('id') == 'recent_titulat' ) {
				$('#recent_titulat').attr('disabled', false);
				$('#cnt_input_resguard').addClass('d-none');
				if ( this.checked ) {
					$('#recent_titulat').attr('disabled', true);
					$('#recent_titulat').next().addClass('desabilitat');
					$(this).attr('disabled', false);
					$(this).next().removeClass('desabilitat');
					if ($(this).attr('id') == 'recent_titulat') {
						checkNovell = 1;
						$('#cnt_input_resguard').removeClass('d-none');
					}
				}
			}
			else {
		...
		$(".form-dades #form_enviar_dades").click(function(e) {
			var comprovacio = '',
				...
				validFileResguard = validarFileResguard();
			...
			if (validFileResguard.length != 0) comprovacio += "<li>Fitxer resguard/titol</li>";

			if (comprovacio != "") {
				var missatgeError = "<p>Els camps següents són incorrectes:</p><ul class='errors'>" + comprovacio + "</ul>";
				if ( preuInscripcio <= 0 && tipusCurs == 'N' ) missatgeError += "<p>Hi ha hagut un error al calcular el preu. Torna a carregar la pàgina per solucionar l'error.</p>";
...
function mostrarPreu() {
	if ( tipusCurs == 'S' ) {
		$('#preu').html("<p>Preu: <span class='preuTatxat'>" + preuCurs + " euros</span> <span class='preu'>0 euros</span> <span class='textMsgPreu'>(subvencionat per a docents de centres públics i de centres privats sostinguts amb fons públics, en el marc del Pla de Recuperació, Transformació i Resiliència (PRTR), finançat per la Unió Europea - Next Generation EU)</span></p>");
		preuInscripcio = 0;
	}
	else
...
function mostrarPerfil(id) {
	var dataExp = "<p class='mb-3'>Els nostres cursos compten com a formació permanent del professorat sempre que es realitzin posteriorment a la data d’expedició del títol d’accés a la docència (Magisteri o CAP / Màster en Educació Secundària).</p>";
	$('#perfils-altres').html('');
...
function aplicarPreuCodiPromocions(dadesPromo) {
	...
	var tipusPromo = dadesPromo.split('|')[7];
	var promoDesc = dadesPromo.split('|')[6];
	var promoPreu = dadesPromo.split('|')[8];
	var promoPreuFix = dadesPromo.split('|')[9];
	var acum = dadesPromo.split('|')[10];
	...
	if ( tipusPromo == 0 ) {
		msgModal += "Se t'ha aplicat el " + promoDesc + "% de descompte";
		preuInscripcio = parseFloat( preuCurs ) - ( (parseFloat( preuCurs ) * parseFloat( promoDesc )) / 100 );
		promocioAplicada = promo + '|' + promoDesc + '|' + tipusPromo;
	}
	else if ( tipusPromo == 1 ){
		msgModal += "S'ha descomptat " + promoPreu + "€";
		preuInscripcio = parseFloat( preuCurs ) - promoPreu;
		promocioAplicada = promo + '|' + promoPreu + '|' + tipusPromo;
	}
	else {
		preuInscripcio = parseFloat( promoPreuFix );
		promocioAplicada = promo + '|' + promoPreuFix + '|' + tipusPromo;
	}

	if ( tipusPromo == 0 || tipusPromo == 1)
		msgModal += " en el preu del curs corresponent al codi promocional " + promo + ".</p>";
	else
		msgModal += "Se t'ha aplicat correctament el codi promocional " + promo + ".</p>";
	...
	)
	{
		...
		*/
		...
		var searchPrice  = $.ajax({
			async: !0,
			url: path + "ajax/calcularPreu.php",
			method: "GET",
			data: {
				codi : codiCurs,
				hores : hores,
				...
				checkVictViolencia : checkVictViolencia
			},
			dataType: "html"
		});
...
function validarFileCarnet() {
	var error = '';
	if ( checkDiscapacitat == 1 || checkFamNum == 1 || checkFamMono == 1 || checkVictViolencia == 1 ) {
		if ( !$('#carnet')[0].files[0] ) {
...
function validarFileResguard() {
	var error = '';
	if ( $('#recent_titulat').is(":checked") ) {
		if ( !$('#resguard')[0].files[0] ) {
			error = "Has d'adjuntar un fitxer";
			mostrarError("resguard_erroni", error);
		}
		else
			eliminarError("resguard_erroni");
	}
	else
		eliminarError("resguard_erroni");
	return error;
...
function enviarInscripcio() {
	...
	requestInscrDupl.done(function( msgInscDupl ) {
		if ( !msgInscDupl.toLowerCase().includes("error") && msgInscDupl == '' ) {
			var sendInscr = $.ajax({
				url: path + "ajax/enviarInscripcio.php",
				data: {
					tipusCurs: tipusCurs,
					nom: nom,
					cog: cog,
			...
			sendInscr.done(function( msg ) {
				if ( !msg.toLowerCase().includes("error") && msg != '' ) {
					...
					if ( tipusCurs != 'S' ) {
						...
						$.ajax({
							url: 'https://www.prisma.cat/ajax/enviarImatgeCarnetInscripcio.php',
							type: 'post',
							data: formData,
							contentType: false,
							processData: false,
							success: function(response) {
								...
								if ( socRecentTitulat == 'yes' ) {
									var formData = new FormData();
									var files = $('#resguard')[0].files[0];
									formData.append('file',files);
									formData.append('codiCurs',codiCurs);
									formData.append('edicio',edicio);
									formData.append('any',any);
									formData.append('documentacio',documentacio);
									$.ajax({
										url: 'https://www.prisma.cat/ajax/enviarImatgeSocRecentTitulat.php',
										type: 'post',
										data: formData,
										contentType: false,
										processData: false,
