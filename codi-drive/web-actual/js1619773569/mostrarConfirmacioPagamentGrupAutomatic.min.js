...
function mostrarPaginaPagament() {
	$.ajax({
		url: path + "ajax/mostrar_pagina_confirmacio_pagament_grup_automatic.php?keyEncr=" + keyEncr,
		cache: false,
		type: "GET",
		success: function(pagina) {
			$("#cnt-pagament-grup").html(pagina);
...
function validarNif() {
	...
	if (error.length == 0) {
		var dni_inscrit = $('#dni').val().toUpperCase();
...
function validarImport() {
	...
	error = validacioCampBuit("importPagare", "import_erroni");
	var pendent = parseFloat($('#import').val());
	if (error=='') {
		if ( parseFloat($('#importPagare').val()) <= 0 && $('#frac').val()=='1') {
			error = "L'import ha der més gran que 0";
			mostrarError("import_erroni", error);
		}
		else if ( parseFloat($('#importPagare').val()) > pendent && $('#frac').val()=='1') {
			error = "L'import a pagar no pot ser més gran que l'import pendent";
			mostrarError("import_erroni", error);
		}
		else
			eliminarError("dni_erroni");
