...
var keyEncr = '';
for (var i=2; i<vectUrlPagina.length; i++) {
...
function mostrarPaginaPagament() {
	$.ajax({
		url: path + "ajax/mostrar_pagina_pagament.php?keyEncr=" + keyEncr,
		cache: false,
		type: "GET",
		success: function(pagina) {
			$("#cnt-pagament-inscripcio").html(pagina);
			...
			$('.form-dades').on('change', '#nom-alumne', function() {
				validarNomAlumne()
			});
			$('.form-dades').on('blur', '#nom-alumne', function() {
				validarNomAlumne()
			});
			$('.form-dades').on('focus', '#nom-alumne', function() {
				eliminarError('nom_cognom_alumne_erroni')
			});
			...
			$('.form-dades').on('change', '#importPagat', function() {
				validarImport()
			});
			$('.form-dades').on('blur', '#importPagat', function() {
				validarImport()
			});
			$('.form-dades').on('focus', '#importPagat', function() {
				eliminarError('import_erroni')
			});
			...
			$(".form-dades #form_enviar_dades").click(function(e) {
				var comprovacio = '',
					...
					validNomAlumne = validarNomAlumne(),
				...
				if (validNomAlumne.length != 0) comprovacio += "<li>Nom i cognoms de l'alumne</li>";
				if ($("#doc .element-selected").html().trim().indexOf("NIF/NIE") >= 0) {
...
function validarNomAlumne() {
	var error = validacioCampBuit("nom-alumne", "nom_cognom_alumne_erroni");
	return error;
}
...
function mostrarDocumentacio(id) {
	...
	var input = "<div class='form-group field-wrap posicio-relativa'>";
	...
	input += "<span id='" + idError + "'></span></div>";
...
function validarNif() {
	console.log("comprovar nif " + $('#dni').val().trim());
	...
	if (error.length == 0) {
		var dni_inscrit = $('#dniInscrit').val().toUpperCase();
		if (nif.toUpperCase() != dni_inscrit) {
			error = "El número no coincideix amb el de la inscripció";
			mostrarError("dni_erroni", error);
		}
		else {
			eliminarError("dni_erroni");
...
function validarImport() {
	...
	error = validacioCampBuit("importPagat", "import_erroni");
	if (error=='') {
		if ( parseFloat($('#importPagat').val()) <= 0 && $('#frac').val()=='1') {
...
function mostrarError(id_error, error) {
	$("#" + id_error).html(error);
	$("#" + id_error).addClass("erroni text-centrat color-white");
}
...
function afegirCSS() {
	$('body').append("<link rel='stylesheet' href='" + path + "css1619773569/pagamentInscripcio.min.css' />");
	...
	$('body').append("<link rel='stylesheet' href='" + path + "css1619773569/font-awesome-prisma.min.css' />");
	$('body').append("<link rel='shortcut icon' type='image/x-icon' href='https://www.prisma.cat/favicon.ico'/>");
	$('body').append("<link rel='stylesheet' href='" + path + "css1619773569/404.min.css' />")
}
afegirCSS();
