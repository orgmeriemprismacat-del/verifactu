...
function mostrarTotsPerfils() {
	var reqPage = $.ajax({
		url: domain + "ajax/mostrar_pagina_perfils.php",
		method: "GET",
		data: {
	...
	reqPage.done(function( perfils ) {
		$("#contingut_perfils").html(perfils);
		mostrarDataActualitzacio();
		mostrarCursosPerfils();
		mostrarPeguntesFrequentsPerfils();
	});
...
function mostrarTaset(codi) {
	if (!$('#veure_tastet_'+codi)[0]) {
		var reqTastet = $.ajax({
			url: domain + "ajax/mostrar_tastet.php",
			method: "GET",
			data: {
				dispositiu : dispositiu,
				codi : codi
		...
		reqTastet.done(function( tastet ) {
			$("#contingut_perfils").append(tastet);
...
function mostrarDataActualitzacio() {
	var req = $.ajax({
		url: domain + "ajax/mostrar_data_actualitzacio_pagina.php",
		method: "GET",
		data: {
			url : urlPagina,
			dispositiu : dispositiu
	...
	req.done(function( date ) {
		$("#modificacio").html(date);
	});

	req.fail(function( jqXHR, textStatus, errorThrown ) {
		errorFunction( jqXHR, textStatus, errorThrown, "Hi ha hagut un error en el request de la data de modificació: " );
	});
}

function mostrarCursosPerfils() {
		var req = $.ajax({
			url: domain + "ajax/mostrar_tots_perfils.php",
			method: "GET",
			data: {
				dispositiu : dispositiu
			},
		...
		req.done(function( perfils ) {
			$("#perfils").html(perfils);
		});
...
function mostrarPeguntesFrequentsPerfils() {
	var req = $.ajax({
		url: domain + "ajax/mostrar_preguntes_frequents_perfils.php",
		method: "GET",
		data: {
			dispositiu : dispositiu
		},
	...
	req.done(function( accordion ) {
		$("#accord").html(accordion);
		mostrarPerfilsCursosAnteriors();
	});

	req.fail(function( jqXHR, textStatus, errorThrown ) {
		errorFunction( jqXHR, textStatus, errorThrown, "Hi ha hagut un error en el request de les preguntes freqüents: " );
	});
}

function mostrarPerfilsCursosAnteriors() {
	var req = $.ajax({
		url: domain + "ajax/mostrar_cursos_perfil_anteriors.php",
		method: "GET",
		data: {
			url : urlPagina,
			dispositiu : dispositiu
	...
	req.done(function( cursos ) {
		$("#cursos-perfil-anteriors").html(cursos);
	});

	req.fail(function( jqXHR, textStatus, errorThrown ) {
		errorFunction( jqXHR, textStatus, errorThrown, "Hi ha hagut un error en el request dels perfils anteriors: " );
	});
}
