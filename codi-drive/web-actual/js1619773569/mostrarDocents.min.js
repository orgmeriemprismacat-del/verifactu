	...
	$.ajax({
		...
		success: function(pagina) {
			...
			$('.prisma-header').on('focus', '.form-control', function(){
				$('.prisma-header label').addClass('active');
			});
			$('.prisma-header').on('blur', '.form-control', function(){
				if ($(this).val()=='')
					$('.prisma-header label').removeClass('active');
			});
		}
	});
	$.ajax({
...
function mostrarTotsDocents() {
	$.ajax({
		url: "https://www.prisma.cat/ajax/mostrar_tots_docents.php",
		cache: !1,
		type: "GET",
		success: function(docents) {
			$("#llistat-docents").html(docents)
		}
	})
}
mostrarTotsDocents();
