...
function mostrarInscripcio() {
	var requestPage  = $.ajax({
		async: !0,
		url: path + "ajax/mostrar_inscripcio_taller.php",
		method: "GET",
		data: {
			url : urlPagina,
...
function buscarIdTaller() {
	var request = $.ajax({
		url: path + "ajax/obtenirIdTaller.php",
		data: { url: urlPagina },
		method: "GET",
		dataType: "html"
	});
...
function buscarIdPreu() {
	var request = $.ajax({
		url: path + "ajax/obtenirIdPreuTaller.php",
		data: {
			codiCurs: codiCurs
		},
		method: "GET",
...
function buscarPreus() {
	var request = $.ajax({
		url: path + "ajax/obtenirPreuTaller.php",
		data: {
			idPreu: idPreu
		},
		method: "GET",
...
function mostrarPreu() {
	...
	$('#preu').html("<p>Preu: " + textPreuCurs + " </p>");
}
...
// 			dataType: "html"
// 		});
// 		request.done(function( esCorreuValid ) {
...
function enviarInscripcio() {
	...
	var sendInscr = $.ajax({
		url: path + "ajax/enviarInscripcioTaller.php",
		data: {
			nom: nom,
			cog: cog,
			dni: documentacio,
	...
	sendInscr.done(function( msg ) {
		if ( !msg.toLowerCase().includes("error") && msg != '' ) {
			...
			var urlConf = "https://www.prisma.cat/tallers/confirmacio/";
			urlConf += msg.trim();
