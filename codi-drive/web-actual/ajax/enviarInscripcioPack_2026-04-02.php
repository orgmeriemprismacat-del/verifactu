...
try {
	...
	// 	ANY, MES, c.CURS, p.ID_CURS, HORES, i.ID, i.ID_AMIGABLE, i.TITOL, c.ESTAT
	...
	$cnsInfoOrig = "SELECT DATAI, DATAF, CURS_ESCOLAR, FISS, GTAF, DATA_RESOL, ANY, MES,
		...
		WHERE ip.ID_PACK = 10034 AND p.PUBLIC=1 AND c.PUBLIC=1 AND c.CURS!='PROVA' AND
   ...
   if ( $stmt=$connexio->prepare($cnsInsc) ) {
	...
	$missatge .= "ja t'hem inscrit en el <em>pack</em> <strong>".$titolPack."</strong> ";

	$missatge .= "que inclou els cursos <strong>".$titols[0]." (".$datesRealitzacioCurs1.")</strong> i <strong>".$titols[1]." (".$datesRealitzacioCurs2.")</strong> amb les dades personals següents:</p>";

	//$missatge .= "que inclou els cursos <strong>".$datesRealitzacioCursos."</strong> amb les dades personals següents:</p>";
	...
	$msgInsc .= "<p><strong>Dates:</strong> ".$datesRealitzacioCursos."</p>";
