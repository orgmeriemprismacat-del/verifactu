...
try {
   ...
   if ( $pagina == 1 && $paginaAnt == 1 ) {
      ...
      $_SESSION['llistatCursos'] = serialize($objLlistatCursos);
   }
   else {
      $objLlistatCursos = unserialize($_SESSION['llistatCursos']);
      $_SESSION['llistatCursos'] = serialize($objLlistatCursos);
   }

	$list .= "<div class='sticky-body'><div class='theiaStickySidebar w-100'><div class='row'>";
   $list .= $objLlistatCursos->mostrarLlistatCursosFiltres($inicPagina,$elementsPerPagina);
   $list .= $mostrant;
   $list .= $pagination;
