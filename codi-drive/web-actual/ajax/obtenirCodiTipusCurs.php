...
try {
	...
	$connexio->consultarBD("SELECT CODI_CURS, TIPUS_CURS FROM informacio WHERE ID_AMIGABLE=".$id_url_consulta." AND ESTAT = 1");
	$row = $connexio->obtenirResultat();
	$codi_curs = $row['CODI_CURS'];
	$tipus_curs = $row['TIPUS_CURS'];

	$connexio->lliurarConsulta();
	$connexio->desconectarBD();

	echo $codi_curs."|".$tipus_curs;
}
catch(Exception $e) {
