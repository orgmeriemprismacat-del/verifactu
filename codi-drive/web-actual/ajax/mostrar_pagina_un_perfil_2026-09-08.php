...
function mostrarAccordionsCursosPerfils($id, $accordion, $obert, $idPerf,$codiCurs,$codiEsc) {
	...
	$cnsPerfils="SELECT NOM_CURS, c.HORES, MES, ANY, CODI_GTAF
		FROM perfils AS p INNER JOIN curs AS c ON c.CURS=p.CODI AND
		c.HORES=p.HORES AND c.CURS_ESCOLAR=p.CURS_ESCOLAR AND p.CODI_GTAF=c.GTAF
		WHERE CODI=? AND ID_PERFIL=? AND p.CURS_ESCOLAR=? AND c.PUBLIC = 1 AND c.ESTAT != '0' AND c.ESTAT != 'P' ORDER BY ANY DESC, MES DESC";
