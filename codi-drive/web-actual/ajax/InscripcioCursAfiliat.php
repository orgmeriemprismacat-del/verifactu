...
class InscripcioCurs {
   ...
   */
   public function mostrar() {
     ...
     //  <h2 class='pt-3 mt-0'>Dates: <span class='nom-curs'>".$textEdicio."</span></h2>";
     //
     //  if ( !$this->placesExharuides() && !$this->edicioTancades() ) {
   ...
   */
   private function __buscarDatesEdicio() {
      ...
      // $mes = $this->edicio;
      //
      // $cnsEd = "SELECT DATAI, DATAF, ANY, MES, c.ESTAT FROM curs AS c
      //    WHERE c.CURS=? AND PUBLIC=1
      //    AND c.CURS!='PROVA' AND c.CURS NOT LIKE '%0%' AND c.ESTAT!='0' AND
      //    ANY = ? AND MES = ?";
