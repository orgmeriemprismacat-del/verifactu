...
try {
   ...
   $connexio->closeStmt();
	...
	if ( $promocioAplicada != '' ) {
		$textAlumne = ' (per haver utilitzat el codi de descompte '.explode('|', $promocioAplicada)[0].")";
	}
	...
	$missatge .= "ja t'hem inscrit en el curs en línia <strong>".$titolCurs."</strong> ";
	...
	if ( $promocioAplicada != '' )  {
		$msgInsc .= "<p><strong>Codi descompte del ".explode('|', $promocioAplicada)[1]."% per una trobada:</strong> ".explode('|', $promocioAplicada)[0]."</p>";
	}
	...
	$mailAlumne = new MailSMTPComvive("suport.informatic@prisma.cat", "Girona05", "suport informatic", "suport.informatic@prisma.cat",
										"suport informatic", "suport.informatic@prisma.cat", $nomTo, $correuTo,
										$subject, $missatge);
