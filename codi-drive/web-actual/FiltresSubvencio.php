...
class Filtres {
   ...
   */
   public function __construct() {
      ...
      $cnsHores="SELECT HORES FROM curs as c WHERE GTAF IS NOT NULL AND GTAF!='' AND
         (
           (DATAI + 7 > CURRENT_DATE AND (c.HORES=30 OR c.HORES = 40 OR c.HORES = 60)) OR
           (DATAI + 7 > CURRENT_DATE AND c.HORES=50) OR
           (DATAI + 7 > CURRENT_DATE AND c.HORES=70) OR
           (DATAI + 14 > CURRENT_DATE AND c.HORES=100)
         )
      ...
      $cnsTemes='SELECT ID FROM temes WHERE ESTAT = 1';
      ...
      $cnsEd = "SELECT MES, ANY FROM curs AS c INNER JOIN aula AS a ON
         ...
         AND ORDRE_TUTOR=1)) GROUP BY ANY, MES LIMIT ?";
   ...
   */
   public function mostrarFiltresLaterals() {
      ...
      $nousBotons = "";
      /* Existeixen pcks */
      /* $nousBotons = "<div class='nou packs boto-blau text-white ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txt."</div>"; */
      $nousBotons .= "<div class='nou subvencio boto-header text-white ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txtSubv."</div>";
      $nousBotons .= "<div class='nou cdd boto-header text-white ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txtCDD."</div>";
       ...
       <div class='nou novetats boto-blau ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0 ml-2'>NOVETATS!</div>
       ".$nousBotons."
       </div>";
   ...
   */
   public function mostrarFiltresSuperiors() {
      ...
      $nousBotons = "";
      /* Existeixen pcks */
      /* $nousBotons .= "<button href='#' class='nou packs boto-blau text-white ml-2 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txt."</button>"; */
      $nousBotons .= "<button href='#' class='nou subvencio boto-header text-white ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txtSubv."</button>";
      $nousBotons .= "<button href='#' class='nou cdd boto-header text-white ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txtCDD."</button>";

      $mostrar.="<div class='filtres-superior d-flex text-right'>
      <button href='#' class='nou novetats boto-blau ml-2 mr-2 mt-0 mb-0 w-auto px-2 border-0'>NOVETATS!</button>
      ".$nousBotons."
   ...
   */
   public function mostrarFiltresSuperiorsFixed() {
      ...
      //      <button class='show-filters border-0'><i class='fas fa-bars'></i></button></div>
      //      <div class='nou novetats boto-blau ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0 ml-2'>NOVETATS!</div>
      //      <div class='nou packs boto-blau text-white ml-0 mr-2 mt-0 mb-0 w-auto px-2 border-0'>".$txt."</div>
      //   </div>
      //</div>";
      $mostrar.="<div class='filter-capcalera filter-fixed'>
         <div class='filter-capcalera'><div class='cnt-filter-button flex-1-0-auto'>
