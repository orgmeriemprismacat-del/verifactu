<?php
	...
	try {
		...
		if (intval($codiResposta)>=0 && intval($codiResposta)<=99) {
			...
			FROM inscripcions WHERE IDPAG=? AND (`INSC CURS`='0' OR `INSC CURS`='1' OR `INSC CURS`='M') GROUP BY IDPAG";
      ...
      if ( $stmtInscAlumne = $connexio2->prepare($cnsInscripcionsPack) ) {
		   ...
		   if ( $diaDataF == "1" or $diaDataF == "11" ) $dataFPref = "l'".$dataFL;
			...
			WHERE TIPUS_INSC = 'P' AND (`INSC CURS` = '0' OR `INSC CURS` = '1' OR `INSC CURS`='M') AND
			...
			$msg = $templates->getTemplate_Dades_RequadreDadesComanda($frac, $pendentPagar);
			$names_template = array("[NU_COMANDA]", "[DATETIME_COMANDA]", "[CODE_RES]",
				"[CONCEPTE]", "[TITOL]", "[EMAIL]",  "[TYPE]", "[URL_PAY]",
				"[IMPORT]", "[PENDENT_PAY]", "[PAY_ORIG]");
			$names_function   = array($ordre, $dataPagNewFormat, "Acceptat",
				$dniTitularPag." | ".$titol, $titol, $email, "pack", $urlPagament,
			...
			if ( $inscrit == 'M' && $pendentPagar <= 0 ) {
				...
				<div style='background-color: #e8ecf5; border: 1px solid #D7DEEE; border-radius: 2px; padding: 5px 25px; margin-bottom: 20px'>
					...
					<p><strong>Concepte:</strong> ".$dniTitularPag." | ".$titol."</p>
					<p><strong>Dates:</strong> ".$datesRealitzacioCursos."</p>
      ...
      if ( $stmtInscAlumne = $connexio2->prepare($cnsInscripcionsPack) ) {
		   ...
		   if ( $diaDataF == "1" or $diaDataF == "11" ) $dataFPref = "l'".$dataFL;
			...
			$names_function   = array($urlPagament, $titol, $idPag, "pack");
