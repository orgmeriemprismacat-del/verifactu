...
class PagamentRegal {
   ...
   public function __construct($idRegal) {
      ...
      $cnsInsc = "SELECT IMPORT, NOM_CURS, MAILC, CODI, CCURS FROM regal WHERE ID=?";
		$stmt=$connexio->prepare($cnsInsc);
		$stmt->bind_param("d", $idRegal);
		$stmt->execute();
		$stmt->store_result();
		if ( $stmt->num_rows() == 1 ) {
			$stmt->bind_result($import, $nomCurs, $correuCurs, $codiRegal, $codiCurs);
			$stmt->fetch();
   ...
   private function __mostrarPagamentTargeta($tipus) {
      ...
      $mostrar .= "<div class='form-group field-wrap position-relative'>
                     ...
                     <span id='nom_cognom_titular_erroni'
                        ...
                        position-absolute text-centrat text-white'></span>
                  </div>";
      $mostrar .= "</div>";
      $mostrar .= "</div>";
      ...
      $mostrar .= "<div class='form-group field-wrap position-relative'>
                     <label class='position-absolute mb-0'>
                        <span class='camp'>Nom i cognoms de l'alumne/a</span>
                        <span class='req'>*</span>
                     </label>
                     <input type='text' class='form-control' id='nom-alumne' name='nom-alumne'>
                     <span id='nom_cognom_alumne_erroni'
                        class='d-flex justify-content-center align-items-center px-2
                        position-absolute text-centrat text-white'></span>
                  </div>";
      $mostrar .= "</div>";
         ...
         <div class='col-12 col-md-6 pl-0 pr-0 pr-md-2'>
            <div class='form-group field-wrap position-relative'>
               <div id='doc' class='select d-flex flex-column justify-content-center w-100 position-relative m-0'>
                  <span class='element-selected font-weight-normal w-100'>NIF/NIE de l'alumne/a</span>
                  <ul class='select-list position-absolute ' style='display: none;'>
                     <li class='border-bottom m-0' id='doc-dni'><a href='#'>NIF/NIE de l'alumne/a</a></li>
                     <li class='border-bottom m-0' id='doc-passaport'><a href='#'>Altres de l'alumne/a</a></li>
                  </ul>
         ...
         <div class='col-12 col-md-6 pl-0 pr-0 pr-md-2' id='input_doc'>
            <div class='form-group field-wrap position-relative'>
               ...
               <span id='dni_erroni' class='d-flex justify-content-center
                      align-items-center px-2 position-absolute text-centrat
                      text-white'></span>
             </div>
          </div>
       </div>";
