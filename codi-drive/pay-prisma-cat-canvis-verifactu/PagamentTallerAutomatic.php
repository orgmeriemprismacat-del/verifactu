...
class PagamentTallerAutomatic {
   ...
   public function __construct($idPag) {
      ...
      $cnsInsc = "SELECT ID, ANY, MES, CURS, A_PAGAR, PAGAMENT, FRACCIONAT, CORREU, DNI, IDPAG, TIPUS_DESC, VALID_DESC
      FROM inscripcions WHERE IDPAG=? AND (`INSC CURS`='0' OR `INSC CURS`='1' OR `INSC CURS`='M')";
		...
		if ( $stmt->num_rows() == 1 ) {
         ...
         if ($this->anyInsc!=null and $this->mesInsc!=null and $this->cursInsc!=null) {
            $cnsEd = "SELECT TITOL, HORES, DATAI FROM info_taller INNER JOIN informacio ON info_taller.CODI_CURS = informacio.CODI_CURS
            WHERE ANY=? AND MES=? AND informacio.CODI_CURS=?";
      		$stmt=$connexio->prepare($cnsEd);
      		$stmt->bind_param("dss", $anyInsc, $mesInsc, $cursInsc);
      		$stmt->execute();
            $stmt->bind_result($nomCurs, $hores, $datai);
   			$stmt->fetch();
   ...
   */
   public function mostrar() {
      ...
      $mostrar .= "<p class='dades pt-3'>Taller: <span class='dada titol'>".$titol."</span></p>";
      $mostrar .= "<p class='dades'>Data: <span class='dada'>".$dates."</span></p>";
   ...
   */
   private function __mostrarDatesEdicio() {
      ...
      $mostrar .= $textDataI->convertirDataLlarga();;
      return $mostrar;
   }
   ...
   */
   private function __mostrarPagamentTargeta($tipus) {
      ...
      $mostrar .= "<form id='frm' name='frm' action='https://www.prisma.cat/tallers/efectuarPagament/' method='post'>";
