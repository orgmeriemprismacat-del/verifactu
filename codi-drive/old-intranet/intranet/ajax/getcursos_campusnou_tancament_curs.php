...
if ($hores=="Tots")
	$result_curs = mysqli_query ($connexio, "SELECT id_Curs, ANY, MES, CURS, AULA, `NOM CURS` AS nom_curs, codi_udg, `DATA FIN` as datafi, Data_llarga_Inici FROM cursos WHERE ANY=".$any." AND MES='".$mes."' AND DNI_TUTOR <> '0' AND CURS NOT LIKE '%0%' ORDER BY CURS, AULA");
else if ($hores=="40")
	$result_curs = mysqli_query ($connexio, "SELECT id_Curs, ANY, MES, CURS, AULA, `NOM CURS` AS nom_curs, codi_udg, `DATA FIN` as datafi, Data_llarga_Inici FROM cursos WHERE ANY=".$any." AND MES='".$mes."' AND DNI_TUTOR <> '0' AND (HORES = 40) AND CURS NOT LIKE '%0%' ORDER BY CURS, AULA");
else
	$result_curs = mysqli_query ($connexio, "SELECT id_Curs, ANY, MES, CURS, AULA, `NOM CURS` AS nom_curs, codi_udg, `DATA FIN` as datafi, Data_llarga_Inici FROM cursos WHERE ANY=".$any." AND MES='".$mes."' AND DNI_TUTOR <> '0' AND (HORES = ".$hores.") AND CURS NOT LIKE '%0%' ORDER BY CURS, AULA");
...
for ($n=0; $n<mysqli_num_rows($result_curs); $n++)
{
	...
	if ($row['codi_udg'] != '#') {
		...
		// echo "Buscar si el missatge està escrit: SELECT id FROM mdl_forum_discussions WHERE course=".$id_curs_moodle." and name LIKE \"%TANCAMENT DEL CURS%\" ORDER BY id DESC<br />";
		$consulta_id_forum_moodle_d = "SELECT id FROM mdl_forum_discussions WHERE course=".$id_curs_moodle." and name LIKE \"%TANCAMENT DEL CURS%\" ORDER BY id DESC";
		$result_forum_d = mysqli_query($connexio_m, $consulta_id_forum_moodle_d);
		$row_d = mysqli_fetch_array($result_forum_d);
		// echo "SELECT id FROM mdl_forum_posts WHERE discussion=".$row_d['id']."<br />";
		...
		// echo "num_rows_forum_discussion: ".mysqli_num_rows($result_forum_d)." num_rows_forum_posts".mysqli_num_rows($result_forum_p);
		if(mysqli_num_rows($result_forum_d)>0 && (mysqli_num_rows($result_forum_p)>0))
			$html_cursos .= "enviat";
		else
		{
			if (mysqli_num_rows($result_forum_d)>0 && (mysqli_num_rows($result_forum_p)==0))
	...
	$consulta_aula_oberta_moodle = "SELECT id, fullname FROM mdl_course WHERE shortname = '".$row['CURS']."'";
	...
	if ($row['codi_udg'] != '#' && (mysqli_num_rows($result_forum_d)==0 || mysqli_num_rows($result_forum_p)==0) )
...
$html_cursos .= "<input type=\"hidden\" id=\"comunicat\" name=\"comunicat\" value=\"tancament_curs\" >";
