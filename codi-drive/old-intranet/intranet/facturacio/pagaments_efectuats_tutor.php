...
<head>
	...
	<script language="Javascript" src="./js/canvia_any.js"></script>
                    ...
                    <div id="llegenda_curs" class="ge">
                    	...
                    	<form name="gestions" method="post" action="<?php echo $PHP_SELF ?>">
                        	<div style="text-align:left; margin-left: 30px;">
                            	<p><br />
                            	Selecciona el tutor que vols cercar:
                                &nbsp;
                                <select name="tutor" id="tutor" size="1">
                                      <option value="Triar" selected>-- Tria un tutor --</option>
                                  <?php
										...
										{
											$result_tutors = mysqli_query ($connexio, "SELECT DISTINCT cobraments.DNI_TUTOR, p.NOM, p.COGNOMS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR = p.DNI ORDER BY COGNOMS;");

											for ($n=0; $n<mysqli_num_rows($result_tutors); $n++)
											{
												$a = mysqli_fetch_array($result_tutors);
												echo "<option value=".$a['DNI_TUTOR'].">".$a['NOM'].' '.$a['COGNOMS']."</option>";
											}
										}
										?>
                                </select>
                            	...
                            	<p>Selecciona l'orde amb el que vols mostrar el contingut:
                                <select name="ordre" id="ordre" size="1" style="margin-left: 5px; margin-right: 10px;">
                            	  ...
                            	  <input type="submit" name="buscar" class="botones" value="buscar" />
                           	      <br />
                           	      <br />
                           	      <br />
                          	            </p>
                            ...
                            <?php  if ($_POST['buscar']=="buscar") {

							  	$connexio = mysqli_connect('localhost','suport','1324GiRoNa','gestio');
								...
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, cobraments.DNI_TUTOR, ROL, GESTIONAT, CURS, MES, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0) AND cobraments.DNI_TUTOR = '".$_POST[tutor]."' ORDER BY cobraments.PAGAT DESC, cobraments.ANY DESC, cobraments.MES DESC, cobraments.CURS, p.NOM, p.COGNOMS");
								}
								else if ($_POST[ordre] == "Tots")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, cobraments.DNI_TUTOR, ROL, GESTIONAT, CURS, MES, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments, personal AS p WHERE DNI_TUTOR=DNI AND GESTIONAT IS NOT NULL AND PAGAT IS NOT NULL  AND cobraments.DNI_TUTOR = '".$_POST[tutor]."'  ORDER BY ANY, MES, CURS");
								}
								else if ($_POST[ordre] == "Cursos")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, cobraments.DNI_TUTOR, ROL, GESTIONAT, CURS, MES, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0)  AND cobraments.DNI_TUTOR = '".$_POST[tutor]."'  ORDER BY cobraments.CURS, cobraments.ANY, cobraments.MES, p.NOM, p.COGNOMS");
								}
								...
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, cobraments.DNI_TUTOR, ROL, GESTIONAT, CURS, MES, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0) AND cobraments.DNI_TUTOR = '".$_POST[tutor]."'  ORDER BY cobraments.ANY, cobraments.MES, cobraments.CURS, p.NOM, p.COGNOMS");
								}
								...
								if(mysqli_num_rows($result)>0)
								{
									...
									<thead class="thead-light">
										<tr>
										  ...
										  <th scope="col">OBSERVACIONS PER AL TUTOR</th>
										</tr>
									</thead>
									<tbody>
									...
									while ($row = mysqli_fetch_array($result))
									{
										...
										else if ($row['DNI_TUTOR']=='77920161W' && time() > 1632994162)
											$nom_tutor = "Noelia_Bueno_Camos";
										else {
									...
									?>
										<tr>
											...
											if (url_exists("https://campus.prisma.cat/intranet-collaboradors/cobraments/factures/".$any_gestio_cobrament.$mes_gestio_cobrament."_Curs_".$row['ANY'].$row['CURS'].$row['MES']."_".$rol_llarg."_".$nom_tutor.".pdf"))
												echo "<td scope=\"col\"><a href=\"".$file."\" target=\"_blank\"><img src=\"https://old.prisma.cat/intranet/img/pdf.png\" alt=\"Imatge pdf\" /></a></td>";
											else
												echo "<td scope=\"col\"><img src=\"https://old.prisma.cat/intranet/img/creu.png\" alt=\"Imatge creu\" /></td>";
