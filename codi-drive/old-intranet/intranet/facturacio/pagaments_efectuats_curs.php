                    ...
                    <div id="llegenda_curs" class="ge">
                    	...
                    	<form name="gestions" method="post" action="<?php echo $PHP_SELF ?>">
                        	<div style="text-align:left; margin-left: 30px;">
                            	<p><br />
                            	Selecciona el curs que vols cercar:
                                &nbsp;
                                <select name="cursos" id="cursos" size="1">
                                      <option value="Triar" selected>-- Tria un curs --</option>
                                  <?php
										...
										{
											$result_cursos = mysqli_query ($connexio, "SELECT DISTINCT CURS FROM cobraments ORDER BY cobraments.CURS");

											for ($n=0; $n<mysqli_num_rows($result_cursos); $n++)
											{
												$a = mysqli_fetch_array($result_cursos);
												echo "<option value=".$a['CURS'].">".$a['CURS']."</option>";
											}
										}
										?>
                                </select>
                            ...
                            <?php  if ($_POST['buscar']=="buscar") { ?>
                             ...
                             <p>Consulta: <?php echo $_POST[cursos]; ?></p>
                            <?php
								...
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, ROL, GESTIONAT, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0) AND CURS = '".$_POST[cursos]."' ORDER BY cobraments.PAGAT DESC, cobraments.ANY DESC, cobraments.MES DESC, cobraments.CURS, p.NOM, p.COGNOMS");

								}
								else if ($_POST[ordre] == "Tots")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, ROL, GESTIONAT, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments, personal AS p WHERE DNI_TUTOR=DNI AND GESTIONAT IS NOT NULL AND PAGAT IS NOT NULL  AND CURS = '".$_POST[cursos]."' AND ALUMNES>0 ORDER BY ANY, MES, CURS");
								}
								else if ($_POST[ordre] == "Tutors")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, ROL, GESTIONAT, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0) AND CURS = '".$_POST[cursos]."' ORDER BY p.NOM, p.COGNOMS, cobraments.ANY, cobraments.MES, cobraments.CURS");
								}
								...
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, ROL, GESTIONAT, IMPORT, NOM, COGNOMS, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0)  AND CURS = '".$_POST[cursos]."' ORDER BY cobraments.ANY, cobraments.MES, cobraments.CURS, p.NOM, p.COGNOMS");
                      ...
                      <td scope="col"><?php echo(	number_format($row['IMPORT'], 2, ',', '.')); ?> €</td>
									...
									?>
										...
										<div align="left" style="padding-top:20px; clear:both">                                       </div>
