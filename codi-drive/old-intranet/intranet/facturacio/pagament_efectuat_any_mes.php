                    ...
                    <div id="llegenda_curs" class="ge">
                    	...
                    	<form name="gestions" method="post" action="<?php echo $PHP_SELF ?>">
                        	<div style="text-align:left; margin-left: 30px;">
                            	<p><br />
                            	Selecciona l'any i el mes que vols cercar:
                                &nbsp;
                                <select name="any" id="any" size="1" onchange="canviAny()">
									...
									<?php
										...
										{
											$result_anys = mysqli_query ($connexio, "SELECT DISTINCT ANY FROM cobraments");
                                ...
                                <select name="mesos" id="mesos" size="1">
                                      <option value="Triar" selected>-- Tria un mes --</option>
                                </select>
&nbsp;
                              &nbsp;</p>
                            ...
                            <?php  if ($_POST['buscar']=="buscar") { ?>
                             ...
                             <p>Consulta: <?php echo $_POST[mesos]; ?>/<?php echo $_POST[any]; ?></p>
                            <?php
								...
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, IMPORT, NOM, COGNOMS, ROL, GESTIONAT, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0) AND ANY = ".$_POST[any]." AND MES = '".$_POST[mesos]."' ORDER BY cobraments.PAGAT DESC, cobraments.ANY DESC, cobraments.MES DESC, cobraments.CURS, p.NOM, p.COGNOMS");
								}
								else if ($_POST[ordre] == "Tots")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, IMPORT, NOM, COGNOMS, ROL, GESTIONAT, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments, personal AS p WHERE DNI_TUTOR=DNI AND GESTIONAT IS NOT NULL AND PAGAT IS NOT NULL  AND ALUMNES>0 AND ANY = ".$_POST[any]." AND MES = '".$_POST[mesos]."' ORDER BY ANY, MES, CURS");
								}
								else if ($_POST[ordre] == "Tutors")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, IMPORT, NOM, COGNOMS, ROL, GESTIONAT, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0)  AND ANY = ".$_POST[any]." AND MES = '".$_POST[mesos]."' ORDER BY p.NOM, p.COGNOMS, cobraments.ANY, cobraments.MES, cobraments.CURS");
								}
								else if ($_POST[ordre] == "Cursos")
								{
									$result = mysqli_query ($connexio, "SELECT DISTINCT ANY, CURS, MES, IMPORT, NOM, COGNOMS, ROL, GESTIONAT, PAGAT, IRPF, APAGAR, cobraments.OBSERVACIONS FROM cobraments INNER JOIN personal AS p ON cobraments.DNI_TUTOR=p.DNI WHERE (((cobraments.PAGAT) Is Not Null) AND ((cobraments.GESTIONAT) Is Not Null) AND ALUMNES>0)  AND ANY = ".$_POST[any]." AND MES = '".$_POST[mesos]."' ORDER BY cobraments.CURS, cobraments.ANY, cobraments.MES, p.NOM, p.COGNOMS");
