                  ...
                  else if ($_POST['gestiook']=="S")
                  {
                    ...
                    if ($_POST[gestio."$_POST[num]"]=="GESTIONAR")
                    {
                      ...
                      else // mostrem les dades del curs a gestionar
                      {
						...
						if ($_SESSION['usuari']=="G67253443") {
							$result_i = mysqli_query ($connexio,"SELECT PREU_ALUMNE AS preu, IRPF, MINIM_ALUMNES, MINIM_COBRAR, MINIM_COBRAR, NOM, COGNOMS, MAIL_PRISMA AS correu, IBAN, OBS_COBRAMENT from personal as p, honoraris as h WHERE p.DNI=h.DNI_TUTOR  and h.ID=314 AND h.PERFIL='tutor'");
						}
						else {
							$result_i = mysqli_query ($connexio,"SELECT PREU_ALUMNE AS preu, IRPF, MINIM_ALUMNES, MINIM_COBRAR, MINIM_COBRAR, NOM, COGNOMS, MAIL_PRISMA AS correu, IBAN, OBS_COBRAMENT FROM (rel_cuho as r INNER JOIN honoraris as h ON r.id_hono = h.ID) INNER JOIN personal as p ON h.DNI_TUTOR = p.DNI WHERE r.id_cuho=".$id_cuho." AND h.PERFIL='tutor'");
						}
                    ...
                    <form name="gestions" method="post" action="<?php echo $PHP_SELF ?>" enctype="multipart/form-data" onSubmit="validar(this)">
                      ...
                      {
                        ?><p align="left"><strong>IRPF: </strong>No aplica</p>
                        <p align="left"><strong>IMPORT (NET):</strong> <?php echo $import; ?> euros</p><?php
                      }
                      else
                      {
                        ?><p align="left"><strong>IRPF (<?php echo $row_irpf_username; ?>%):</strong> <?php echo $irpf; ?> euros</p>
                        <p align="left"><strong>IMPORT (NET):</strong> <?php echo $importnet; ?> euros</p><?php
