        ...
        <tr>
            <td>
                ...
                <div id="menu" style="clear:both; padding-top:10px; padding-left:50px">                
                    <div class="nav2"><a href="resum_mes_minims.php" style="margin-left:1px;"><span style="font-size:10px">RESUM</span><br />D'UN MES</a></div>
                <div class="nav_on2" style="margin-left:1px;"><span style="font-size:10px">RESUM</span><br />D'UN MES (antic)<br /></div>		
								<?php
                  //if ($_SESSION['rol']=="admin") {
                ?>
								...
								if(mysql_num_rows($result)>0)
								{
									...
									while ($c = mysql_fetch_array($result_cursos))
									{
										if ($row['CURS']==$c['CURS'])
										{
											...
											if ($c['id_cuho']<>17)
											{ 
												$result_despeses = mysql_query ("SELECT PERFIL, PREU_ALUMNE, MINIM_ALUMNES, CURS, id_cuho FROM honoraris AS h, rel_cuho AS r WHERE r.id_cuho=".$c['id_cuho']." AND r.id_hono=h.ID", $conexion);
												...
												{												
													if ($row_desp['PERFIL']=='tutor')
													{
														...
														if (($row_desp['MINIM_ALUMNES'] == 15) && ($row['inscrits'] >=15))
															$despeses = $despeses + ($row_desp['PREU_ALUMNE']+$neuro)*$row['inscrits'];
														else if ($row_desp['CURS'] == 'GED') { 
															...
															$despeses = $despeses + ($row_desp['PREU_ALUMNE']+$neuro)*15;
													}													
													else
														$despeses = $despeses + $row_desp['PREU_ALUMNE']*$row['inscrits'];
