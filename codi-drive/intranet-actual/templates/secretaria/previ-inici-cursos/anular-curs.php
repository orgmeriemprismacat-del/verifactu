...
if ( count($dates) > 0 ) {
   $textProxDates = "Si prefereixes fer el curs més endavant, podem traslladar la
   teva inscripció a una de les properes edicions:</p>";
   $textProxDates .= "<ul style='list-style: circle; margin-left: 30px; padding: 0;'>";
   for ( $i = 0; $i < count($dates); $i++ ) {
      $textProxDates .= "<li style='margin-top: 8px; line-height: 24px;'>".$dates[$i]."</li>";
   }
   $textProxDates .= "</ul>";
   $textProxDates .= "<p>Si t’interessa alguna d’aquestes opcions o prefereixes
   esperar-te a una convocatòria posterior, <strong>posa’t en contacte amb nosaltres
   indicant a quina edició et vols inscriure</strong>, i et gestionarem el canvi.</p>";
}
else {
   $textProxDates = "Si prefereixes fer el curs més endavant, podem mantenir la
   teva inscripció per a una edició futura (<strong>properes dates: a concretar</strong>).</p>";
   $textProxDates .= "<p>En aquest cas, <strong>cal que et posis en contacte amb
   nosaltres</strong> per indicar-nos que vols reservar plaça per a una propera
   convocatòria o esperar-te a noves dates. Quan tinguem noves edicions programades,
   t’ho notificarem i gestionarem el canvi segons la teva preferència.</p>";
}
...
$missatge = "<p><p>Benvolgut/da [NOM_ALUMNE],</p>
   ...
   <ol style='margin-bottom: 20px; margin-left: 30px; padding: 0;'>
      ...
      <li style='margin-top: 8px; line-height: 24px;'>
         <p><strong>Posposar la inscripció a una altra convocatòria</strong><br />
         ".$textProxDates."
      </li>
      ".$textPagat."
   </ol>

   <p>Disculpa les molèsties que aquesta anul·lació et pugui haver causat.
   Restem a la teva disposició per qualsevol dubte o gestió.</p>
";
