...
requestMain.done(function( message ) {
	...
	*/
	...
	/* TODO FUNCTION CLICK SAVE OBS saveObservacionsInscripcio
		Quan es clica sobre el botó de guardar, es desarà la observació corresponent
	*/
	$("#cursos").on("click", ".saveObs", function(e) {
		var idSaveObs = $(this).attr('id');
		var codiCurs = $(this).attr('id').split("-")[1];
		var idInsc = $(this).attr('id').split("-")[2];
		var obsInsc = $( "#obs-" + codiCurs + "-" + idInsc ).val();

		if ( $( "#obs-" + codiCurs + "-" + idInsc ).val() != '' ) {
			var request = $.ajax({
				url: path + "alumnes/saveObservacionsInscripcio.php",
				global: false,
				method: "GET",
				data: {
					idInsc: idInsc,
			...
			request.done(function( msg ) {
				if ( !msg.toLowerCase().includes("error") ) {
					afegirHeaderModalSuccess("Observació desada!");
					afegirTextModalSuccess(msg);
					mostrarModalSuccess();
				}
				else {
					afegirHeaderModalError("Alerta!");
					afegirTextModalError("Hi ha hagut un error a l'hora de guardar l'observació");
					mostrarModalError();
				}
			});
			request.fail(function( jqXHR, textStatus, errorThrown ) {
				errorFunction( jqXHR, textStatus, errorThrown,
						"Hi ha hagut un error a l'hora de guardar l'observació: ");
			});
		}
		else {
			afegirHeaderModalError("Alerta");
			afegirTextModalError("El camp d'observacions no pot estar buit.");
			mostrarModalError();
		}
	});
	...
	$("#cursos").on("click", ".sendAvis", function(e) {
		...
		$("#cntCurs-" + codiCursActual + "-" + $anyCurs + "-" + $mesCurs + " .avisar-alumne").each(function() {
			if ( $(this).hasClass('lightGreen') ) { //Indica que sí
				...
				var request = $.ajax({
					url: path + "alumnes/sendMsgAvisCursosPendents.php",
					global: false,
					method: "GET",
					data: {
						idInsc: idInsc
