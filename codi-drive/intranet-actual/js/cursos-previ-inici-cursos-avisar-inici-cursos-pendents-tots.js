...
requestMain.done(function( message ) {
	...
	*/
	...
	$("#cursos").on("click", ".avisar-tutor", function(e) {
		if ( $(this).hasClass("lightGreen") ) {
			$(this).addClass('lightRed');
			$(this).removeClass('lightGreen');
			$(this).html('NO');
	...
	*/
	$("#cursos").on("click", ".sendAvis", function(e) {
		...
		if ( dataMaxim != '' && hourDataMaxim != '' ) {
			...
			$("#cntCurs-" + codiCursActual + "-" + $anyCurs + "-" + $mesCurs + " .avisar-tutor").each(function() {
				if ( $(this).hasClass('lightGreen') ) { //Indica que sí
					var dniTutor = $(this).attr('id').split('-')[1];
					var any = $(this).attr('id').split('-')[2];
					var mes = $(this).attr('id').split('-')[3];
					var curs = $(this).attr('id').split('-')[4];
					var request = $.ajax({
						url: path + "tutors/sendMsgAvisCursosPendentsTots.php",
						global: false,
						method: "GET",
						data: {
							dniTutor: dniTutor,
