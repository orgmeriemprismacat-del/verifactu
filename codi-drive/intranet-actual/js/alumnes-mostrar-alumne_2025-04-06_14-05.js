...
/* Mostrem el main */
$.ajax({
	url: path + "mostrarMain.php?url=" + urlPagina,
	cache: !1,
	global: false,
	type: "GET",
	success: function(message) {
		...
		$('#cercar-alumne').on('click', function() {
			...
			if (
				...
				if (dni != '') elementsCercats += "DNI «" + dni + "»";
				if (email != '') {
				...
				//crida ajax cercar alumnes que dni = dni o email = email o els cognoms contingui cognoms o el telefeon = telefon
				$.ajax({
					url: path + "alumnes/consultaUsuari.php?dni=" + dni + "&email=" +
						email + "&nom=" + nom + "&cognoms=" + cognoms + "&telefon=" + telefon +
						"&any=" + any + "&mes=" + mes + "&curs=" + curs + "&inscrit=" + inscrit +
						"&certificat=" + certificat + "&poblacio=" + poblacio + "&perfil=" + perfil +
						"&titulacio=" + titulacio + "&comhapagat=" + comhapagat + "&obspagament=" +
						obspagament + "&reclamat=" + reclamat + "&obs=" + obs + "&comentaris=" +
						comentaris + "&databaixa=" + databaixa + "&perenne=" + perenne,
					cache: !1,
					global: false,
					type: "GET",
					success: function(dnies) {
						let vectDnies = dnies.split('#');
						if ( dnies.includes("Error") || dnies.includes("error") ) { //Hi ha un error
							amagarLoadingModal();
							afegirHeaderModalError("Hi ha hagut un error al cercar l'alumne");
							mostrarModalError();
							reloadUrl();
							...
							if (vectDnies2.length == 1) { //Hi ha un sol usuari amb la cerca realitzada
...
function mostraLlistatUsuaris(dnies, orderby, asc) {
	...
	//Mostra la taula amb els usuaris trobats a partir de la cerca realitzada
	$.ajax({
		url: path + "alumnes/mostrarTaulaUsuaris.php?dnies=" + dnies + "&orderBy=" + orderby + "&asc=" + asc,
		cache: !1,
		global: false,
		type: "GET",
...
function cercarUSuari(dniUser) {
	$.ajax({
		url: path + "alumnes/mostrarInformacioUsuari.php?dni=" + dniUser + "&cercaPer=" + cercaPer,
		cache: !1,
		global: false,
		type: "GET",
		success: function(info) {
...
function mostrarModalConsultaInformacio(id) {
	$('.modal-info').off();
	$.ajax({
		url: path + "alumnes/mostraModalConsultaInformacio.php?idInsc=" + id,
		cache: !1,
		global: false,
		type: "GET",
		success: function(res) {
			if (!res.toLowerCase().includes("error") || (res.toLowerCase().includes("error") && !res.toLowerCase().includes("not found")) ) {
				...
				$("#modalConsultaInformacio").on('hidden.bs.modal', function (e) {
					console.log('hide');
					$('#cercar-alumne').click();
				})
...
function mostrarModalCanviCurs(id) {
	...
	$.ajax({
		url: path + "alumnes/mostrarModalCanviCurs.php?idInsc=" + id,
		cache: !1,
		global: false,
		type: "GET",
		global: false,
				...
				$('#modalCanviCurs').on('click', '.save-result', function() {
					...
					if (id != '' && any != '' && mes != '' && !mes.includes("Triar") && curs != '' && numeroCanvi != -1 && motiu != '') {
						if (any == anyActual && mes == mesActual && curs == cursActual) {
							...
							$("#modalConfirmacioCanvi").modal('show');

							$('#modalConfirmacioCanvi').on('click', '#confirmar-canvi', function() {
								mostrarModalLoading();
								$("#modalConfirmacioCanvi").modal('hide');
								$.ajax({
									url: path + "alumnes/realitzarCanviCurs_CanviCurs.php?idinsc=" +
										id + "&any=" + any + "&mes=" + mes + "&curs=" + curs +
										"&numero=" + numeroCanvi + "&apagar=" + apagar + "&pagat=" +
										pagat + "&pendent=" + pendent + "&despeses=" + despeses +
										"&obs=" + obs + "&motiu=" + motiu + "&enviarCoreu=" + enviarCoreu +
										"&tipusDesc=" + tipusDesc + "&validDesc=" + validDesc,
									cache: !1,
									global: false,
									type: "GET",
									global: false,
									success: function(res) {
										...
										if (!res.toLowerCase().includes("error")) {
											afegirHeaderModalSuccess("El canvi s'ha realitzat correctament");
											mostrarModalSuccess();
											//torno a carregar el resultat de la cerca
											$('#cercar-alumne').click();
										} else {
											afegirHeaderModalError("Hi ha hagut un error al realitzar el canvi");
											mostrarModalError();
											reloadUrl();
										}
									}
	...
	function buscarAnys_modalCanviCurs() {
		...
		$.ajax({
			url: path + "alumnes/buscarAnysDisponibles_modalCanviCurs.php?curs=" + curs,
			cache: !1,
			global: false,
			type: "GET",
			success: function(res) {
	...
	function buscarMesos_modalCanviCurs() {
		...
		var curs = $('#dades-canvi-curs .element-selected').html().trim();
		$.ajax({
			url: path + "alumnes/buscarMesosDisponibles_modalCanviCurs.php?any=" + any + "&curs=" + curs,
			cache: !1,
			global: false,
			type: "GET",
			success: function(res) {
	...
	function buscarCursos_modalCanviCurs() {
		...
		if (!mes.toLowerCase().includes("triar")) {
			$.ajax({
				url: path + "alumnes/buscarCursosDisponibles_modalCanviCurs.php?any=" + any + "&mes=" + mes,
				cache: !1,
				global: false,
				type: "GET",
				success: function(res) {
	...
	function buscarPreuAPagar() {
		...
		var tipusDesc = $('#tipusDesc-registre').html().trim();
		$.ajax({
			url: path + "alumnes/buscarPreuAPagar_modalCanviCurs.php?id=" +
				id + "&any=" + anyC + "&mes=" + mesC + "&curs=" + cursC + "&apagar=" + apagar + "&tipusDesc=" + tipusDesc + "&validDesc=" + validDesc ,
			cache: !1,
			global: false,
			type: "GET",
			success: function(res) {
	...
	function calcularDespesGestio(numero) {
		...
		if (numero == 4) {
			$.ajax({
				url: path + "alumnes/calcularDespesesGestio_modalCanviCurs.php?any=" +
					any + "&mes=" + mes + "&curs=" + curs,
				cache: !1,
				global: false,
				type: "GET",
				success: function(despeses) {
...
function mostrarModalDonarBaixa(id) {
	$('.modal-info').off();
	$.ajax({
		url: path + "alumnes/mostraModalDonarBaixa.php?idInsc=" + id,
		cache: !1,
		global: false,
		type: "GET",
		success: function(res) {
			if (!res.toLowerCase().includes("error") || !res.toLowerCase().includes("404")) {
				...
				amagarLoadingModal();
				$("#modalDonarBaixa").modal('show');

				$('.confirma-baixa').off();

				/*Si es clica el botó d'.confirma-baixa' es confimra la baixa */
				$('.confirma-baixa').on('click', function() {
					var id = $('#dades-baixa-inscripcio #id-baixa').html().trim();
					...
					if (id != '' && motiu != '') {
						...
						mostrarModalLoading();
						$.ajax({
							url: path + "alumnes/confirmacioBaixa_DonarBaixa.php?idinsc=" +
								id + "&motiu=" + motiu + "&enviarCoreu=" + enviarCoreu,
							cache: !1,
							global: false,
							type: "GET",
							success: function(res) {
...
function mostrarModalConsultaFactura(id) {
	$('.modal-info').off();
	$.ajax({
		url: path + "alumnes/mostraModalConsultaFactura.php?idInsc=" + id,
		cache: !1,
		global: false,
		type: "GET",
		success: function(res) {
			...
			if (!res.toLowerCase().includes("error") || !res.toLowerCase().includes("404")) {
				...
				$('.download-factura').on('click', function() {
					...
					$.ajax({
						url: path + "alumnes/descarregaFactura.php?id=" + id,
						cache: !1,
						global: false,
						type: "GET",
...
function mostrarModalConsultaCertificat(id, tipus) {
	...
	request.done(function( res ) {
		if (!res.toLowerCase().includes("error")) {
			...
			amagarLoadingModal();
			$("#modalConsultaCertificat").modal('show');
...
function amagarModalConsultaCertificat() {
	$("#modalConsultaCertificat").modal('hide');
}
...
function mostrarModalObservacions() {
	$("#modalObservacions").modal('show');
}

function amagarModalObservacions() {
	$("#modalObservacions").modal('hide');
}
