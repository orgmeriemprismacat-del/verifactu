...
requestMain.done(function( message ) {
		...
		$('#cursos').on('click', '#cercar-cursos', function() {
			...
			if ( anyCercat == '' || mesCercat == '' || horaCercada == '' || horaCercada == 'Qualsevol' ) {
				afegirHeaderModalError("Oops...!");
				afegirTextModalError("Has d'omplir els camps de l'ANY, el MES i les HORES per poder fer la cerca! ");
				mostrarModalError();
			}
			else {
				mostrarAlumnesEnviarCertificat(anyCercat, mesCercat, horaCercada, 'codi', 1);
			}
		});
});
...
function mostrarAlumnesEnviarCertificat(any, mes, hores, orderBy, asc) {
	$('#resultats-cerca').off();
	$('.table-order').off();

	/* Consulta cursos a partir d'un any i un mes */
	var reqCursos = $.ajax({
		url: path + "cursos/buscarAlumnesEnviarCertficat.php",
		method: "GET",
		data: {
			any : any,
			mes : mes,
			...
			$("#resultats-cerca").on("click", ".sendAvis", function(e) {
				...
				if ( nElements <= 0 ) {
					...
					$("#resultats-cerca .avisar-alumne.lightGreen").each(function() {
						...
						var idsError = "";
						var request = $.ajax({
							url: path + "alumnes/sendMsgAvisCertificat.php",
							global: false,
							method: "GET",
							data: {
								idInsc: idInsc
...
function mostrarAlumnesEnviarCertificatCurs(any, mes, curs, aula) {
	$('#resultats-cerca').off();
	$('.table-order').off();

	/* Consulta cursos a partir d'un any i un mes */
	var reqCursos = $.ajax({
		url: path + "cursos/buscarAlumnesEnviarCertficatCurs.php",
		method: "GET",
		data: {
			any : any,
			mes : mes,
