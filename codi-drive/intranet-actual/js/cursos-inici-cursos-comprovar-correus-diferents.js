...
requestMain.done(function( message ) {
		...
		$('#cursos').on('click', '#cercar-cursos', function() {
			...
			if ( anyCercat == '' || mesCercat == '' ) {
				...
				mostrarAlumnesCorreuDiferent(anyCercat, mesCercat, 'dni', 1);
			}
		});
});
...
function mostrarAlumnesCorreuDiferent(any, mes, orderBy, asc) {
	$('#resultats-cerca').off();
	$('.table-order').off();

	/* Consulta cursos a partir d'un any i un mes */
	var reqCursos = $.ajax({
		url: path + "cursos/buscarAlumnesCorreuDiferentBDiCampus.php",
		method: "GET",
		data: {
			any : any,
			mes : mes,
	...
	reqCursos.done(function( res ) {
			...
			$("#resultats-cerca").on("click", ".sendAvis", function(e) {
				...
				if ( nElements <= 0 ) {
					...
					$("#resultats-cerca .avisar-alumne.lightGreen").each(function() {
						...
						var idsError = "";
						var request = $.ajax({
							url: path + "alumnes/sendMsgAvisCorreuDiferentBDCampus.php",
							global: false,
							method: "GET",
							data: {
								idInsc: idInsc
			...
			$('#resultats-cerca .table-order').on('click', '.sorting', function() {
				...
				if ($(this).hasClass('asc'))
					mostrarAlumnesCorreuDiferent(any, mes, id, 0);
				else
					mostrarAlumnesCorreuDiferent(any, mes, id, 1);
			});
