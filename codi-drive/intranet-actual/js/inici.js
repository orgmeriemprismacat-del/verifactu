...
function aplicarFuncio( idFuncio ) {
	...
	else if ( idFuncio == 17 ) { // 1a Reclamació Pagament
		$('#content-page').on('click', '.reclamar', function() {
			var idApartat = $(this).attr('id').split('-')[1];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 18 ) { // Baixes segona setmana
		$('#content-page').on('click', '.baixa', function() {
			var idApartat = $(this).attr('id').split('-')[1];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 19 ) { // Recordatori pagament final
		$('#content-page').on('click', '.rec-pag', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 20 ) { // Reclamació i baixes morosos
		$('#content-page').on('click', '.claim-pag', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 21 ) { // Control morosos
		$('#content-page').on('click', '.gest-moros', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 23 ) {
		$('#content-page').on('click', '.calendari-dev-meriem', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 24 ) {
		$('#content-page').on('click', '.calendari-mrkt-meriem', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 25 ) {
		$('#content-page').on('click', '.calendari-xs-meriem', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
	else if ( idFuncio == 26 ) {
		$('#content-page').on('click', '.calendari-mail-meriem', function() {
			var idApartat = $(this).attr('id').split('-')[2];
			obreApartat(idApartat);
		});
	}
