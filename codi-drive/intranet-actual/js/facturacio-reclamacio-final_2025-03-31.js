...
function activaModalResponsable() {
	$('.tipus.lightOrange').addClass('pointer');
	$('.tipus.lightRose').addClass('pointer');
	$('#reclamacions').on('click', '.tipus', function() {
		if ( ($(this).hasClass('lightOrange') || $(this).hasClass('lightRose')) ) {
			var idInsc = $(this).attr('id').split('-')[2];
			var tipusInsc = $(this).attr('id').split('-')[3];
			mostrarModalLoading();
			mostrarModalResponsable(idInsc, tipusInsc);
		}
	});
}
...
function activaFuncionsTaula() {
	...
	activaButtonUpd();
	activaMarcatge();
	activaModalResponsable();
}

function mostrarModalResponsable(id, tipus) {
	var upd = $.ajax({
		url: path + "facturacio/viewModalResp.php",
		method: "GET",
		data: {
			idInsc : id,
			tipusInsc : tipus
	...
	upd.done(function( res ) {
	   	 if ( res.toLowerCase().includes("error") ) {
				...
				amagarLoadingModal();
				$("#modalResponsable .modal-body").html(res);
				bootstrap.Modal.getOrCreateInstance(document.getElementById('modalResponsable')).show();

				$('#modalResponsable').on('click', '.btn-success', function() {
					bootstrap.Modal.getInstance(document.getElementById('modalResponsable')).hide();
				});
				$('#modalResponsable').on('click', '.close', function() {
					bootstrap.Modal.getInstance(document.getElementById('modalResponsable')).hide();
				});
			}
	});
