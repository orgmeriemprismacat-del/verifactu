				...
				if ( count($contentCSV)-1 == 12 ) {
					...
					while ( ! feof( $handle ) ) {
						...
						if ( $content != '')  {
							...
							// echo "<br />";
							// for ($j = 0; $j<count($contentCSV); $j++) {178
							// 	echo "[".$i."][".$j."]".mb_convert_encoding($contentCSV[$j], "UTF-8")."\n";
							// }
							// echo "<br />";

							$ignore = (
								($transaccio != "Autorización" && $transaccio != "Devolución" && $transaccio != "Autorizaci?n" && $transaccio != "Devoluci?n") || $numComanda == 0 ||
							...
							if ( !$ignore ) {
								if ( strripos($accioTransaccio,'Autorizada') !== false ) {
									...
									if ( $transaccio == "Devolución" || $transaccio == "Devoluci?n" ) {
										$tipus = 'R';
										// $import = 0 - $importRetornat;
										$import = 0 - $importEuros;
									}
									else {
										$tipus = 'A';
										$import = $importEuros;
									...
									// echo $cnsFacturaAmbComanda."<br />";
									// echo $datePayTPVYYYYMMDD."<br />";
									// echo $numComanda."<br />";
									// echo $import."<br />";
									// echo $tipus."<br />";
									...
									// echo $cnsFacturaSenseComanda."<br />";
									// echo $datePayTPVYYYYMMDD."<br />";
									// echo $import."<br />";
									// echo $tipus."<br />";
									// echo $cif."<br />";
									if ( !$existeix ) {
										if ( $stmtFact2=$conWeb2->prepare( $cnsFacturaSenseComanda ) ) {
											$stmtFact2->bind_param("sdss", $datePayTPVYYYYMMDD, $import, $tipus, $cif);
											$stmtFact2->execute();
											$stmtFact2->store_result();
											$existeix = $stmtFact2->num_rows() > 0;
											$conWeb2->closeStmt();
										}
									}
								...
								// echo "ignore ".$i."<br />";
								if ( $transaccio != "Autorización" && $transaccio != "Devolución" && $transaccio != "Autorizaci?n" && $transaccio != "Devoluci?n") $typeError = 0;
