...
try {
	...
	$path = "https://intranet.prisma.cat";
	$rols = $_SESSION['usuari']->getRols();
	$nameuser = $_SESSION['usuari']->getUsuari()->get();
	$menuExt = $_SESSION['usuari']->getMenuExt();
		...
		if ($trobat) {
			...
			if ($stmtIntra2->num_rows() > 0) {
				...
				$mostrar .= "' data-nitem='".$nItem."'>
					<a data-bs-toggle='collapse' class='nav-link collapsed' href='#item".$id."' aria-expanded='false'>
						".$icona."
						<p>".$nom."</p>
						<b class='caret'></b>
					</a>
