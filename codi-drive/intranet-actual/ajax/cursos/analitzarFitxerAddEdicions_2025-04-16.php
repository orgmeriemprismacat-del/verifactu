		...
		if ( file_exists($path_file) ) {
			if (!$handle = fopen($path_file, "r")) {
		      ...
		      $conWeb3->connectarBD();
				...
				if ( count($contentCSV)-1 > $numColumnesCSVCorrect ) {
					...
					while ( ! feof( $handle ) ) {
						...
						if ( $contentCSV != '' ) {
							...
							if ( !$ignore ) {
						      ...
						      $cursEscolar = mb_convert_encoding($contentCSV[1], "UTF-8");
						      $codiGtaf = mb_convert_encoding($contentCSV[2], "UTF-8");
						      // $nomCurs = utf8_encode($contentCSV[3]);
						      $nomCurs = ($contentCSV[3]);
						      $numHores = intval($contentCSV[15]);
						      $dataI = $contentCSV[16];
						      $dataF = $contentCSV[17];
						      $codiStatus = $contentCSV[31];

								// echo "status:".$codiStatus."<br />";
								// echo "status:".$nomCurs."<br />";
								// echo "status:".$dataI."<br />";
								// echo "status:".$dataF."<br />";
								// echo "status:".$cursEscolar."<br />";
								// echo "status:".$codiGtaf."<br />";
