...
class Info extends Curs {
   ...
   */
   public function __construct($url ,$dispositiu) {
      ...
      VALORACIONS, ID_ARTICLE FROM informacio WHERE ID_AMIGABLE=? AND ESTAT=1";
      ...
      $stmInfo->bind_result($idInfo, $codi, $titol, $desc, $idVideo, $idImgLarge, $idImgSmall, $pres, $autoria, $dest, $obj, $prog, $val, $idArticle);
      $stmInfo->fetch();
      $connexio->closeStmt();
      ...
      if ($this->estat==1) {
         ...
         $cnsEd = "SELECT DATAI, DATAF, CURS_ESCOLAR, FISS, GTAF, DATA_RESOL, ANY, MES, c.CURS,
            DNI_TUTOR, HORES, c.ESTAT FROM curs AS c INNER JOIN aula AS a ON
         ...
         $cntEd = 0;
         while ( $stmtEd->fetch() ){
            $this->edicions[$cntEd] = new Edicio($curs, $this->dispositiu, $any, $mesDesc, $hores, $datai, $dataf, $cursEscolar, $gtaf, $dataRes, $fiss, $estatEd);
            $cntEd++;
         }
         $connexio->closeStmt();
      }
