...
try {
	...
	*/
	...
	$buscarCursos = "SELECT ID_AULA, GTAF, DATA_VALORACIO, DATA_RESOL FROM curs";
	$updGtaf = "UPDATE aula SET GTAF_AULA = ?, DATA_VALORACIO_AULA = ?, DATA_RES = ? WHERE ID_AULA = ?";
   ...
   $conWeb2->connectarBD();
	...
	if ( $stmt=$conWeb->prepare( $buscarCursos ) ) {
		if ( $stmt2=$conWeb2->prepare( $updGtaf ) ) {
			$stmt->execute();
			$stmt->bind_result($idAula, $gtaf, $dataValoracioGtaf, $dataResol);
			while ( $stmt->fetch() ) {
				$stmt2->bind_param("sssd", $gtaf, $dataValoracioGtaf, $dataResol, $idAula);
