...
$pagina = "gestio_cobraments";

if ($USER->username=="43400030") //Daniel Gabarro
{
	header("Location: https://www.prisma.cat/intranet/acces_extern.php");
	exit;
}
else if ($USER->username=="43674436" or $USER->username=="79302336") //Sistèmiques
{
	header("Location: https://www.prisma.cat/intranet/acces_extern.php");
	exit;
}
  	...
  	<table width="1100" align="center" bgcolor="#FFFFFF" cellspacing="0" cellpadding="0" id="main">
        <tr>
          ...
          <tr>
              ...
              <div id="login">
                <div id="llegenda_curs" class="ge">
                  ...
                  if ($_POST['revisio']=="ENVIAR PER A REVISIÓ")
                  {
                    ...
                    ?><div align="center"><br /><input type="button" name="boto" class="botones" value="GESTIONAR UN ALTRE CURS" onclick="window.location.href='./gestio_cobraments.php'"/></div><?php
                  }
                  ...
                  else if ($_POST['gestiook']=="S")
                  {
                    ...
                    if ($_POST[gestio."$_POST[num]"]=="GESTIONAR")
                    {
                      ...
                      {
                        $result = mysqli_query ($connexio,"SELECT c.CURS, c.MES, c.ANY, `NOM CURS` AS titol, AULA, Data_llarga_Inici, Data_llarga_Fin, data_informe, DNI_TUTOR, id_cuho, SUM(CASE WHEN (`INSC CURS` = '1') THEN 1 ELSE 0 END) inscrits FROM cursos AS c, inscripcions AS i WHERE ".$text." and c.CURS='".$_POST[curs_tut."$n"]."' AND c.ANY=".$_POST[any_tut."$n"]." AND c.MES='".$_POST[mes_tut."$n"]."' AND pagat='N' AND c.CURS=i.CURS AND c.ANY=i.ANY AND c.MES=i.MES AND AULA=Grup GROUP BY Grup");
                    ...
                    <form name="gestions" method="post" action="<?php echo $PHP_SELF ?>" enctype="multipart/form-data" onSubmit="validar(this)">
                      ...
                      <?php
        							if ($_POST[gestio."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="confirmar" class="botones" value="HI ESTIC D'ACORD" onclick="return comprovar_validar()"/></div><?php
        							}
        							if ($_POST[gestio2."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="confirmar2" class="botones" value="HI ESTIC D'ACORD" onclick="return comprovar_validar()"/></div><?php
        							}
        							if ($_POST[gestio3."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="confirmar3" class="botones" value="HI ESTIC D'ACORD" onclick="return comprovar_validar()"/></div><?php
        							}
                      if ($_POST[gestio4."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="confirmar4" class="botones" value="HI ESTIC D'ACORD" onclick="return comprovar_validar()"/></div><?php
        							}
        							?>
                      ...
                      <p align="left"><br /><br />Hi ha alguna dada incorrecta? (Quantitat d'alumnes, número de compte, el botó «HI ESTIC D'ACORD» està desactivat...) Pots especificar-ho en el camp següent i clicar al botó «ENVIAR PER A REVISIÓ».</p>
                      <textarea name="comentaris" style="width:990px; height:65px"></textarea>

                      <?php
        							if ($_POST[gestio."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="revisio" class="botones" value="ENVIAR PER A REVISIÓ" onclick="return  comprovar_revisar(this.form)"/></div><?php
        							}
        							if ($_POST[gestio2."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="revisio2" class="botones" value="ENVIAR PER A REVISIÓ" onclick="return  comprovar_revisar(this.form)"/></div><?php
        							}
        							if ($_POST[gestio3."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="revisio3" class="botones" value="ENVIAR PER A REVISIÓ" onclick="return comprovar_revisar(this.form)"/></div><?php
                      }
                      if ($_POST[gestio4."$_POST[num]"]=="GESTIONAR")
        							{
        							  ?><div align="center"><br /><input type="submit" name="revisio4" class="botones" value="ENVIAR PER A REVISIÓ" onclick="return comprovar_revisar(this.form)"/></div><?php
                      }
                      ?>
                        ...
                        if (mysqli_num_rows($tutories)>0)
                        {
                          ...
                          while ($row_t = mysqli_fetch_array($tutories))
                          {
                            ...
                            if(mysqli_num_rows($tutories_ja_pagades)==0)
                            {
                              ...
                              {
                                echo "<em>AVÍS <span style=color:#F39132>(*)</span></em>";
                                $nota = "<p style=text-align:left;clear:both; font-weight:bold><br /><span style=color:#F39132>(*)</span> Per gestionar aquesta factura has hagut d'haver enviat el <em>Certificat d'estar al corrent en les obligacions de la Seguretat Social</em>. Pots clicar <a href=\"https://www.prisma.cat/campus/intranet/tutors/estat_laboral.php\" title=\"Clica aquí\" target=\"_blank\" style=\"text-decoration: none; color: #8FA97A; font-weight: bold;\">aquí</a> per fer-ho.</p>";
                              }
                            ...
                            if ($OK=="true" && $dies_passats>=0)
                            {
                              ...
                              if(mysqli_num_rows($coordinacions_ja_pagades)==0 && $posterior)
                              {
                                ...
                                if ($cap4 == true)
                                {
                                   $cap4 = false;
                                   ?>
                                   <tr><td style="width:80px; text-align:center; background-color:#839872; font-weight: bold; color:#FFFFFF;" colspan="6">Coordinacions</td></tr>
                                   <tr>
                                     <td style="width:80px; text-align:center"><b>ANY</b></td>
                                     <td style="width:80px; text-align:center"><b>CURS</b></td>
                                     <td style="width:80px; text-align:center"><b>TRIMESTRE</b></td>
            ...
            {
              $text_no_identificat = "<p align=center style=font-size:12px>";
              $text_no_identificat = $text_no_identificat."<br /><br />Per accedir a la Intranet has d'estar identificat/da en el Campus Virtual.";
              $text_no_identificat = $text_no_identificat."<br /><br /><a href=http://www.prisma.cat/ target=_blank style=text-decoration:none>www.prisma.cat</a>";
              $text_no_identificat = $text_no_identificat."</p>";
              echo $text_no_identificat;
            }
            ?>
            </td>
        </tr>
