    	...
    	<body>
            ...
            <?php
              ...
              if ($_POST['revisio']=="ENVIAR PER A REVISIÓ")
              {
                ...
                <strong>MÍNIM A COBRAR:</strong> ".$_POST[minimcobrar]."<br>
              ...
              else if ($_POST['gestiook']=="S")
              {
                ...
                if ($_POST[gestio."$_POST[num]"]=="GESTIONAR")
                {
                  ...
                  {
                    $result = mysqli_query ($connexio,"SELECT c.CURS, c.MES, c.ANY, `NOM CURS` AS titol, AULA, `DATA INICI` AS datainici, Data_llarga_Inici, Data_llarga_Fin, data_informe, DNI_TUTOR, id_cuho, SUM(CASE WHEN (`INSC CURS` = '1' OR `INSC CURS` = 'M') THEN 1 ELSE 0 END) inscrits
                    ...
                    $result_i = mysqli_query ($connexio,"SELECT PREU_ALUMNE AS preu, IRPF, MINIM_ALUMNES, MINIM_COBRAR, NOM, COGNOMS, MAIL_PRISMA AS correu, IBAN, OBS_COBRAMENT FROM
                    ...
                    echo "<p class='w-100 text-left'><strong>PREU PER ALUMNE:</strong> ".$row_i['preu']." euros</p>";
                   ...
                   // }
                    ...
                    if ($datainici < '2026-03-01') { 
                       if (($row_i['MINIM_ALUMNES'] == 15) && ($inscrits_totals < 15)) { // tutors antics amb mínim d'alumnes
				   		$import = 15 * $row_i['preu'];
                        $nota = "<p align=left style=color:#666666><em>NOTA: es cobra un mínim de 15 alumnes per edició.</em></p>";
                        }
                        else { // tutors nous sense mínims d'alumnes
                            $import = $inscrits_totals * $row_i['preu'];					
                        }                         
                    }
                    else {  
                       if (($row_i['MINIM_COBRAR'] > 0) && ($inscrits_totals < $row_i['MINIM_COBRAR'])) { // tutors amb mínim d'alumnes, a partir del març 2026
				   		$import = $row_i['MINIM_COBRAR'] * $row_i['preu'];                        
                        }
                        else { // tutors nous sense mínims d'alumnes
                            $import = $inscrits_totals * $row_i['preu'];					
                        }
                        if ($row_i['MINIM_COBRAR'] > 0) {
                            $nota = "<p align=left style=color:#666666><em>NOTA: es cobra un mínim de ".$row_i['MINIM_COBRAR']. " alumnes per edició.</em></p>";
                        }
                    }                         
                ...
                <form class='w-100' name="gestions" method="post" action="<?php echo $PHP_SELF ?>" enctype="multipart/form-data" onSubmit="validar(this)">
                  ...
                  <input type="hidden" name="minimcobrar" value="<?php echo $row_i['MINIM_COBRAR']; ?>" />
                ...
                else
                {
                  ...
                  if ($USER->username == '40526355') // C. Peinado
                    ...
                    $tutories = mysqli_query ($connexio, "SELECT c.ANY, c.CURS, c.MES, `DATA INICI` AS datai, DATEDIFF(CURRENT_DATE,`DATA FIN`) AS dies_passats, h.DNI_TUTOR, c.id_cuho, MINIM_COBRAR, h.preu_alumne, h.irpf, h.perfil FROM cursos as c, honoraris as h, rel_cuho as r WHERE ".$text_h." AND (DATEDIFF(CURRENT_DATE,`DATA FIN`)>=0 ) AND c.pagat = 'N' AND PERFIL = 'tutor' AND c.id_cuho = r.id_cuho AND r.id_hono = h.id AND (c.CURS != 'ACOS' AND c.CURS != 'CPSA') GROUP BY c.CURS, c.ANY, c.MES ORDER BY c.MES, c.CURS, c.ANY");
