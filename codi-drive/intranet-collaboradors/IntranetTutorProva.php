	 ...
	 public function __construct()	{
 		...
 		$this->cnsBD_Web = [
 			...
 			"cnsInscCursosNoAcabats" => "SELECT c.NOM_CURS, c.HORES, p.IMPORT, i.ID,
 						...
 						c.MES = i.MES AND c.CURS = i.CURS INNER JOIN preus AS p ON p.NUM = c.ID_PREU
   ...
   */
	public function mostrarPage($nom, $idPicture, $idUser, $mdlUsername) {
		if ( $this->url->get() == '/collaboradors/' ) {
			...
			$mostrar = $this->__mostrarPage_Tutors_Consulta_Cobraments( $this->url->get() );
		}
	...
	public function __mostrarPage_Tutors_Consulta_Cobraments() {
		//
	}
	...
	private function __retornaTableAlumne_Mostrar_Consulta_Alumne($any, $mes, $curs, $aula) {
		...
		$titolsTableAlumnes[3] = "TREBALLA A";
		$titolsTableAlumnes[4] = "TITULACIÓ";
		...
		$dept = [
			...
			[
				...
				[
					...
					["laura-coto", "Laura Coto"]
				],
				[
					["+34678123687", "678 12 36 87", "(Laura)"],
					["+34682647644", "682 64 76 44", "(Pablo)"]
				]
			],
			["coord", "Coordinació", [["cristina-peinado", "Cristina Peinado"]], [["+34638272362", "638 27 23 62", ""]] ],
		...
		for ( $i=0; $i < count($dept); $i++ ) {
			...
			for ( $j=0; $j < count($dept[$i][2]); $j++ ) {
				$peopleOver .= "<div class='people-dept'>
					<img role='img' class='w-100 mx-2' src='https://campus.prisma.cat/intranet-collaboradors/consultes/img/".$dept[$i][2][$j][0].".jpg?ver=1.0' alt='".$dept[2][$j][1]."'>
	...
	*/
	public function enviarMsgConsultaSecreCoordFact( $typeCons, $nom, $cognoms, $email, $prioritat,
		...
		if ( $typeCons == 'dept-secre' ) {
			...
			$correuTo = "coordinacio@prisma.cat";
			$correuReplyHeadAl = "coordinacio@prisma.cat";
		}
