...
// IMPORTANT!!!
// en el cas de NEURO s'ha de mirar si és una de les dues tutores que porten la mateixa aula (cotutorització)
...
<html>
				...
				if (isloggedin())  // comprovem si està identificat en el campus
			   	{
					...
					if (($USER->username == '33881437') || ($USER->username == '78100121'))
						...
						$result_g = mysqli_query ($conexion, "SELECT codic, guardat, finalitzat FROM informe_tutor, cursos WHERE ".$text." AND codic='".$curs."' AND codic=id_Curs");
	...
	/**************************************************/
							...
							else // mostrem el formulari buit
							{
								...
								$result_dades = mysqli_query ($conexion,"SELECT id_Curs, c.MES, `NOM CURS` AS ncurs, AULA, `DATA FIN` AS dataf, DNI_TUTOR, p.NOM AS tnom, p.COGNOMS AS tcogs, MAIL_PRISMA, m.nom as nmes, SUM(CASE WHEN (`INSC CURS` = '1') THEN 1 ELSE 0 END) inscrits, SUM(CASE WHEN ((`CERTIFICAT` NOT LIKE '%no aprovat%' OR `CERTIFICAT` IS NULL) AND `INSC CURS`='1') THEN 1 ELSE 0 END) aprovats, SUM(CASE WHEN ((`CERTIFICAT` LIKE '%no aprovat%') AND `INSC CURS`='1') THEN 1 ELSE 0 END) pendents, DATEDIFF(CURRENT_DATE,`DATA FIN`) AS dies_passats FROM cursos AS c, personal AS p, inscripcions AS i, mesos AS m WHERE id_Curs='".$curs."' AND ".$text." AND DNI_TUTOR=p.DNI AND c.ANY=i.ANY AND c.MES =i.MES AND c.CURS =i.CURS AND c.AULA=i.Grup AND m.num=i.MES");
