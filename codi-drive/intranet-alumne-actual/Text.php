	...
	class Text
	{
	   ...
	   */
	   public function getDiaSetmana() {
		...
		/**
	   * @brief Retorna true si el text té format de data dd/mm/yyyy.
	   * @return Retorna true si el text té format de data dd/mm/yyyy.
	   */
		function teFormatData() {
			$valid = 0;
			$valor = $this->text;
			if ( strlen($valor) > 0) {
				//separem per l'espai per si el format de les dates té la hora
				$vectDataHora = explode(' ', $valor);
				if (count($vectDataHora) > 0) {
					$vectData = explode('/', $vectDataHora[0]);
					if (count($vectData) > 0 && count($vectData) == 3 && strlen($vectData[0]) == 2
						&& strlen($vectData[1]) == 2 && strlen($vectData[2]) == 4)
						$valid = 1;

					if (count($vectDataHora) > 1) {
						$vectHora = explode(':', $vectDataHora[1]);
						if (count($vectHora) > 0 && (count($vectData) == 2 || count($vectData) == 3) &&
	   ...
	   */
		...
		/**
	   * @brief Reemplaça els . per comes
	   * @return Reemplaça els . per come
	   */
		...
		/**
	   * @brief Reemplaça els . per comes. Afegeix ,00 si el nombre es enter i els símbol d'euros
	   * @return Reemplaça els . per comes. Afegeix ,00 si el nombre es enter i els símbol d'euros
	   */
		function setFormatNumberEuros( ) {
