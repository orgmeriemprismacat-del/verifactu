...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to preview manual group invoices with SIF_ENV=production.\n");
    exit(1);
}
...
try {
    ...
    $snapshot = (new LegacyGroupSnapshotRepository())->loadByIdpag($legacyDb, $idpag, $input['amount']);
    $payload = (new ManualGroupInvoicePayloadBuilder())->buildFromSnapshot($snapshot, $input);
...
function usage(): void
{
    fwrite(
        STDERR,
        "Usage: php sif/scripts/preview-manual-group.php IDPAG AMOUNT MOVEMENT_DATE [--reference=REF] [--bank=BANK] [--notes=TEXT] [--created-by=USER]\n"
    );
    exit(1);
}
