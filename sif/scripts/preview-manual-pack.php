...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to preview manual pack invoices with SIF_ENV=production.\n");
    exit(1);
}
...
try {
    ...
    $snapshot = (new LegacyPackSnapshotRepository())->loadByIdpag($legacyDb, $idpag, $input['amount']);
    $payload = (new ManualPackInvoicePayloadBuilder())->buildFromSnapshot($snapshot, $input);
...
function usage(): void
{
    fwrite(
        STDERR,
        "Usage: php sif/scripts/preview-manual-pack.php IDPAG AMOUNT MOVEMENT_DATE [--reference=REF] [--bank=BANK] [--notes=TEXT] [--created-by=USER]\n"
    );
    exit(1);
}
