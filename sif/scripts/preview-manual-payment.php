...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to preview manual payments with SIF_ENV=production.\n");
    exit(1);
}
...
try {
    ...
    $invoice = loadManualPaymentInvoice($sifDb, $selector);
    $input['num_visible'] = $input['num_visible'] ?? $invoice['NUM_VISIBLE'];
    $payload = (new ManualPaymentPayloadBuilder())->forExistingInvoice($invoice['UUID_FACTURA'], $input);
...
function loadManualPaymentInvoice(\PDO $sifDb, array $selector): array
{
