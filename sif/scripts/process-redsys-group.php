...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to process Redsys group invoices with SIF_ENV=production.\n");
    exit(1);
}
...
if ($dsOrder === '') {
    fwrite(STDERR, "Usage: php sif/scripts/process-redsys-group.php DS_ORDER [--sync-legacy]\n");
    exit(1);
}
...
try {
    ...
    $service = new RedsysGroupInvoiceService(
        $notifications,
