...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to preview manual refunds with SIF_ENV=production.\n");
    exit(1);
}
...
try {
    ...
    $invoice = loadManualRefundInvoice($sifDb, $selector);
    $input['num_visible'] = $input['num_visible'] ?? $invoice['NUM_VISIBLE'];
    $payload = (new ManualRefundPayloadBuilder())->forExistingInvoice($invoice['UUID_FACTURA'], $input);
...
function loadManualRefundInvoice(\PDO $sifDb, array $selector): array
{
