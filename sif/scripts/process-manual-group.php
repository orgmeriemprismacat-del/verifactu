...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to process manual group invoices with SIF_ENV=production.\n");
    exit(1);
}
...
try {
    ...
    $service = new ManualGroupInvoiceService(
        new LegacyGroupSnapshotRepository(),
        new ManualGroupInvoicePayloadBuilder(),
        $invoiceService
    );

    $result = $service->issueFromLegacyGroupPayment($legacyDb, $idpag, $input);
...
function usage(): void
{
    fwrite(
        STDERR,
        "Usage: php sif/scripts/process-manual-group.php IDPAG AMOUNT MOVEMENT_DATE [--reference=REF] [--bank=BANK] [--notes=TEXT] [--created-by=USER] [--sync-legacy]\n"
    );
    exit(1);
}
