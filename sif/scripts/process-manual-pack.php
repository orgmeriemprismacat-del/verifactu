...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to process manual pack invoices with SIF_ENV=production.\n");
    exit(1);
}
...
try {
    ...
    $service = new ManualPackInvoiceService(
        new LegacyPackSnapshotRepository(),
        new ManualPackInvoicePayloadBuilder(),
        $invoiceService
    );

    $result = $service->issueFromLegacyPackPayment($legacyDb, $idpag, $input);
...
function usage(): void
{
    fwrite(
        STDERR,
        "Usage: php sif/scripts/process-manual-pack.php IDPAG AMOUNT MOVEMENT_DATE [--reference=REF] [--bank=BANK] [--notes=TEXT] [--created-by=USER] [--sync-legacy]\n"
    );
    exit(1);
}
