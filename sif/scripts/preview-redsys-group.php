...
if (($config['env'] ?? 'local') === 'production') {
    fwrite(STDERR, "Refusing to preview Redsys group invoices with SIF_ENV=production.\n");
    exit(1);
}

$dsOrder = trim((string) ($argv[1] ?? ''));
if ($dsOrder === '') {
    fwrite(STDERR, "Usage: php sif/scripts/preview-redsys-group.php DS_ORDER\n");
    exit(1);
}
...
try {
    ...
    $snapshot = (new LegacyGroupSnapshotRepository())->loadByIdpag($legacyDb, $idpag, $amount);
    $basePayload = (new LegacyGroupInvoicePayloadBuilder())->build($snapshot);
