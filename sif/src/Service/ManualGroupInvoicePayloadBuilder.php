...
final class ManualGroupInvoicePayloadBuilder
{
    private LegacyGroupInvoicePayloadBuilder $groupPayloads;

    public function __construct(?LegacyGroupInvoicePayloadBuilder $groupPayloads = null)
    {
        $this->groupPayloads = $groupPayloads ?? new LegacyGroupInvoicePayloadBuilder();
    }
    ...
    public function buildFromSnapshot(array $snapshot, array $input): array
    {
        ...
        $snapshot['payment'] = array_replace($snapshot['payment'] ?? [], [
            'amount' => $amount,
            'idpag' => $idpag,
        ]);

        $invoiceKey = $this->invoiceIdempotencyKey($method, $idpag, $amount, $movementDate, $reference, $bank);
        $payload = $this->groupPayloads->build($snapshot);
        $this->assertAmountMatchesInvoiceTotal($amount, $payload);

        $payload['idempotency_key'] = $invoiceKey;
        $payload['source_channel'] = 'INTRANET';
        $payload['created_by'] = $this->optionalString($input, ['created_by', 'user', 'usuari'], 'passar-pagaments-grup');
        $payload['relations'] = $this->withManualRelations($payload['relations'] ?? [], $idpag);
    ...
    private function invoiceIdempotencyKey(
        ...
        if ($reference !== null) {
            return $method . '|GRUP|IDPAG:' . $idpag . '|REF:' . $this->keyPart($reference);
        }
    ...
    private function withManualRelations(array $relations, int $idpag): array
    {
        foreach ($relations as $index => $relation) {
            if (!is_array($relation)) {
                throw SifException::validation('Invalid group relation');
            }
    ...
    private function assertAmountMatchesInvoiceTotal(string $amount, array $payload): void
    {
        ...
        if ($amount !== $total) {
            throw SifException::validation('Manual group payment amount must match group invoice total');
        }
    }
    ...
    private function idpag(array $snapshot, array $input): int
    {
        ...
        $firstInscription = is_array($firstItem) && isset($firstItem['inscription']) && is_array($firstItem['inscription'])
            ? $firstItem['inscription']
            : [];

        $value = $this->optional($input, ['idpag', 'IDPAG'])
            ?? $this->optional($payment, ['idpag', 'IDPAG'])
            ?? $this->optional($firstInscription, ['IDPAG', 'idpag']);

        if ($value === null || $value === '' || !is_numeric($value)) {
            throw SifException::validation('Missing manual group IDPAG');
        }
