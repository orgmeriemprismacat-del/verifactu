...
final class ManualPackInvoicePayloadBuilder
{
    private LegacyPackInvoicePayloadBuilder $packPayloads;

    public function __construct(?LegacyPackInvoicePayloadBuilder $packPayloads = null)
    {
        $this->packPayloads = $packPayloads ?? new LegacyPackInvoicePayloadBuilder();
    }
    ...
    public function buildFromSnapshot(array $snapshot, array $input): array
    {
        ...
        $payload = $this->packPayloads->build($snapshot);
        $this->assertAmountMatchesInvoiceTotal($amount, $payload);

        $payload['idempotency_key'] = $invoiceKey;
        $payload['source_channel'] = 'INTRANET';
        $payload['created_by'] = $this->optionalString($input, ['created_by', 'user', 'usuari'], 'passar-pagaments-pack');
    ...
    private function invoiceIdempotencyKey(
        ...
        if ($reference !== null) {
            return $method . '|PACK|IDPAG:' . $idpag . '|REF:' . $this->keyPart($reference);
        }
    ...
    private function withManualRelations(array $relations, int $idpag): array
    {
        foreach ($relations as $index => $relation) {
            if (!is_array($relation)) {
                throw SifException::validation('Invalid pack relation');
            }
    ...
    private function assertAmountMatchesInvoiceTotal(string $amount, array $payload): void
    {
        ...
        if ($amount !== $total) {
            throw SifException::validation('Manual pack payment amount must match pack invoice total');
        }
    }
    ...
    private function idpag(array $snapshot, array $input): int
    {
        ...
        if ($value === null || $value === '' || !is_numeric($value)) {
            throw SifException::validation('Missing manual pack IDPAG');
        }
