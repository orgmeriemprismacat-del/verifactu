...
final class ManualRefundPayloadBuilder
{
    public function forExistingInvoice(string $uuidFactura, array $input): array
    {
        $uuidFactura = trim($uuidFactura);
        if ($uuidFactura === '') {
            throw SifException::validation('Missing refund invoice UUID');
        }

        $amount = $this->amount($this->required($input, ['amount', 'import', 'refund'], 'refund amount'));
        ...
        $payload = [
            ...
            'allocations' => [[
                ...
                'amount' => $amount,
                'allocation_type' => $this->optionalString($input, ['allocation_type'], 'INVOICE_REFUND'),
            ]],
        ];
    ...
    private function idempotencyKey(
        ...
        if ($reference !== null) {
            return 'REFUND|REF:' . $this->keyPart($reference);
        }
    ...
    private function method(?string $value): string
    {
        ...
        if (!in_array($method, ['TRANSFERENCIA', 'MANUAL'], true)) {
            throw SifException::validation('Invalid manual refund method');
        }
